...
int main(){
	...
	SecuenciaCaracteres secuencia1(cad), secuencia2(cad);

	int max = secuencia1.TotalUtilizados() - 1;

	// Lee una posición

	cout << endl;

	leer.SetTitulo("Primera posicion (entre " + to_string(0) + " y " 
											+ to_string(max) + "): ");

	pos1 = leer.LeeEnteroEnRango(0, max);

	cout << endl;

	// Lee otra posición

	leer.SetTitulo("Segunda posicion (entre " + to_string(0) + " y " 
											+ to_string(max) + "): ");
	
	pos2 = leer.LeeEnteroEnRango(0, max);

	// CÁLCULOS

	secuencia1.EliminaDesde(pos1);

	secuencia2.EliminaHasta(pos2);

	// SALIDAS

	cout << endl;

	cout << "Sus cadenas: " << endl;
	cout << secuencia1.ToString() << endl;
	cout << secuencia2.ToString() << endl;
