...
\begin{document}
    ...
    \begin{ejercicio}[2,25 puntos]
        ...
        \begin{enumerate}
            ...
            parámetro $\theta \in \Theta = \Rmas$, y $\X = \Rmas$. Para hallar el estadístico suficiente,
            usaremos el Teorema \ref{teo:t1}. Primero obtenemos la función de densidad 
            $$f_\theta(x) = \dfrac{\partial F_\theta(x)}{\partial x} = \theta e^{-\theta x}, \quad x > 0$$
            ...
            Suponemos en este punto que $\theta > 0$ (de lo contrario, $f_\theta(x_i) = 0 \quad \forall i = 1, \ldots, n$), y vemos que 
            $$x_i > 0 \quad \forall i = 1, \ldots, n \iff I_{\Rmas}(x_i - 0) = 1 \quad \forall i = 1, \ldots, n \iff I_{\Rmas}(x_{(1)}) = 1$$ de donde se deduce que 
            $$f^n_\theta(x_1, \ldots, x_n) = \prod_{i=1}^{n} \theta e^{-\theta x_i} I_{\Rmas}(x_{(1)}) = 
            I_{\Rmas}(x_{(1)}) \theta^n e^{-\theta \left( \displaystyle\sum_{i=1}^{n} x_i \right)}$$
            Tomando $S(X_1, \ldots, X_n) = \displaystyle\sum_{i=1}^{n} X_i$ y 
            $$h(x_1, \ldots, x_n) = I_{\Rmas}(x_{(1)}), \quad g_\theta(t) = \theta^n e^{-\theta t}$$
            Se cumple que 
            $$f^n_\theta(x_1, \ldots, x_n) = h(x_1, \ldots, x_n) g_\theta(S(x_1, \ldots, x_n)) \quad \forall (x_1, \ldots, x_n) \in \X^n$$
            donde $h$ es independiente del parámetro $\theta$ y $g_\theta$ depende de la muestra solo a través del estadístico, luego,
            por el Teorema de Factorización de Neyman-Fisher, el estadístico $S$ es suficiente. \\

            Sabemos que $S = \sum_{i=1}^{n} X_i \rightsquigarrow \Gamma(n,\theta)$, ya que $X_i \rightsquigarrow \Exp(\theta) \quad \forall i = 1, \ldots, n$.

            Ahora encontraremos el intervalo de confianza por medio del método de la cantidad pivotal. Nos piden que el IC esté basado en un estadístico suficiente, como $S$. Usaremos la función de densidad de la distribución $\Gamma(p,a), p,a > 0$ conocida:
            $$f(x) = \dfrac{a^p}{\Gamma(p)} x^{p-1} e^{-ax}, \quad x > 0$$
            el Teorema de Cambio de Variable, para $Y = g(X)$ $$f_Y(y) = f_X(g^{-1}(y)) \left| \frac{d}{dy} g^{-1}(y) \right|$$
            y la relación vista en teoría entre la distribución $\Gamma(p,a)$ y la $\chi^2(n)$:
            \begin{equation}\label{eq:relchigamma}
                X \rightsquigarrow \chi^2(n) \iff X \rightsquigarrow \Gamma \left( \dfrac{n}{2}, \dfrac{1}{2} \right)
            \end{equation}
            Primero consideramos el cambio $Y = \theta S \Longrightarrow S = Y / \theta$, y entonces
            $$f_Y(y) = f_S(y/\theta) \cdot \dfrac{1}{\theta} = \dfrac{\theta^n}{\Gamma(n)} \left( \dfrac{y}{\theta} \right)^{n-1} e^{-\theta(y/\theta)} \cdot \dfrac{1}{\theta} = 
            \dfrac{1}{\Gamma(n)} y^{n-1} e^{-y} \rightsquigarrow \Gamma(n,1), \quad y > 0$$
            Repetimos el mismo procedimiento con el cambio $W = 2 Y \Longrightarrow Y = 2 / W$, y entonces
            $$f_W(w) = f_Y(w/2) \cdot \dfrac{1}{2} = \dfrac{1}{\Gamma(n)} \left( \dfrac{w}{2} \right)^{n-1} e^{-w/2} \cdot \dfrac{1}{2} = 
            \dfrac{(1/2)^n}{\Gamma(n)} w^{n-1} e^{-w/2} \rightsquigarrow \Gamma \left(n, \dfrac{1}{2} \right), \quad w > 0$$
            Por $(\ref{eq:relchigamma})$, $W \rightsquigarrow \Gamma (n, 1/2) \iff 2 \theta S = W \rightsquigarrow \chi^2(2n)$ y 
            podemos considerar como función pivote, 
            $T : \X^n \times \Theta \to \R$, dado por 
            $$T \equiv T(X_1, \ldots, X_n; \theta) = 2 \theta S(X_1, \ldots, X_n) \equiv 2 \theta S$$ que es
            una variable aleatoria cuya distribución, $\chi^2(2n)$, es independiente del parámetro $\theta$ (solo depende de $n \in \N$).
            Además:
            \begin{enumerate}
                \item $T$ es estrictamente monótona en $\theta$, pues
                $$\dfrac{\partial T}{\partial \theta} = \dfrac{\partial}{\partial \theta} (2 \theta S) = 2 S > 0, \quad \forall \theta \in \Theta, \quad \forall(x_1, \ldots, x_n) \in \X^n = (\Rmas)^n$$
                \item $T = \lm$ tiene solución en $\theta$, para todo $\lm \in \Lambda = \Rmas$, con $\Lambda$ la imagen de $T$, pues
                $$T = \lm \iff 2 \theta S = \lm \iff \theta = \dfrac{\lm}{2 S} \in \Rmas$$ 
            \end{enumerate}
            ...
            Es decir,
            $$\lm_1 < T < \lm_2 \iff \lm_1 < 2 \theta S < \lm_2 \iff \dfrac{\lm_1}{2S} < \theta < \dfrac{\lm_2}{2S}$$
            y el intervalo tendrá longitud $$L = \dfrac{\lm_2}{2S} - \dfrac{\lm_1}{2S} = \dfrac{\lm_2 - \lm_1}{2S}$$ que es constante, luego coincidirá 
            con la longitud esperada $\E_\theta[L] = L \quad \forall \theta \in \Theta$. \\
            
            Sea ahora $F$ la función de distribución de la distribución $\chi^2$ de Pearson $\chi^2(2n)$, 
            ...
            \end{equation*}
            En este caso, la asimetría de la distribución hace que los valores $\lm_1$ y $\lm_2$ no sean los de colas
            iguales. Sin embargo, la diferencia no es suficientemente importante (sobre todo para grandes muestras), y en 
            la práctica se usa el intervalo de colas iguales: 
            $$\lm_1 = \chi^2_{2n; 1 - \alpha/2} \quad \lm_2 = \chi^2_{2n; \alpha/2}$$
            siendo $P[\chi^2_{2n} > \chi^2_{2n;\alpha/2}] = \alpha/2$.
            En conclusión, el intervalo de confianza para $\theta$ de menor longitud media
            uniformemente a nivel de confianza $1-\alpha$, basado en un estadístico suficiente $S$ es:
            \begin{equation*}
                \left] \dfrac{\lm_1}{2S}, \dfrac{\lm_2}{2S} \right[ = \left] \dfrac{\chi^2_{2n; 1 - \alpha/2}}{2S}, \dfrac{\chi^2_{2n; \alpha/2}}{2S} \right[
            \end{equation*}
        \end{enumerate}
    \end{ejercicio}
