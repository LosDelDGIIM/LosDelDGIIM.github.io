...
\begin{ejercicio}
    ...
    Tomando:
    \begin{gather*}
        h(x_1, \ldots, x_n) = \prod_{i=1}^{n}\dfrac{1}{\sqrt{x_i}}\cdot I_{\left]0,+\infty\right[}(X_{(1)}-0)  , \qquad T(X_1, \ldots, X_n) = X_{(n)} \\
        g_\theta(t) = \dfrac{I_{\left]-\infty,0\right[}(t-\theta)}{{\left(2\sqrt{\theta}\right)}^{n}} 
    \end{gather*}
...
\begin{ejercicio}
    ...
    \begin{enumerate}[label=\alph*)]
        \item Probar que la familia de distribuciones de $X$ es regular y calcular la función de información asociada a la muestra.
            ...
            \begin{align*}
                I_X(\theta) &= Var\left(\dfrac{1-\theta - X\theta}{\theta(1-\theta)}\right) = \dfrac{\theta^2}{\theta^2{(1-\theta)}^{2}}Var(X) \\
        ...
        \item Especificar la clase de funciones paramétricas que admiten estimadores eficientes y los correspondientes estimadores.
            ...
            \begin{equation*}
                T(\cc{X}^n) = [0,n]\cap \mathbb{N} \subset g(\left]0,1\right[)
            \end{equation*}
            ...
            \begin{equation*}
                a\cdot \frac{n(1-\theta)}{\theta} + b, \qquad a,b\in \mathbb{R}, \quad a\neq 0
            \end{equation*}
        ...
        \item Calcular la varianza de cada estimador eficiente y comprobar que coincide con las correspondiente cota de Fréchet-Cramér-Rao.
            ...
            \begin{align*}
                Var(T) &= Var\left(a\cdot \sum_{i=1}^{n}X_i+b\right) \stackrel{\text{indep.}}{=} a^2\sum_{i=1}^{n}Var(X_i) = a^2\sum_{i=1}^{n} \left(\frac{1-\theta}{\theta^2}\right) \\ &= \frac{a^2n(1-\theta)}{\theta^2} \\
        ...
        \item Calcular, si existen, los UMVUE para $P_\theta[X=0]$ y para $E_\theta[X]$ y decir si son eficientes. 
            \begin{itemize}
                \item Para $P_\theta[X=0]$:
                    ...
                    \begin{equation*}
                        E[h(T)] = \sum_{t=0}^{\infty} h(t) \dfrac{(t+n-1)!}{t!(n-1)!}{(1-\theta)}^{t}\theta^n = \theta
                    \end{equation*}
                    Es decir, que cumpla (dividiendo todo entre $\theta$):
                    \begin{equation*}
                        E[h(T)] = \sum_{t=0}^{\infty} h(t) \dfrac{(t+n-1)!}{t!(n-1)!}{(1-\theta)}^{t}\theta^{n-1} = 1
                    \end{equation*}
                    Notemos que si $Y\rightsquigarrow B(n-1,\theta)$, tendremos entonces que:
                    \begin{equation*}
                        P[Y=t] = \binom{t+n-2}{t}{(1-\theta)}^{t}\theta^{n-1} = \dfrac{(t+n-2)!}{t!(n-2)!}{(1-\theta)}^{t}\theta^{n-1}
                    \end{equation*}
                    Por lo que ha de ser (por ser una función masa de probabilidad):
                    \begin{equation*}
                        1 = \sum_{t=0}^{\infty}\dfrac{(t+n-2)!}{t!(n-2)!}{(1-\theta)}^{t}\theta^{n-1}
                    \end{equation*}
                    ...
                    \begin{itemize}
                        \item $h(T)$ es un estimador, puesto que $h(T)\in \left]0,1\right[$.
                        \item Calculamos su momento de segundo orden:
                            \begin{align*}
                                E[{(h(t))}^{2}] &=\sum_{t=0}^{\infty}{(h(t))}^{2}\dfrac{(t+n-1)!}{t!(n-1)!}{(1-\theta)}^{t}\theta^n \\
                                                &=\sum_{t=0}^{\infty}{\left(\frac{n-1}{t+n-1}\right)}^{2}\dfrac{(t+n-1)!}{t!(n-1)!}{(1-\theta)}^{t}\theta^n
                            \end{align*}
                            Como tenemos que $h(t)\in \left]0,1\right[$ para todo $t\in \mathbb{N}$, tenemos que ${(h(t))}^{2}\in \left]0,1\right[$, por lo que cada sumando de esta nueva serie es menor o igual que cada sumando de la serie $E[h(t)]$, que era convergente, por lo que por el criterio de comparación esta nueva serie también será convergente, $E[{(h(t))}^{2}]<\infty$.
                    \end{itemize}
                ...
                \item Para $E_\theta[X]$:
                    ...
                    \end{equation*}
                    Por el apartado anterior tenemos ya que $T=\sum\limits_{i=1}^{n}X_i$ es un estadístico suficiente y completo. Buscamos ahora una función $h$ medible tal que $h(T)$ sea insesgado para $\frac{1-\theta}{\theta}$, es decir:
                    \begin{equation*}
                    ...
                    \begin{equation}\label{eq:despejar_ej8}
                        \sum_{t=0}^{\infty}h(t) \frac{(t+n-1)!}{t!(n-1)!}{(1-\theta)}^{t-1}\theta^{n+1} = 1
                    \end{equation}
                    Si $Y\rightsquigarrow BN(n+1,\theta)$, entonces:
                    \begin{equation*}
                        P[Y=t] = \binom{n+t}{t}{(1-\theta)}^{t}\theta^{n+1} = \dfrac{(t+n)!}{t!n!}{(1-\theta)}^{t}\theta^{n+1}
                    \end{equation*}
                    De donde sabemos que:
                    \begin{equation*}
                        \sum_{t=0}^{\infty}\dfrac{(t+n)!}{t!n!}{(1-\theta)}^{t}\theta^{n+1} = 1
                    \end{equation*}
                    Si sacamos fuera de la suma de~(\ref{eq:despejar_ej8}) el primer sumando, tenemos:
                    \begin{equation*}
                    ...
                    \begin{equation*}
                        h(t) = \frac{t}{n}
                    \end{equation*}
                    tendremos en particular que $h(0) = 0$, con lo que el primer sumando se anula y el segundo coincide con la suma en $\mathbb{N}$ de la función masa de probablidad de una variable aleatoria con distribución binomial negativa, que ha de sumar 1, como queríamos en la igualdad~(\ref{eq:despejar_ej8}). Observemos ahora que:
                    \begin{itemize}
                        \item $h(t)\in \left[0,1\right]$ para todo $t\in \{0, \ldots, n\}$, por lo que $h(T)$ es un estimador.
                        \item Para el momento de segundo orden:
                            \begin{equation*}
                                E[{(h(t))}^{2}] = \sum_{t=0}^{\infty}\frac{t^2}{n^2}\frac{(t+n-1)!}{t!(n-1)!}{(1-\theta)}^{t}\theta^n 
                            \end{equation*}
                            Aplicamos el criterio del cociente:
                            \begin{align*}
                                &\dfrac{\dfrac{{(t+1)}^{2}(t+n)!}{\cancel{n^2}(t+1)!\cancel{(n-1)!}}{(1-\theta)}^{t+1}\cancel{\theta^n}}{\dfrac{t^2(t+n-1)!}{\cancel{n^2}t!\cancel{(n-1)!}}{(1-\theta)}^{t}\cancel{\theta^n}} = \dfrac{{(t+1)}^{2}(t+n)!t!(1-\theta)}{(t+1)!t^2(t+n-1)!} \\ &= \dfrac{{(t+1)}^{2}(t+n)(1-\theta)}{t^2(t+1)} = \dfrac{(t+1)(t+n)(1-\theta)}{t^2} \rightarrow 1-\theta
                            \end{align*}
                            Con $\theta\in \left]0,1\right[$, por lo que $1-\theta\in \left]0,1\right[$, de donde la serie es convergente.
                    \end{itemize}
            ...
            Por lo que:
            \begin{itemize}
                \item Como $\theta$ no está en dicha familia de funciones paramétricas, el UMVUE obtenido para $\theta$ no puede ser eficiente.
                \item Si tomamos $a = \frac{1}{n}$ y $b=0$, tenemos que l UMVUE obtenido para $\frac{1-\theta}{\theta}$ coincide con el estimador eficiente en dicho caso.
            \end{itemize}
    \end{enumerate}
\end{ejercicio}
...
\begin{ejercicio}\label{ej:10_rel4} 
