...
\begin{ejercicio}
    ...
    \begin{enumerate}[label=\alph*)]
        ...
        \item Buscamos ahora comprobar si $h(T) = \frac{T}{n}{\left(1-\frac{T}{n}\right)}^{2}$ es insesgado para $p{(1-p)}^{2}$.  % // TODO: PEDIR A IRINA
    \end{enumerate}
\end{ejercicio}
...
\begin{ejercicio}
    ...
    \begin{itemize}
        ...
        \item % // TODO: Momento de 2o orden
    \end{itemize}
    % Entonces:
    % \begin{equation*}
    %     N^{n+1} = h(1) + h(2)(2^n-1) + \ldots + h(N) (N^n - {(N-1)}^{n})
    % \end{equation*}

    % de donde:
    % \begin{equation*}
    %     N^{n+1} = 1^n (h(1)-h(2)) + 2^n (h(2) - h(3)) +\ldots + {(N-1)}^{n}(h(N-1)-h(N)) + N^nh(N)
    % \end{equation*}

    % tomamos:
    % \begin{align*}
    %     h(1) - h(2) &= N^n = {\left(\frac{N}{1}\right)}^{n} \\
    %     h(2) - h(3) &= \frac{N^n}{2^n} = {\left(\frac{N}{2}\right)}^{n} \\
    %     h(3) - h(4) &= {\left(\dfrac{N}{3}\right)}^{n} \\
    %                 &\vdots \\
    %     h(N-1) - h(N) &= {\left(\frac{N}{N-1}\right)}^{n} \\
    %     h(N) &= 1
    % \end{align*}
    % Si sumamos las desigualdades desde la $i-$ésima en adelante ($j = 1, \ldots, N$), obtenemos:
    % \begin{equation*}
    %     h(i) = \sum_{j=i}^{N}{\left(\frac{N}{j}\right)}^{n}\Longrightarrow h(T) \text{\ es insesgado por construcción}
    % \end{equation*}
    % Veamos el momento de segundo orden:
    % \begin{equation*}
    %     E[{(h(T))}^{2}] = \sum_{t=1}^{N}{(h(t))}^{2} \dfrac{t^n - {(t-1)}^{n}}{N} < \infty
    % \end{equation*}
    % De donde $h(T)$ es el UMVUE.\\
\end{ejercicio}
