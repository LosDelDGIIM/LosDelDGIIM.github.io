...
% % Hechos: 1, 2, 3, 4, 9, 12, 13, 14

% // TODO: EN el 2 no se toma supremo, se ponene las 2
% // TODO: donde no diga nada de sigma = sigma, comprobar si son = y decir que la muestra no lo contradice
% // TODO: El 13 está mal, H0: mu1 <= mu2
...
\begin{ejercicio}
    ...
    \begin{gather*}
        L_x(p) = P_p[X=x] = \binom{n}{x}p^x {(1-p)}^{n-x} \\ 
        \ln L_x(p) =\ln\binom{n}{x} + x\ln p + (n-x)\ln(1-p) \\
    ...
    \begin{equation*}
        \frac{L_x(p_0)}{L_x(\nicefrac{x}{n})} = \frac{\binom{n}{x}p_0^x {(1-p_0)}^{n-x}}{\binom{n}{x}{\left(\frac{x}{n}\right)}^{x}{\left(1-\frac{x}{n}\right)}^{n-x}} = {\left(\frac{np_0}{x}\right)}^{x} {\left(\frac{1-p_0}{1-\frac{x}{n}}\right)}^{n-x}
    \end{equation*}
    ...
    \begin{gather*}
        h(p) = P_p[X=t] = \binom{n}{x} p^t {(1-p)}^{n-t} \quad\Longrightarrow\quad \ln h(p) =\ln\binom{n}{x}+ t\ln p + (n-t)\ln (1-p) \\
        \dfrac{\partial \ln h(p)}{\partial p} = \frac{t-np}{p(1-p)}
    \end{gather*}
    Observamos ahora que:
    \begin{equation*}
        t \geq \lfloor k\rfloor +1 > k \geq np_0 \geq np 
    \end{equation*}
...
\begin{ejercicio}% // TODO: HACER
    ...
    \noindent
    Sale 1 si $36  > \chi^2_{25,;0.02}$, $\chi^2_{25;0.02}$. Los datos no dan evidencia para rechazar.
\end{ejercicio}
...
\begin{ejercicio}
    ...
    Si tomamos:
    \begin{gather*}
        X\equiv \text{``Puntuaciones del grupo A (nuevo método)''} \rightsquigarrow\cc{N}(\mu_1,\sigma_1^2) \\
        Y\equiv \text{``Puntuaciones del grupo B (método tradicional)''} \rightsquigarrow\cc{N}(\mu_2,\sigma_2^2) 
    \end{gather*}
    Consideramos la hipótesis $\mu_1>\mu_2 \Longleftrightarrow \mu_1-\mu_2>0$. Como no sabemos resolver este tipo de contrastes (donde en $H_0$ no aparece una igualdad), consideramos que esta hipótesis es $H_1$, por lo que el contraste sería el siguiente:
    \begin{equation*}
        \left\{\begin{array}{l}
            H_0 : \mu_1-\mu_2\leq 0 \\
            H_1 : \mu_1-\mu_2> 0
        \end{array}\right.
    \end{equation*} % // TODO: CAMBIAR (Hacer lo de sigma = sigma)
    ...
    \end{equation*}
    Como $t_{exp}$ no es mayor que $t_{n_1+n_2-2;\alpha}$, los datos no proporcionan evidencia suficiente para rechazar el nuevo método a nivel de significación $0.05$. %// TODO: ?
\end{ejercicio}
