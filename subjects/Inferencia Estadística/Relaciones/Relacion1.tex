...
% // TODO: Pasar a limpio a partir de aqui
...
\begin{ejercicio}
    ...
    \noindent
    Sea $n=50$, tenemos una m.a.s. $(X_1, \ldots, X_n)$ idénticamente distribuidas a $X\rightsquigarrow \cc{P}(2)$.
    \begin{equation*}
        F_X(2) = \sum_{k=0}^{2} e^{-2}\dfrac{2^k}{k!} =  e^{-2}\left(\dfrac{2^0}{0!} + \dfrac{2^1}{1!} + \dfrac{2^2}{2!}\right) = e^{-2} (1+2+2) = 0.6767
    \end{equation*}
    \begin{align*}
        P\left[|F_{X_1, \ldots, X_n}^\ast(2) - F_X(2)| < 0.02\right] &= P[-0.02 < F_{X_1, \ldots, X_n}^\ast(2) - 0.6767 < 0.02] \\
                                                                     &= P[32.835 < 50\cdot F{X_1, \ldots, X_n}^\ast(2) < 34.835]
    \end{align*}
    Tomando $Y = 50F_{X_1, \ldots, X_n}^\ast(2) \rightsquigarrow B(50, F_X(2))$, tenemos:
    \begin{equation*}
        P\left[|F_{X_1, \ldots, X_n}^\ast(2) - F_X(2)| < 0.02\right] = P[Y=33] + P[Y=34] = 0.2397
    \end{equation*}
    Donde para la última igualdad hemos tenido que consultar la tabla de la Binomial.\\

    \noindent
    Ahora, calculemos el tamaño $n$ para que la probabilidad sea como mínimo $0.99$. Sabemos para $n$ grande que:
    \begin{equation*}
        Y = nF_{X_1, \ldots, X_n}^\ast(2) \rightsquigarrow \cc{N}(nF_X(2), nF_X(2)(1-F_X(2)))
    \end{equation*}
    Por lo que:
    \begin{equation*}
        Z = \dfrac{n(F_{X_1, \ldots, X_n}^\ast(2) - F_X(2))}{\sqrt{nF_X(2)(1-F_X(2))}} \rightsquigarrow \cc{N}(0,1)
    \end{equation*}
    \begin{align*}
        0.99 &\leq P[|F_{X_1, \ldots, X_n}^\ast - F_X(2)| < 0.02] = P\left[|Z| < \dfrac{n\cdot 0.02}{\sqrt{nF_X(2)(1-F_X(2))}}\right] \\
             &= P[|Z| < \sqrt{n}\cdot 0.04276] = 2P[Z < \sqrt{n}\cdot 0.04276] -1
    \end{align*}
    \begin{equation*}
        P[Z<\sqrt{n}\cdot 0.04276] = 0.995 \Longrightarrow 2.65 = \sqrt{n}\cdot 0.04276 \Longrightarrow n = 3639.8
    \end{equation*}
    \begin{equation*}
        n \geq 3640
    \end{equation*}
...
\begin{ejercicio}
    ...
    \begin{equation*}
        F_\theta(x) = \int_{\theta}^{x} f_\theta(t)~dt = \int_{\theta}^{x} e^{\theta-t}~dt  = \left[-e^{\theta-t}\right]_0^x = 1 - e^{\theta - x} \qquad \forall x\in \mathbb{R}^+
    \end{equation*}
    Para el máximo:
    \begin{equation*}
