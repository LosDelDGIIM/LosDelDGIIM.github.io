...
\begin{ejercicio}\label{Ej:Tema2_2}
    ...
    \begin{enumerate}
        ...
        La situación está ilustrada en la figura \ref{fig:Ej:Tema2_2.1}.
        \begin{figure}
            \centering
            \begin{tikzpicture}
                ...
                % Puntos de interés
                \filldraw[black, red] (0,0) circle (2pt) node[anchor=north east, red] {$x$};
                \filldraw[black, blue] (3,0) circle (2pt) node[anchor=north west, blue] {$y$};
                ...
                \node at (5,1) [blue] {$B(y,\rho)$};
            \end{tikzpicture}
            \caption{Situación del apartado \ref{Ej:Tema2_2.1} del ejercicio \ref{Ej:Tema2_2}.}
            \label{fig:Ej:Tema2_2.1}
        \end{figure}
        ...
        \item $B(y,\rho)\subset B(x,r) \Longleftrightarrow ||y-x|| \leq r-\rho$. \label{Ej:Tema2_2.2}\\
        
        La situación está ilustrada en la figura \ref{fig:Ej:Tema2_2.2}.
        \begin{figure}
            \centering
            \begin{tikzpicture}
                \draw[thick, red] (0,0) circle (2cm);
                \draw[thick, blue] (1,0) circle (0.8cm);

                % Puntos de interés
                \filldraw[black, red] (0,0) circle (2pt) node[anchor=north east, red] {$x$};
                \filldraw[black, blue] (1,0) circle (2pt) node[anchor=north east, blue] {$y$};

                % Etiquetas de los radios
                \draw[<->, dashed, red] (0,0) -- (0,2) node[midway, left] {$r$};
                \draw[<->, dashed, blue] (1,0) -- (1,0.8) node[midway, right] {$\rho$};

                % Punto z
                \filldraw[black] (1.4,0) circle (2pt) node[anchor=north] {$z$};

                % Etiqueta de las bolas
                \node at (-2.2,1.7) [red] {$B(x,r)$};
                \node at (-0.5,-0.7) [blue] {$B(y,\rho)$};
            \end{tikzpicture}
            \caption{Situación del apartado \ref{Ej:Tema2_2.2} del ejercicio \ref{Ej:Tema2_2}.}
            \label{fig:Ej:Tema2_2.2}
        \end{figure}

        \begin{description}
            \item[$\Longrightarrow)$] Supuesto $x=y$, tenemos que $B(y,\rho)\subset B(x,r)$ si y solo si $\rho\leq r$, lo cual es equivalente a $||y-x||=0\leq r-\rho$. Supongamos por tanto que $x\neq y$.            
            
            Sea $z=y+\lm \dfrac{y-x}{\|y-x\|}$, con $\lm\in \left]0,\rho\right[$. Entonces, tenemos que:
            \begin{equation*}
                \|z-y\| = \left\|\lm \dfrac{y-x}{\|y-x\|}\right\| = \lm \left\|\dfrac{y-x}{\|y-x\|}\right\| = \lm < \rho
            \end{equation*}

            Por tanto, $z\in B(y,\rho)$. Como $B(y,\rho)\subset B(x,r)$, tenemos que $z\in B(x,r)$. Es decir, $\|z-x\|<r$. Entonces:
            \begin{align*}
                \|z-x\| &= \left\|y-x+\lm \dfrac{y-x}{\|y-x\|}\right\| = \left\|(y-x)\left(\frac{\lm}{\|y-x\|}+1\right)\right\| =\\&= 
                \|y-x\|\left(\frac{\lm}{\|y-x\|}+1\right) = \|y-x\|+\lm
            \end{align*}

            Por tanto, tenemos que $\|y-x\|=\|z-x\|-\lm<r-\lm$. Puesto que $\lm\in \left]0,\rho\right[$ ra arbitrario, tomamos límite cuando $\lm\to \rho$, y tenemos que $\|y-x\|\leq r-\rho$.
