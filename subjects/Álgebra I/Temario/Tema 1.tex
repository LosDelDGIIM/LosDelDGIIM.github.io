...
\begin{definicion}
    Dado un conjunto $X$, si $X$ tiene un número finito de elementos, diremos que $X$ es un \textbf{conjunto finito}. En caso contrario, diremos que $X$ es un \textbf{conjunto infinito}.
\end{definicion}

\begin{definicion}[Cardinal de un conjunto]
    Dado un conjunto finito $X$, llamamos \emph{cardinal} al número de elementos de $X$. Es decir, si $X$ tiene $n$ elementos, diremos que su cardinal es $n$, y lo notaremos por:
    $$|X| = \#X = n$$

    Si $X$ es un conjunto infinito, diremos que su cardinal es infinito.
\end{definicion}
...
\begin{definicion}[Proyección canónica]
    Sea $X$ un conjunto no vacío y $R$ una relación de equivalencia sobre $X$. Definimos la \textbf{proyección canónica} como la aplicación:
    \Func{\pi}{X}{X/R}{x}{[x]}
\end{definicion}

Es claro que $\pi$ es sobreyectiva, ya que $\forall [a] \in X/R$, se tiene que $\pi(a) = [a]$.
...
\begin{prop}
    ...
    \Func{\ol{f}}{X/R}{Y}{[a]}{\ol{f}([a])=f(a)}

    \begin{figure}[h]
        \centering
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAA0QBfU9TXfIRQBGclVqMWbAJrdeIDNjwEiZYePrNWiDgHoASt3EwoAc3hFQAMwBOEALZIyIHBCSiJWtlZDUGdACMYBgAFfmUhEAYYKxw5aztHRGdXJAAmak0pHQAdHLQsXyjA4LClQTYbLFMACzieBId06lTEDyztEDyIBmArLiL-INDwip1o2KMuIA
        \begin{tikzcd}
        X \arrow[r, "f"] \arrow[d, "\pi"'] & Y \\
        X/R \arrow[ru, "\ol{f}"]           &  
        \end{tikzcd}
        \caption{Diagrama conmutativo de la proposición.}
        \label{fig:DiagramaConmutativo}
    \end{figure}
    
    En la Figura~\ref{fig:DiagramaConmutativo} se muestra el diagrama conmutativo de la proposición, donde $\pi$ es la proyección canónica.
    Se verifica que:
    \begin{enumerate}
