...
\begin{definicion}[Anillo Conmutativo, \emph{Emmy Noether, 1921}]
    ...
    \begin{enumerate}
        ...
        \item Existencia de opuesto: $\forall a\in A~\exists -a \in A \mid a + (-a) = n$.\label{item:opuesto}
...
Notemos que las primeras cuatro propiedades se refieren a la operación aditiva, mientras que las tres siguientes se refieren a la operación multiplicativa. La última propiedad relaciona ambas operaciones. Es importante notar que no se exige la existencia de inversos multiplicativos, es decir, no se requiere que para cada $a \in A$ exista un $b \in A$ tal que $ab = 1$.\\
...
\begin{ejemplo}
    ...
    \begin{enumerate}
        ...
        Notemos que $\N$ no lo es (no cumple la Propiedad~\ref{item:opuesto} de la definición de anillo conmutativo).
        ...
        \Func{f+g}{[0,1]}{\R}{x}{(f+g)(x) = f(x) + g(x)}
        \Func{f\cdot g}{[0,1]}{\R}{x}{(f\cdot g)(x) = f(x) g(x)}

        El cero y el uno son las aplicaciones constantes $0$ y $1$, respectivamente:
        \Func{0}{[0,1]}{\R}{x}{0(x) = 0}
        \Func{1}{[0,1]}{\R}{x}{1(x) = 1}

        La aplicación opuesta de $f$ es la aplicación $-f$ definida por:
        \Func{-f}{[0,1]}{\R}{x}{(-f)(x) = -f(x)}

    \item Sea $n \geq 2$, entonces $\cc{M}_n(\R)$, conjunto de matrices cuadradas de orden $n$ con entradas reales es un ejemplo de anillo no conmutativo\footnote{Ya que si $A$ y $B\in \cc{M}_n(\mathbb{R})$, puede suceder que $A\cdot B\neq B\cdot A$.} con la suma y el producto de matrices.
...
\begin{definicion}
    ...
    \begin{equation*}
        aR_nb \Longleftrightarrow \exists q \in \Z \mid a-b = qn
    \end{equation*}
\end{definicion}
...
Aunque el siguiente concepto se desarrollará con profundidad en la asignatura de Álgebra III, lo introducimos aquí para poder hablar de cuerpos y de campos.
...
\begin{prop}
    Sea $A$ un anillo conmutativo y sea $n \geq 1$. Sea $(a_1, a_2, \ldots, a_n) \in A^n$. Entonces:
...
\begin{prop}
    Sea $A$ un anillo conmutativo y sean $m,n \in \Z$, $a,b \in A$, $u,v \in \cc{U}(A)$. Se verifican:
    \begin{enumerate}
        \item $(m+n)a = ma+na$
        \item $n(a+b) = na + nb$
        \item $n(ma) = (nm)a$
...
\begin{enumerate}
    ...
    \item La proyección canónica en $\bb{Z}_n$: 
    \begin{enumerate}
        \item $\pi(a+b) = [a+b] = [a]+[b] = \pi(a)+\pi(b)$.
        \item $\pi(ab) = [ab] = [a][b] = \pi(a)\pi(b)$.
        \item $\pi(1) = [1]$.
    \end{enumerate}
\end{enumerate}
\end{ejemplo}
...
Notemos que la definición, junto con la anterior proposición, nos muestran que un homomorfismo de anillos mantiene la estructura de anillo, es decir, preserva la suma, el producto y el elemento neutro. Además, también preserva los elementos inversos y las potencias de los elementos del anillo.
...
\end{prop}
\begin{proof}
    Basta con demostrar que $Img(\phi)$ es cerrado para la suma, producto y opuesto de $A'$ y que contiene al 1 y al 0 de $A'$.\\
...
\begin{prop}
    Sea $\phi:A\longrightarrow A'$ un isomorfismo de anillos. Entonces, se verifica que $\phi^{-1}:A'\longrightarrow A$ es también un isomorfismo.
\end{prop}
\begin{proof}
...
\begin{ejemplo}
    Consideramos la proyección canónica en $\Z_5$:
    $$\pi(12^3) = \pi(12)^3 = 2^3 = 3$$
\end{ejemplo}
...
Puesto que hemos demostrado que los homomorfismos llevan unidades en unidades, tenemos el siguiente resultado de demostración evidente.
\begin{prop}
    Sean $A,B$ dos anillos conmutativos isomorfos. Entonces:
    $$|\cc{U}(A)| = |\cc{U}(B)|$$
\end{prop}
...
\end{itemize}
Esto se debe a que, como no pertenecen al mismo anillo, por norma general no podemos operar entre ellos, pero al identificar $A$ con $Img(\iota)$, podemos operar entre ellos como si pertenecieran al mismo anillo.

\begin{ejemplo}
    Sea $A = \{f:[0,1]\longrightarrow \R \mid f \mbox{ aplicación}\}$.
    
    Consideramos la aplicación que incluye a los números reales en $A$ como funciones constantes:
    \Func{\iota}{\R}{A}{r}{\iota(r)}

    Donde, para cada $r \in \R$, $\iota(r)$ es la aplicación constante:
    \Func{\iota(r)}{[0,1]}{\R}{t}{\iota(r)(t)=r}
    
    Veamos ahora que $\iota$ es un monomorfismo de anillos. Probamos primero que $\iota$ es un homomorfismo. Sean $a,b \in \R$, $\forall t \in [0,1]$:
    \begin{gather*}
...
Veamos ahora el siguiente teorema, que introducirá de manera formal los polinomios, que con seguridad son ya conocidos por el lector.
\begin{teo}
    Sea $A$ un anillo conmutativo cualquiera. Entonces exite otro anillo conmutativo $P$ que contiene una copia isomorfa de $A$
...
La siguiente propiedad es muy importante y nos permitirá definir de manera formal la evaluación de un polinomio en un elemento de un anillo.
\begin{teo}[Propiedad universal del anillo de polinomios]
    Sean $A$, $B$ anillos conmutativos y $\phi:A\longrightarrow B$ un homomorfismo de anillos conmutativos. Entonces, para todo $b\in B$, existe un único homomorfismo $\phi_b:A[x]\longrightarrow B$ tal que:
    \begin{enumerate}
        \item $\phi_b(a) = \phi(a), \quad \forall a\in A$,
        \item $\phi_b(x) = b$.
    \end{enumerate}
    La propiedad universal del anillo de polinomios se representa mediante el diagrama de la Figura~\ref{fig:propiedad_universal}.
    \begin{figure}[h]
        \centering
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEQBfU9TXfIRQBGclVqMWbdsgAeFbrxAZseAkVHDx9Zq0QgAQt3EwoAc3hFQAMwBOEALZIyIHBCSiQDOgCMYDAAV+NSEvGGscEGodKX0AHTj8HDpFG3snRBc3JAAmaMk9EAS0AAssVJA7Rw9qbMQ8iV02BJhZLDgcOAB9YQA-AAJisq6fKK8sMEK4CAYsKGMuIA
        \begin{tikzcd}[scale=2]
        A \arrow[r, "i"] \arrow[rd, "\phi"] & {A[x]} \arrow[d, "\exists_1~ \phi_b"] \\
                                                & B                                    
        \end{tikzcd}
        \caption{Diagrama de la propiedad universal del anillo de polinomios.}
        \label{fig:propiedad_universal}
    \end{figure}
\end{teo}
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item $\phi_b(x) = b$.
        $$\phi_b(x) = \sum_{i=0}^1 \phi(a_i) b^i = 0 b^0 + 1 b^1 = 0 + b = b$$
    \end{enumerate}
...
\begin{definicion}[Homomorfismo de evaluación de polinomios]
    ...
    Entonces, por el teorema anterior tomando $\phi=i$, se tiene que para todo $b \in B$, existe un único homomorfismo $ev_b:A[x]\longrightarrow B$ tal que:
    \begin{enumerate}
...
\begin{teo}[Homomorfismo inducido en el anillo de polinomios]
    ...
    \begin{figure}[h]
        \centering
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEQBfU9TXfIRQBGclVqMWbdsgAeFbrxAZseAkVHDx9Zq0QgAQnIU8+qwUTJbqOqfoPdxMKAHN4RUADMAThAC2SGQgOBBIoiAMdABGMAwACvxqQhEwnjggNpJ6IFiKXr4BiOEhSABMmbpsADpVMLJYcDhwAPrCAH4ABDVoABa51AxYYNlwEINQeSA+-oHUJYgAzBV2IN19GRHRsQnm6vreWC496aZTBUhLwaGI5RKV+rlcFFxAA
        \begin{tikzcd}
        A \arrow[r, "i"] \arrow[d, "\phi"'] & {A[x]} \arrow[d, "\exists_1~ \phi"] \\
        B \arrow[r, "i"]                    & {B[x]}                             
        \end{tikzcd}
        \caption{Diagrama del homomorfismo inducido en el anillo de polinomios.}
        \label{fig:homomorfismo_inducido}
    \end{figure}

    Esta propiedad se representa mediante el diagrama de la Figura~\ref{fig:homomorfismo_inducido}. El homomorfismo inducido está definido por:
