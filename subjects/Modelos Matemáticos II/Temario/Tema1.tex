...
% Van a salir ecuaciones de Euler-Lagrange del siguiente tipo
% Respecto a ecuaciones
% - Variables separadas
% - Lineales homogéneas con coeficientes constantes
% - Ecuaciones de tipo Euler (g,h,i): coeficientes son potencias y el coeficiente que aparece es el orden de la derivada
%   Como en coef es el mismo orden, se hace siempre un mismo cambio de variable, x = e^u
% - Versiones no homogéneas de lo anterior (método de coefs. indeterminados)

\subsection{Método de los coeficientes indeterminados}
para resolver una lineal completa se obtiene una solución particular y se le suman las soluciones de la homogénea asociada.\\
...
\noindent
Queremos ver que $y_\ast$ es un mínimo local para $\cc{F}$, con lo que debería existir un entorno en el que $\cc{F}(y_\ast)\leq \cc{F}(y)$ para todo $y$ en el entorno.

Basta estudiar el funcional en puntos de la forma $y_\ast+\varepsilon\varphi$ para $\varphi$ dirección admisible y $\varepsilon$ pequeño ($|\varepsilon| << 1$).\\

\noindent
Desarrollando por Taylor en $\varepsilon$, para estudiar si $y_\varepsilon$ es mejor que $y_\ast$ se escribe:
\begin{equation*}
...
\end{equation*}
Y nos interesa comparar $\cc{F}(y)$ con $\cc{F}(y_\ast + \varepsilon\varphi)$. Si tenemos un punto crítico la primera derivada será igual a 0, con lo que tenemos que estudiar el desarrollo hasta orden 2.
