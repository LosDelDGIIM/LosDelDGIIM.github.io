...
\begin{ejemplo}
    ...
    de donde:
    \begin{equation*}
        {(y'(x))}^{2} = c_1^2 \left( 1+{(y'(x))}^{2} \right) \quad\Longleftrightarrow\quad {(y'(x))}^{2} = c_2
    \end{equation*}
...
\begin{lema}\label{lema:criterio_convexidad}
    ...
    \begin{proof}
        ...
        \begin{multline*}
            F(x, \theta y_1(x) + (1-\theta)y_2(x), \theta y_1'(x) + (1-\theta)y_2'(x)) \\ \leq \theta F(x,y_1(x),y_1'(x)) + (1-\theta) F(x,y_2(x),y_2'(x)) \qquad \forall \theta \in  [0,1] \quad \forall x\in I
        \end{multline*}
...
\begin{equation*}
    \langle Au, v  \rangle  = \langle u,A^T v \rangle  \qquad \forall u,v\in \mathbb{R}^d
\end{equation*}
En el caso de que $A$ sea simétrica que cumple que:
\begin{equation*}
    \langle Au,v \rangle  = \langle u,Av \rangle  \qquad \forall u,v\in \mathbb{R}^d
\end{equation*}
Así, el Teorema Fundamental para $A$ simétrica sería:
\begin{enumerate}
    \item Estudiar $\ker A$, resolvemos el sistema homogéneo $Ax = 0$.
    \item Si $\ker A = \{0\}$, entonces $Im A = \mathbb{R}^d$, por lo que $Ax = b$ tiene solución única para cualquier $b\in \mathbb{R}^d$.
...
\end{enumerate}
Dados $p(x),q(x)$, podemos definir $A:C_c^\infty(I)\to C_c^\infty(I)$ dada por:
\begin{equation*}
    Ay = (p(x)y'(x))' + q(x)y(x), \qquad C_c^\infty(I) \subset L^2(I)
\end{equation*}
¿Es cierto que $\langle Au,v \rangle_{L^2} = \langle u,Av \rangle_{L^2}$ para todos $u,v\in C_c^\infty(I)$? Vemos que:
\begin{align*}
    \langle Au,v \rangle  &= \int_I((pu')' + qu)v~dx \AstIg -\int_I pu'v'~dx + \int_I quv~dx \\
                          &\AstIg \int_I u(pv')'~dx + \int_I quv~dx = \langle u,Av \rangle 
\end{align*}
donde en $(\ast)$ hemos integrado por partes.\\
...
Fredhölm nos dice:
\begin{enumerate}
    \item Estudiar $Ay(x) = 0$ (así calculamos $\ker A$).
    \item Si $Ay = 0$ solo admite la solución $y(x) = 0$, luego $Ay(x) = r(x)$ tiene una única solución.
\end{enumerate}

\begin{ejemplo}
    Apliquemos la teoría a varios ejemplos:
    \begin{enumerate}
        \item Para el problema:
        \begin{equation*}
            \left\{\begin{array}{l}
                y''(x) - y(x) = f(x) \quad \text{en\ } [0,\pi] \\
                y(0) = 0 \\
                y(\pi) = 0 
            \end{array}\right.
        \end{equation*}
        Problema homogéneo asociado ($q\equiv-1, p\equiv1$):
        \begin{equation*}
            \left\{\begin{array}{l}
                    y_h'' - y_h = 0 \\
                    y_h(0) = 0 \\
            ...
            \end{array}\right.
        \end{equation*}
        La solución general es ($\lm^2-1=0$) $y_h(x) = c_1e^x + c_2 e^{-x},\quad  c_1,c_2\in \mathbb{R}$. Tenemos:
        \begin{equation*}
            \left\{\begin{array}{l}
                0 = y_h(0) = c_1 + c_2  \\
                0 = y_h(\pi) = c_1e^\pi + c_2 e^\pi
            \end{array}\right. \quad\Longleftrightarrow\quad c_1 = 0 = c_2
        \end{equation*}
        En este caso, el Teorema de Fredhölm nos dice que el problema inicial solo tiene una única solución
    \item Para:
        \begin{equation*}
            \left\{\begin{array}{l}
                y''(x) + y(x) = f(x)\qquad \text{en\ } [0,\pi] \\
                y(0) = 0 \\
                y(\pi) = 0 \\
            \end{array}\right.
        \end{equation*}
        ...
        \begin{equation*}
            ...
            \end{array}\right.
        \end{equation*}
        La solución general es $y_h(x) = c_1\sen x + c_2 \cos x, \quad c_1,c_2\in \mathbb{R}$. Para sacar las constantes:
        \begin{equation*}
            \left\{\begin{array}{l}
                0 = y_h(0) = c_2 \\
                0 = y_h(\pi) = -c_2 \\
            \end{array}\right. \quad\Longrightarrow\quad y_h(x) = c_1\sen x, \quad c_1\in \mathbb{R}
        \end{equation*}
        El problema admite soluciones no triviales, con lo que para que el problema tenga solución necesitamos que $\langle f,c_1 \sen \rangle  = c_1 \langle f,\sen \rangle = 0$, es decir, que:
        \begin{equation*}
            \int_{0}^{\pi} f(x)\sen x~dx  = 0
        \end{equation*}
