...
\begin{teo}
    ...
    \begin{proof}
        ...
        \noindent
        Suponemos por tanto que $a\neq 0$, con lo que $n$ puede ser cualquiera distinto de $\car(F)$, para que $x^n-a$ sea separable. Sea $R$ el conjunto de las raíces en $K$ de $x^n-a$, tenemos que $|R| = n$, puesto que $x^n-a$ es separable. Además, si $r,s\in R$, tenemos que $r^{-1}s\in K$ es una raíz $n-$ésima de la unidad. 

        \noindent
        Fijado $r$, entonces el conjunto $\{r^{-1}s : s\in R\}$ contiene $n$ raíces $n-$ésimas de la unidad distintas, por lo que en dicho conjunto las tenemos todas, luego ha de contener al menos una raíz $n-$ésima primitiva de la unidad, llamémosla $\zeta\in K$ .\\

        \noindent
        Para la segunda afirmación, fijado $r$ raíz de $x^n-a$, es claro que:
        \begin{equation*}
            F(\zeta, r)\leq K
        \end{equation*}
        Para la otra inclusión, si $r_1,\ldots,r_n$ son las raíces de $x^n-a$ tenemos entonces que $K = F(r_1,\ldots,r_n)$, hemos visto anteriormente que:
        \begin{equation*}
            \{r_1,\ldots,r_n\} = \{r^{-1}s : s\in R\} = \{r^{-1}\zeta^k : k\in \cc{U}(\mathbb{Z}_n)\}
        \end{equation*} % // TODO: No sé si esto está bien, respasar demostración
...
\begin{ejemplo} % // TODO: EL Teorema es el 3.9
...
\end{equation*}
Dada una permutación $\sigma\in S_n$, aplicando $n$ veces la Propiedad Universal del anillo de polinomios, obtenemos un homomorfismo de anillos $\overline{\sigma}:F[x_1,\ldots,x_n]\to F[x_1,\ldots,x_n]$ determinado por:
\begin{align*}
