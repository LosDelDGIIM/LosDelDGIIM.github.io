...
\begin{ejemplo}
    ...
    \begin{figure}[H]
        ...
        \shorthandoff{""}
        \begin{tikzcd}
            F \arrow[r, "\iota", hook] & {F[x]} \arrow[r, "p"] & {\dfrac{F[x]}{\langle p\rangle}}
        \end{tikzcd}
        \shorthandon{""}
    \end{figure}
    \noindent
...
\begin{prop}\label{prop:cuerpo_descomposicion}
    ...
    \begin{proof}
        ...
        \end{equation*}
        es decir, todo elemento de $F$ es raíz de $x^q-x$. Como su polinomio derivado es $qx^{q-1}-1 = 0$, tenemos entonces que $x^q-x$ tiene exactamente $q$ raíces distintas, que deben ser todos aquellos elementos de $F$. Como además $\mathbb{Z}_p\leq F$ es el subcuerpo primo, tenemos que $F$ es cuerpo de descomposición de $f\in \bb{Z}_p[x]$.
    \end{proof}
\end{prop}
...
\begin{ejercicio}
    ...
    \end{equation*}
    \begin{enumerate}[label=\alph*)]
        \item Para $E = \mathbb{Q}\left(\sqrt{6},i\right)$. % // TODO: 
