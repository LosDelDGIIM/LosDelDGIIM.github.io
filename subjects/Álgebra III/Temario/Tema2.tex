...
\begin{definicion}[Polinomio separable]
    Sea $f\in F[x]$ con $degf \geq 1$, se dice que $f$ es separable si todas sus raíces (en un cuerpo de descomposición) son simples.
\end{definicion}
...
% Hay muchos separables si car(F) = 0, hay separables si car(F) = p y no hay separables:
\begin{ejemplo}
    Si $car(F) = 0$ y $f$ es irreducible, entonces $f$ es separable.\\

    \noindent
    Como $car(F) =0$ y $degf\geq 1$, tenemos entonces que $f'\neq 0$, y como $f$ es irreducible, tenemos que $\mcd(f,f') = 1$, de donde $f$ es separable.
\end{ejemplo}

\begin{ejemplo}
    $f=x^q-x\in \bb{F}_p[x]$ es separable para $q=p^n$, puesto que $f' = -1\neq 0$.
\end{ejemplo}

\begin{ejemplo}
    Sea $\bb{F}_p(t)$ el cuerpo de fracciones del anillo de polinomios $\bb{F}_p[t]$, tomamos el polinomio:
    \begin{equation*}
        f = x^p - t \in \bb{F}_p(t)[x]
    \end{equation*}
    que es irreducible (por Eisenstein para $t$) y $f' = 0$, con lo que $\mcd(f,f')=f\neq 1$, por lo que $f$ no es separable.
\end{ejemplo}
...
\begin{teo}
    ...
    \begin{proof}
        ...
        \begin{description}
            \item [$i)\Longrightarrow  ii)$] Sabemos que el cuerpo de descomposición de un polinomio es finitamente generado, y como es algebraica, entonces es finita. 

                Sea $F' = K^{Aut_F(K)}$, es claro que $F\leq F'$, por el Teorema anterior tenemos que tomando $G = Aut_F(K)$, tenemos entonces que:
                \begin{equation*}
                    Aut_F(K) = Aut_{F'}(K)
                \end{equation*}
                $K$ es cuerpo de descomposición de $f\in F[x]$ y tenemos también que $K$ es cuerpo de descomposición de $f\in F'[x]$. Como $f$ es separable, tenemos que:
                \begin{equation*}
            ...
            \item [$iii)=>iv)$] La Proposición de Artin nos dice que $F\leq K$ es finita. 
                ...
                \end{equation*}
                y vemos que $g = \prod\limits_{i=1}^{t}(x-\sigma(\alpha_i))$, puesto que al aplicar $\sigma$ sobre los elementos de la órbita los permuta, con lo que de la igualdad de la derecha deducimos que $\sigma(a_j) = a_j$, para todo $j\in \{1,\ldots,t\}$, con lo que $a_j\in F[x]$ para todo $j \in \{1,\ldots,t\}$, con lo que $g\in F[x]$.

                Por una parte $g(\alpha) = 0$, puesto que $\alpha\in Orb(\alpha)$. Como $h=Irr(\alpha,F)$, tenemos que $h$ divide a $g$.

                Por otra parte, cada $\alpha_i$ es raíz de $h$, ya que % // TODO: TERMINAR
        \end{description}
    \end{proof}
\end{teo}
