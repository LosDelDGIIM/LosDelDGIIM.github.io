...
\begin{itemize}
    ...
    \item La aplicación es un homeomorfismo, pues si $\sigma,\tau\in \Aut_F(K)$ tenemos entonces que:
        ...
        \end{equation*}
\end{itemize}
Además dicho homomorfismo de grupos es siempre inyectivo, 

% Razonamiento del profe
Si $F(\alpha)\stackrel{\sigma}{\to}F(\alpha)$ con $\sigma\big|_F = id_F$ con $\sigma(\alpha) = \alpha$ entonces $\sigma= id_{F(\alpha)}$.

\noindent
Sabemos que $\{1,\alpha,\ldots,\alpha^{n-1}\}$ es una $F-$base de $F(\alpha)$.

\noindent
Si ahora metemos dos, si $F(\alpha,\beta)\stackrel{\sigma}{\to}F(\alpha,\beta)$ con $\sigma\big|_{F} = id_F$ y $\sigma(\alpha)=\alpha,\sigma(\beta)=\beta$. Viendo $F(\alpha,\beta) = F(\alpha)(\beta)$ estamos en el caso anterior.


% Algo que razoné yo
pues la Proposición de Extensión nos dice que cada automorfismo del grupo de Galois queda unívocamente determinado por la imagen de cada raíz de $f$, puesto que sabemos que el grupo de Galois de $f$ coincide con las extensiones de la inclusión:
\begin{equation*}
...
\begin{ejemplo}
    ...
    \end{equation*}
    Y sabemos que son todas porque como el grupo de la derecha tiene $6$ elementos y el de la derecha 24; con lo que $\ker S = V$ ($r$ es la restricción, viene del teorema que habla sobre subgrupos normales). Donde $N$ es el núcleo, ya que $N$ es $\Aut_E(K)$.\\
    ...
    \begin{figure}[H]
        ...
        1 \arrow[r] & N \arrow[r, hook] & G \arrow[u] \arrow[rr, "r"'] \arrow[rru, bend left] \arrow[rru, bend right] &  & Aut_F(E) \arrow[r] \arrow[u]             & 1
        \end{tikzcd}
        \shorthandon{""}
    \end{figure}
    \noindent
    Donde los homomorfismos que suben son los k ven el dominio como cuerpo de descomposición y el otro contiene las raíces.\\
...
\begin{prop}
    ...
    \begin{proof}
        ...
        \end{equation*}
        Y si tomamos $\overline{\sigma}\in G$ es claro que $f^{\overline{\sigma}} = f$, tenemos:
        \begin{equation*}
            f = x^n-s_1x^{n-1} + \ldots + {(-1)}^{n}s_n 
        \end{equation*}
        Y como cada $s_k\in E^G$ tenemos que $F(s_1, \ldots, s_n)\leq E^G$. Tenemos que $E$ es cuerpo de descomposición de $f$ sobre $F(s_1, \ldots, s_n)$. Es claro que $f$ es separable por su propia definición, con lo que la extensión $F(s_1, \ldots, s_n) \leq E$ es de Galois.\\
...
\begin{prop}
    ...
    \begin{proof}
        ...
        \end{equation*}
        que extiende a $\varepsilon$. Tenemos que $\overline{\varepsilon}$ es $F-$lineal y es un isomorfismo de cuerpos. Cardano-Vietta nos dice que $g^{\overline{\varepsilon}} = f$. Teemos que $E = F(x_1,\ldots,x_n)$ es cuerpo de descomposición de $f$, por lo que al aplicar el isomorfismo $\overline{\varepsilon}$ tenemos que  la correstricción:
        \begin{equation*}
...
\begin{teo}[de Abel-Ruffini]
    Si $\car(F) = 0$ y $n\geq 5$, entonces $g$ no es resoluble por radicales.
    \begin{proof}
...
\section{Resolución de las ecuaciones de grado hasta 4}
\noindent
...
\end{equation*}
Por lo que la ecueación cuadrática es resoluble por radicales para cualquier cuerpo $F$ con $\car(F)\neq 2$.
...
\end{equation*}
para ciertos $p,q\in F$. El polinomio $g$ recibe el nombre \underline{cúbica reducida de $f$}. Sea $K$ una extensión de $F$ donde están las raíces de $f$ y una raíz cúbica primitiva de la unidad\footnote{Aquí también necesitamos que $\car(F)\neq 3$, para que $x^3-1$ sea separable.} $\omega$. Sean $\alpha_1,\alpha_2,\alpha_3\in K$ las raíces de $g$. Tenemos por las ecuaciones de Cardano-Vietta:
\begin{equation*}
...
\end{align*}
Sumando vemos que ($\omega$ es raíz de $x^2+x+1$):
\begin{equation*}
...
% // TODO: El siguiente teorema generaliza un teorema que vimos anteriormente para F_p <= F_p^n con p primo, pero no hace falta que p sea primo
\begin{teo}
    Tomamos (para $q=p^k$) $\bb{F}_q\leq \bb{F}_{q^n}$ una extensión de cuerpos finitos. Entonces $\Aut_{\bb{F}_q}(\bb{F}_{q^n})$ es cíclico generado por $\phi$, donde:
    \begin{equation*}
        \phi(\alpha) = \alpha^q \qquad \forall \alpha \in \bb{F}_{q^n}
    \end{equation*}
    Además, $\bb{F}_{q^n}$ es cuerpo de descomposición de $x^{qn}-x\in \bb{F}_q[x]$.
    \begin{proof}
        Con $q=p^k$ con $p$ primo y $k\geq 1$, tenemos la extensión:
        \begin{equation*}
            \bb{F}_p \leq \bb{F}_{p^k} \leq \bb{F}_{p^{kn}}
        \end{equation*}
        y son las tres extensiones de Galois. El automorfismo de Frobenius $\tau$ nos permite describir:
        \begin{equation*}
            \Aut_{\bb{F}_p}(\bb{F}_{p^{kn}}) = \langle \tau \rangle 
        \end{equation*}
        donde $\tau:\bb{F}_{p^{kn}}\to \bb{F}_{p^{kn}}$, donde:
        \begin{equation*}
            \tau(\alpha) = \alpha^p \qquad \forall \alpha\in \bb{F}_{p^{kn}}
        \end{equation*}
        Como $\Aut_{\bb{F}_{p^k}}(\bb{F}_{p^{kn}})$ es un subgrpo de $G = \Aut_{\bb{F}_p}(\bb{F}_{p^{kn}})$ este último cíclico tenemos que el primero es cíclico.\\

        \noindent
        Si buscamos ahora su generador, como todas las extensiones son de Galois calcular el orden del grpuo equivale a calcular el grado de la extensión. Si observamos de esta forma que:
        \begin{equation*}
            [\bb{F}_{p^{kn}} : \bb{F}_{p^k}] = n \quad\Longrightarrow\quad |\Aut_{\bb{F}_{p^k}}(\bb{F}_{p^{kn}})| = n
        \end{equation*}
        tenemos entonces en vista de $\tau$ y del orden del grupo que un generador suyo es $\tau^k$. Llamamos $\phi = \tau^k$, y calculamos:
        \begin{equation*}
            \phi(\alpha) = (\tau^k)(\alpha) = \alpha^{p^k} = \alpha^q
        \end{equation*}
        Finalmente, como $\bb{F}_p^{kn}$ es cuerpo de descomposición de $x^{p^{kn}}-x=x^{q^n}-x\in \bb{F}_p[x]$ tenemos entonces que $\bb{F}_{q^n}$ es cuerpo de descomposición de $x^{q^n}-x\in \bb{F}_q[x]$.
    \end{proof}
\end{teo}

\begin{notacion}
    En vistas el Teorema anterior, bajo sus mismas hipótesis, llamaremos a $\phi$ automorfismo de Frobenius de la extensión $\bb{F}_q\leq \bb{F}_{q^n}$.
\end{notacion}

\begin{teo}
    Si $f\in \bb{F}_q[x]$ es un polinomio irreducible de grado $n$, entones su cuerpo de descomposición  es $\bb{F}_{q^n}$. Además, si $\alpha\in \bb{F}_{q^n}$ es una raíz de $f$, entonces el resto de sus raíces son
    \begin{equation*}
        \alpha^q, \ldots, \alpha^{q^{n-1}}
    \end{equation*}
    \begin{proof}
        $f$ tiene una raíz en alguna extensión de grado $n$ de $\bb{F}_q$, por ejemplo $\frac{\bb{F}q[x]}{\langle f \rangle }$. Así, tomamos:
        \begin{equation*}
            \bb{F}_{q^n} = \frac{\bb{F}_q[x]}{\langle f \rangle }
        \end{equation*}
        Y como la extensión $\bb{F}_q\leq \bb{F}_{q^n}$ es de Galois, tenemos que $f$ es separable y que todas sus raíces están en $\bb{F}_{q^n}$. Si $\alpha$ es una raíz de $f$ en $\bb{F}_{q^n}$, entonces:
        \begin{equation*}
            \alpha^{q^k} = \phi(\alpha)^k  \qquad \forall k \in \{1,\ldots n\}
        \end{equation*}
        son raíces de $f$, gracias al grupo de Galois. Además, como tenemos $n$, son todas sus raíces.
    \end{proof}
\end{teo}

\noindent
Si observamos ahora que cada uno de estos $f$ que montamos es un factor irreducible del polinomio $x^{q^n}-x\in \bb{F}_{q}[x]$.

\begin{teo}
    Un polinomio irreducible $f\in \bb{F}_q[x]$ de grado $n$ divide a $x^{q^m}-x\in \bb{F}_q[x]$ si, y solo si, $n$ divide a $m$.\newline
    Como consecuencia, $x^{q^m}-x\in \bb{F}_q[x]$ es producto de todos los polinomios irreducibles en $\bb{F}_q[x]$ cuyo grado divide a $m$.
    \begin{proof}
        Por doble implicación:
        \begin{description}
            \item [$\Longrightarrow )$] Supongamos que $f$ es irreducible y que divide a $x^{q^m}-x\in \bb{F}_q[x]$. Tomamos sendos cuerpos de descomposición sobre $\bb{F}_q$: $\bb{F}_{q^n}\leq \bb{F}_{q^m}$, de donde $n\mid m$ por el Lema de la Torre.
            \item [$\Longleftarrow )$] Si $f$ es irreducible y $n\mid m$ tenemos entonces que $\bb{F}_{q^n}\leq \bb{F}_{q^m}$ (tomamos $\bb{F}_{q^m}$ y por la conexión de Galois encontramos $\bb{F}_{p^n}$, ya que el grupo de Galois es cíclico). Ahora, los elementos de $\bb{F}_{q^m}$ son las raíces de $x^{q^m}-x\in \bb{F}_q[x]$. $n$ de ellas serán las raíces de $f$, y como todas las raíces son de $x^{q^m}-x$ tenemos entonces que $f$ divide a este. 
        \end{description}
        La consecuencia viene del Teorema de factorización y por el si y solo si.
    \end{proof}
\end{teo}
...
\begin{ejemplo}
    ...
    \begin{itemize}
        \item De grado 1: $x,x-1$
        \item De grado 2: $x^2+x+1$.
        \item De grado 4 sabemos que quedan 3 posibles, que tenemos que buscar sin raíces y distintos de ${(x^2+x+1)}^{2} = x^4+x^2 + 1$.
            \begin{equation*}
    ...
    \end{equation*}
    En esta última factorización vemos que $\bb{F}_{16}$ puede presentarse de 3 formas distintas sobre $\bb{F}_2$, tomando $\bb{F}_{16} = \bb{F}_2(a)$ con $a$ raíz de cualquiera de los 3 polinomios de grado 3. Para cada polinomio tendremos una presentación posible. Es decir:
    ...
    \begin{itemize}
        ...
        \item Si tomamos $g=x^4+x^3+1\in \bb{F}_2[x]$, queremos calcular ahora las raíces de $g$ en $\bb{F}_{16}$ en función de $\alpha$.
            ...
            Sabemos que $\bb{F}_{16}$ tiene que tener una raíz de $g$ porque $\bb{F}_{16}$ consiste en exclusivamente las raíces de $x^{16}+x\in \bb{F}_2[x]$, que según la descomposición en factores irreducibles nos dice que todas las raíces de $x^4+x^3+1$ están en $x^{16}+x$.
        ...
        \item Nos falta clasificar 6 raíces, 2 de $k=x^2+x+1$ y 4 de $h=x^4+x^3+x^2+x+1$.
            ...
            Podríamos haberlo hecho también pensando también en que $\alpha^5$ tiene orden 3 y $\alpha^3$ tiene orden 5 sobre $\bb{F}_{16}^{\times}$. Como $x^2+x+1$ tiene cuerpo de descomposición $\bb{F}_4$, tendremos un elemento $\alpha^2 +\alpha+1=0$  con orden 3 en $\bb{F}_3^\times$, por lo que viendo la extensión $\bb{F}_4\leq \bb{F}_{16}$ obtendremos finalmente que las raíces de $k$ son las de orden 3.
    \end{itemize}
\end{ejemplo}
