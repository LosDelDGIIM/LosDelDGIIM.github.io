...
\begin{document}
    ...
    \portadaExamen{ffccA4.jpg}{Variable Compleja I\\Examen XVIII}{Variable Compleja I. Examen XVIII}{MidnightBlue}{-9.5}{28}{2024-2025}{José Juan Urrutia Milán\\Adolfo Martínez Olmedo}
    ...
    \begin{ejercicio}[2.5 puntos]
        Sea $S\subset \mathbb{C}$ un conjunto sin puntos de acumulación y $f\in \cc{H}(\mathbb{C}\setminus S)$ verificando que tiene un polo en cada punto de $S$ (en tal caso se dice que $f$ es una función meromorfa). Supongamos además que $f$ diverge en infinito. Se pide:
        \begin{enumerate}[label=\alph*)]
    ...
    \begin{ejercicio}[2.5 puntos]
        ...
        \begin{enumerate}[label=\alph*)]
            ...
            Como \(f \in \mathcal{H}(D(0,1))\), existe una sucesión \(\{\alpha_n\}\) tal que:
            \[
                f(z) = \sum_{n=0}^\infty\alpha_n z^n \qquad \forall z \in D(0,1)
            \]
            Podemos entonces considerar la sucesión asociada a \(f^\ast(z)\) de la siguiente manera:
            \[
                f^\ast(x) = \overline{f(\overline{z})} = \overline{\sum_{n=0}^\infty\alpha_n (\overline{z})^n} \qquad \forall z \in D(0,1)
            \]
            Y, como el conjugado se distribuye a través de la suma de los productos, obtenemos:
            \[
                f^\ast(x) = \sum_{n=0}^\infty\overline{\alpha_n}z^n \qquad \forall z \in D(0,1)
            \]
            Nos preguntamos si esta serie es convergente en \(D(0,1)\) ya que, de serlo, tendremos garantizada la holomorfía de \(f^\ast\). En efecto, como conjugar un número complejo lo lleva a su simétrico respecto al eje real, no cambia su magnitud, y, en definitiva \(|z| = |\overline{z}|\).
            Luego:
            \[
                f \in \mathcal{H}(D(0,1) \Longrightarrow\left\{ \sqrt[n]{|\alpha_n|}\right\} \xrightarrow{} 1
                \iff \left\{ \sqrt[n]{\left|\overline{\alpha_n}\right|}\right\} \xrightarrow{} 1. 
            \]
            Por tanto, \(f^\ast \in \mathcal{H}(D(0,1)\), como queríamos demostrar.
            \item Supongamos que se cumple $|f(z)| \leq |f(\ol{z})|$ para cada $z\in D(0,1)$. Probar que existe $\lm\in \bb{T}$ tal que $f(z)=\lm f^\ast(z)$ para cada $z\in D(0,1)$.
            
            Si \(f \equiv 0\), entonces \(f^\ast \equiv 0\) y lo pedido se tiene trivialmente para cualquier \(\lambda\). Supongamos entonces que \(f \not \equiv 0\), entonces \(f^\ast \not \equiv 0\), y por ser holomorfa sus ceros son aisaldos.

            Definimos por tanto la función:
            \Func{\tilde{g}}{D(0,1) \setminus Z(f^\ast)}{\mathbb{C}}{z}{\dfrac{f(z)}{f^\ast(z)}}

            Necesitamos que \(\tilde{g}\) sea holomorfa en todo el disco \(D(0,1)\) para poder aplicar el principio del módulo máximo. Por hipótesis, \(|f(z)| \leq |f(\overline{z})|\) para cada \(z \in~D(0,1)\), luego

            \[
                \left|\tilde{g}(z)\right| = \left|\frac{f(z)}{f^\ast(z)}\right| = \frac{\left|f(z)\right|}{\left|f^\ast(z)\right|} = \frac{\left|f(z)\right|}{\left|\overline{f(\overline{z})}\right|} = \frac{\left|f(z)\right|}{\left|f(\overline{z})\right|} \leq 1 \qquad \forall z \in D(0,1) \setminus Z(f^\ast)
            \]
            Sea \(z_0 \in Z(f^\ast)\), como los ceros de \(f^\ast\) son aislados, \(\exists \delta >0\) tal que \(f^\ast(z) \neq 0\) para todo \(z \in D(z_0,\delta) \setminus \left\{z_0\right\}\). En ese disco \(\tilde{g}\) es holomorfa y, como acabamos de ver, está acotada por \(1\). Por tanto, por el teorema de extensión de Riemann, la singularidad es evitable y \(\tilde{g}\) se puede extender de forma holomorfa. Haciendo esto para todo cero de \(f^\ast\) obtenemos:
            \Func{g}{D(0,1)}{\mathbb{C}}{z}{\dfrac{f(z)}{f^\ast(z)}}
            con \(g \in \mathcal{H}(D(0,1))\).
            Como la parte imaginaria de un número real es \(0\), tenemos que es igual a su conjugado. Por tanto, sea 
            \(x \in ]0,1[\) y vemos que:
            \[
                \left|g(x)\right| = \frac{\left|f(x)\right|}{\left|f(\overline{x})\right|} = \frac{\left|f(x)\right|}{\left|f(x)\right|} = 1
            \]
            Además, sabemos que \(|g(z)| \leq 1 \enspace \forall z \in D(0,1) \).


            Por tanto, tenemos que \(|g|\) alcanza un máximo relativo en un punto interior \(x\) del segmento de recta real contenido en \(D(0,1)\). Luego por el principio del módulo máximo, \(g\) es constante. Es decir, \(g(z) = \lambda\) para todo \(z \in D(0,1)\), y sabemos que \(|\lambda| = 1\), luego \(\lambda \in \mathbb{T}\). Así, se cumple lo pedido:
            \[
                f(z) = \lambda f^\ast(z) \qquad \forall z \in D(0,1), \enspace \lambda \in \mathbb{T}
            \]
        \end{enumerate}
    \end{ejercicio}
