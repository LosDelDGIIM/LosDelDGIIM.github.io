...
\begin{ejercicio}\label{ej:rel3_3}
    ...
    \end{multicols}
    \end{figure}
    Ahora, desarrollamos el código del proceso Buffer, donde \verb|out[i]| indica la siguiente posición a leer del consumidor \verb|i|-ésimo. Por tanto, si tras consumir un valor del buffer vemos que 
    \begin{minted}{pascal}
        process Buffer;
...
\begin{ejercicio}\label{ej:rel3_5}
    Considerar un conjunto de \verb|N| procesos, \verb|P[i]|, ($i = 0, \ldots, N-1$) que se pasan mensajes cada uno al siguiente (y el primero al último), en forma de anillo. Cada proceso tiene un valor local almacenado en su variable local \verb|mi_valor|. Deseamos calcular la suma de los valores locales almacenados por los procesos de acuerdo aon el algoritmo que se expone a continuación.
    \begin{figure}[H]
        \centering
        ...
        \begin{tikzpicture}[align=center]
            % Paso 1
            \node[draw, rectangle] (a1) at (0, 0) {mi\_valor = 1\\ suma = 1};
            \node[draw, rectangle, right=of a1] (b1) {mi\_valor = 2\\ suma = 2};
            \node[draw, rectangle, below=of b1] (c1) {mi\_valor = 3\\ suma = 3};
            \node[draw, rectangle, left=of c1] (d1) {mi\_valor = 4\\ suma = 4};
