...
\subsection{Ejercicios adicionales (fuera de la relación)}
\begin{ejercicio}
...
% \begin{ejercicio}
%     Se tienen $N$ procesos cliente que interactúan con el proceso servidor que simula la actuación de 1 cajero automático. Los procesos cliente obtienen dinero del cajero realizando la operación retirar (num_cliente, cantidad) que obliga a la identificación para poder soli­citar una cantidad de dinero al cajero, quien responde entregando la cantidad solicitada si el cliente tiene suficiente saldo en su cuenta y aún queda efectivo en el cajero. En caso contrario, el cajero responde denegando la petición al cliente. Para ingresar dinero en el cajero, se implementará la aceptación de la operación ingresar(num_cliente, cantidad), con la que los procesos solo tienen que identificarse e indicar la cantidad que van a ingresar. Suponemos que cada número de cliente corresponderá con el denominado código cuenta­ cliente, es decir, este número tiene asignada 1 sola cuenta bancaria.
% \end{ejercicio}
