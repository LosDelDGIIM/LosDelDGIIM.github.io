...
\begin{ejemplo}
    ...
    \begin{itemize}
        ...
        \item Si el buffer está vacío y el consumidor se dispone a leer un dato, quedará bloqueado el proceso que ejecute el procedimiento del monitor. Cuando el productor escriba un dato, enviará una señal al consumidor, desbloqueándolo.
    \end{itemize}
...
\subsubsection{Cuidado con las interferencias}
...
\begin{ejemplo}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \item Finalmente, hemos de probar el triple $\{IM\}\ V\ \{IM\}$ para garantizar al fin que $IM$ es un IM\@. Para ello, hemos de probar el triple
                \begin{equation*}
...
\begin{ejercicio*}
    ...
    \begin{enumerate}
        ...
        \item Para probar ahora el triple del procedimiento \verb|V|:
            ...
            \end{gather*}
    \end{enumerate}
    Con lo que tenemos probado que $IM$ es un IM\@.
\end{ejercicio*}
...
El monitor que usaremos tendrá dos procedimientos exportables, uno llamado \verb|esperar| que será el que use el proceso consumidor, y otro llamado \verb|notificar|, que será el que use el proceso productor para avisar al consumidor de que ya ha realizado la acción.
...
\begin{proof}
    ...
    \begin{gather*}
        ...
        \{IM\}\ \texttt{salir}\ \{IM\}
    \end{gather*}
    Comenzamos por el código de inicialización:
    \begin{equation*}
        \{V\}\ \texttt{ocupada = false;}\ \{ocupada = false\} \rightarrow \{IM\}
    \end{equation*}
    Ahora, comprobamos el triple del procedimiento \verb|salir|:
    \begin{gather*}
        \texttt{salir(); begin} \\
        \{IM\} \\
        \texttt{ocupada = false;} \\
        \{IM \land ocupada = false\} \\
        \texttt{cola.signal();} \\
        \{IM\} \\
        \texttt{end}
    \end{gather*}
    Con lo que ya sabemos que la condición de sincronización de la variable condición $c$ es $\{IM \land ocupada = false\}$. Finalmente, el triple del procedimiento \verb|entrar|:
    \begin{gather*}
        \texttt{entrar(); begin} \\
        \{IM\} \\
        \texttt{if ocupada then} \\
        \{IM \land ocupada = true\} \\
        \texttt{cola.wait();} \\
        \{IM \land ocupada = false\} \\
        \texttt{else} \\
        \{IM \land ocupada = false\} \\
        ...
        \{IM \land ocupada = false\} \\
        \texttt{ocupada = true;} \\
        \{IM \land ocupada = true\} \rightarrow \{IM\} \\
        \texttt{end}
    \end{gather*}
\end{proof}
