...
\subsubsection{Instanciación de monitores} % // TODO: Si un monitor no se instancia, por qué este título?
...
\begin{ejemplo}
    ...
    % // TODO: Si un monitor no es una clase, por qué esta palabra reservada?
...
% // TODO: Cuando un proceso se desbloquea, a qué cola va? Por qué tiene preferencia la cola de la variable cond y no la cola del monitor?
...
Consideramos un programa concurrente en el que tenemos $n$ procesos ejecutándose que podemos representar como triples de Hoare ciertos $\{P_i\}\ S_i\ \{Q_i\}$ con $i \in \{1, \ldots, n\}$ de forma que ninguna variable en $P_i$ o en $Q_i$ es modificada por ningún $S_j$ con $i\neq j$. Si en dicho código tenemos $m$ monitores de forma que para cada uno hemos conseguido probar un IM, $IM_k$ con $1 \leq k \leq m$, entonces podemos aplicar la \textbf{regla de concurrencia para programas con monitores}:
\begin{equation*}
