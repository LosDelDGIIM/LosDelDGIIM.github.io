...
\begin{ejercicio}
    ...
    \begin{minted}{pascal}
        Monitor Recurso;
           ...
           begin
              ...
              if peticion[j] then {j es el de mayor prioridad que pide el recurso}
              begin
                 peticion[j] := false; {Le cedemos el recurso directamente}
                 cola.signal();
              else
                 libre := true;
              end
    ...
    \begin{itemize}
        \item \verb|libre|, de forma que si \verb|libre[i] = true|, entonces el recurso $i$-ésimo estará libre.
        \item \verb|peticion|, con el mismo comportamiento que antes.
    \end{itemize}
...
\begin{ejercicio}
    ...
    \begin{enumerate}
        \item En primer lugar, vemos que una de las variables permanentes del monitor ha de ser el array \verb|tenedor_ocupado|, donde \verb|tenedor_ocupado[i]=true| indica que el tenedor $i$-ésimo ha sido cogido.
        Por otro lado, necesitamos un array de variables condición, \verb|cola_tenedor|, en cuya cola esperen los procesos filósofos que necesitan del tenedor $i-$ésimo.
         \begin{minted}{pascal}
         monitor MonFilo;
            var   tenedor_ocupado : array[0..4] of boolean;
                  cola_tenedor : array[0..4] of condition;

            procedure coge_tenedor(num_tenedor, num_proceso : integer);
            begin
               { Si ya han cogido el tenedor }
               if tenedor_ocupado[num_tenedor] then
                  cola_tenedor[num_tenedor].wait();

               tenedor_ocupado[num_tenedor] := true;
            end

            procedure libera_tenedor(num_tenedor : integer);
            begin
               tenedor_ocupado[num_tenedor] := false;
               cola_tenedor[num_tenedor].signal();
            end
        ...
        \begin{comment}       
        Hemos diseñado el siguiente monitor, donde \verb|tenedores[i]| indica si el tenedor $i$-ésimo ha sido (\verb|true|) cogido o si no (\verb|false|).
            \begin{minted}{pascal}
                monitor MonFilo;
        ...
        \item Para que el monitor no de lugar a un interbloqueo de los procesos, es necesario introducir un nuevo array, que controle qué filósofos disponen de un único tenedor, \verb|un_tenedor|; así como un entero que controle el número de filósofos que tienen un único tenedor, \verb|fil_1t|; y una variable condición \verb|cola|, que nos permita bloquear a los filósofos que traten de coger el 5º tenedor y bloquear al resto:
            ...
            monitor MonFilo;
               ...
               begin
                  ...
                  {En este punto, no hay riesgo de interbloqueo. O bien porque no hay 4 filósofos con un solo tenedor, o bien porque el proceso actual ya tiene un tenedor}
                  if tenedor_ocupado[num_tenedor] then
                     cola_tenedor[num_tenedor].wait();

                  tenedores[num_tenedor] := true;

                  {Hay que actualizar las nuevas variables}
                  if not un_tenedor[num_proceso] then {Si antes no tenía tenedor}
                     ...
                     { Ya no hay riesgo de interbloqueo, porque este filósofo liberará ambos tenedores }
                     if fil_1t < 4 then
                        cola.signal();
                  end
               end
...
\begin{ejercicio}
En un pueblo existe una pequeña barbería con una puerta de entrada y otra de salida, dichas puertas son tan estrechas que solo permiten el paso de una persona cada vez que son utilizadas. En la barbería hay un número indeterminado de sillas (suponemos que siempre hay sillas libres porque los barberos no hacen esperar demasiado a los clientes) de 2 tipos:
\begin{enumerate}[label=(\alph*)]
    \item Sillas donde los clientes esperan a los barberos (sillas de espera).
    \item Sillas de barbero donde se realiza el corte de pelo a los clientes por parte de los barberos.
\end{enumerate}
No hay espacio suficiente para que más de una persona (barbero o cliente) pueda moverse dentro de la barbería en cada momento, por ejemplo, si los clientes se dan cuenta de que ha entrado un barbero, entonces solo un cliente puede levantarse.y dirigirse a una silla de tipo (b); un barbero, por su parte, no podría moverse para ir a cortarle el pelo hasta que el cliente se hubiera sentado.\\
    
Cuando entra un cliente en la barbería, puede hacer una de estas 2 cosas:
\begin{itemize}
    \item Aguarda en una silla de tipo (a) y espera a que haya barberos disponibles para poder ser atendido, cuando sucede esto último, el cliente se levanta y vuelve a sentarse, esta vez en una silla de tipo (b), para esperar hasta que termine su corte de pelo.
    \item Se sienta directamente en una silla de tipo (b), si hay barberos disponibles. Un cliente no se levanta de la silla del barbero hasta que este le avisa abriéndole la puerta de salida.
\end{itemize}
Cuando entra en la barbería, un barbero aguarda a que haya clientes sentados en una silla de tipo (b) esperando su corte de pelo. Después, revisa el estado de los clientes siguiendo un orden numérico establecido, hasta que encuentra un cliente que espera ser atendido; cuando lo encuentra comienza a cortarle el pelo y él mismo pasa a estar ocupado. Cuando termina con un cliente, le abre la puerta de salida y espera a que el cliente le pague, después sale a la calle para refrescarse. El barbero no podrá entrar de nuevo en la barbería hasta que haya cobrado al cliente que justamente acaba de atender. No se admite que un cliente pague a un barbero distinto del que cortó el pelo.

\begin{minted}{pascal}
    monitor Peluqueria;
       var pelando : array[0..+INF] of (silla_vacia, silla_pelando, silla_esperando);
           pagado : array[0..+INF] of boolean;
           clientes_esperando, clientes_pelando, barberos_ociosos, puerta_salida : condition;

       begin
          pelando := (silla_vacia, silla_vacia, ...);
          pagado := (false, false, ...);
       end

       {Procedimiento para pelar a los clientes}
       procedure pelar(i : integer);
       begin
          {Esperar hasta que haya un barbero disponible}
          if not barberos_ociosos.queue() then
             clientes_esperando.wait();
          
          {Hay un barbero disponible}
          pelando[i] := silla_esperando; {Se sienta en una silla}
          barberos_ociosos.signal();

          {Espera su corte de pelo (hasta que el barbero le levante)}
          while pelando[i] <> silla_vacia do begin
             clientes_pelando.signal();
             clientes_pelando.wait();
          end

          {Espera a ser cobrado}
          pagado[i] := false;
          while not pagado[i] do begin
             puerta_salida.signal();
             puerta_salida.wait();
          end
       end

       {Procedimiento para barbero para pasar al siguiente cliente}
       procedure siguiente_cliente() : integer;
       var k := 0;
       begin
          {Si hay clientes esperando, los despertamos}
          if clientes_esperando.queue() then
             clientes_esperando.signal();
          else {Si no hay clientes esperando, esperamos clientes}
             barberos_ociosos.wait();
          end

          {Hay un cliente esperando a ser pelado en la silla, lo buscamos}
          while pelando[k] <> silla_esperando do begin
             k := k + 1;
          end
          pelando[k] := silla_pelando;
          return k;
       end

       {Procedimiento para terminar de pelar al cliente}
       procedure terminar(i : integer);
       begin
          {Termina de pelar al cliente}
          pelando[i] := silla_vacia;
          clientes_pelando.signal();
