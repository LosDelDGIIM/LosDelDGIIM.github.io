/**
 * @file filosofosSinInterbloqueo.cpp
 * @brief Problema de los filósofos, básico, sin interbloqueo.
 * 
 * No se da interbloqueo. La situacion anterior se soluciona como sigue:
 * 	- El filosofo 1 coge primero el de la derecha y luego el de la izquierda.
 * 	- El resto de filosofos cogen primero el de la izquierda y luego el de la derecha.
 * 
...
void funcion_filosofos( int id_filosofo ){
	...
	while ( true ) {
		...
		// Esta distincion evita el interbloqueo
		if (id_filosofo == 0){
			cout <<"Filosofo " <<id_filosofo <<" solicita primero el tenedor derecho: " <<id_ten_der <<endl;
			MPI_Ssend( &peticion, 1, MPI_INT, obtener_id_global_tenedor(id_ten_der), 0, MPI_COMM_WORLD );

			cout <<"Filosofo " <<id_filosofo << " solicita despues el tenedor izquierdo: " <<id_ten_izq <<endl;
