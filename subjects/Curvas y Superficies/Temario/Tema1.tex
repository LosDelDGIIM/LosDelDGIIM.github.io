...
\end{equation*}
Supongamos ahora que $\alpha$ es regular y queremos que $\beta$ también lo sea. Para ello, en vista de la expresión superior, tendremos que suponer que $h'(t)\neq 0 \quad \forall t\in J$, es decir, que bien $h'(t)>0 \quad \forall t\in J$ o bien que $h'(t)<0\quad \forall t\in J$. En vista de estas condiciones, tendremos que $h:J\to I$ es un \textbf{difeomorfismo\footnote{Aplicación inyectiva diferenciable con inversa diferenciable.}} (bien creciente o decreciente).
...
\end{equation*}
Hemos llegado a la fórmula del Teorema de cambio de variable, $u = h(t)$, por lo que:
\begin{equation*}
    \boxed{
...
\begin{equation*}
    Mx = Ax + b 
\end{equation*}
Con\footnote{Una matriz es $O(3)$ si su inversa coincide con su traspuesta.} $A\in O(3)$ y $b\in \mathbb{R}^3$. En este caso las trazas son geométricamente equivalentes:
\begin{equation*}
...
\begin{observacion}
    ...
    \noindent
    Este es otro motivo por el que preferir las curvas regulares.
\end{observacion}
...
\end{equation*}
Este tipo de curvas reciben el nombre de ``\underline{curvas parametrizadas por el arco}'', y será habitual abreviarlo como p.p.a. Acabamos de ver que toda curva regular admite una parametrización por el arco.\\

\noindent
A continuación consideraremos que todas las curvas que aparecen son regulares, salvo que se diga lo contrario.
...
\begin{ejercicio} % // TODO: HACER
    ...
    \noindent
    Cualquier reparametrización $h:J\to I$ dada por $h(t) = t+b$ es una reparametrización por el arco, ¿son todas?
\end{ejercicio}
...
\begin{ejercicio}
    ...
    \noindent
    Se pide ver cómo son las rectas normales de una recta y de una circunferencia. ¿Hay otras curvas que satisfagan algunas de esas dos cosas?\\

    \noindent
    Sucede que las normales a una recta son paralelas y las normales a una circunferencia se cortan todas ellas en el centro.
\end{ejercicio}

\section{Teoría local de curvas planas. Diedro de Frenet. Curvatura}
\noindent
Sea $\alpha:I\to \mathbb{R}^2$ una curva plana regular. Entonces podemos considerarla parametrizada por el arco. Representaremos el vector tangente $\alpha'(s)$ de $\alpha$ en el instante $s\in I$ por $T(s)\in \mathbb{R}^2$, del que sabemos que $|T(s)| = 1 \quad \forall s\in I$.
...
\begin{ejemplo}
    ...
    \begin{enumerate}
        ...
        \item Sea $\beta:\mathbb{R}\to \mathbb{R}^2$ dada por:
            ...
            Vemos que:
            \begin{equation*}
                \beta'(s) = \left(-\sen\left(\frac{s}{r}\right),\cos\left(\frac{s}{r}\right)\right) \qquad \forall s\in I
            \end{equation*}
            por lo que obtenemos $|\beta'(s)| = 1 \quad \forall s\in I$. Así, hemos obtenido:
            \begin{equation*}
                T(s) = \left(-\sen\left(\frac{s}{r}\right),\cos\left(\frac{s}{r}\right)\right) \qquad \forall s\in I
            \end{equation*}
            Por lo que:
            \begin{equation*}
            ...
            \end{equation*}
            Hemos obtenido que $k(s) = \frac{1}{r} \quad \forall s\in I$.
    \end{enumerate}
\end{ejemplo}
