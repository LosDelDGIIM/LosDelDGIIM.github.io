...
Dado ahora $X=(X_1,\dots,X_n)$ un vector aleatorio, consideramos ahora la distribución:
\begin{equation*}
    (\max X, \min X)
\end{equation*}

Dados $x,y\in \bb{R}^2$, tenemos que:
\begin{equation*}
    P[(\max X, \min X)\leq (x,y)] = P[\max X\leq x, \min X\leq y]
\end{equation*}
\begin{itemize}
    \item Si $x\leq y$, entonces $\min X\leq \max X\leq x\leq y$, luego:
    \begin{equation*}
