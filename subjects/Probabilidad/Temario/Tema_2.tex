...
\begin{prop}
    Sea $X$ un vector aleatorio $n-$dimensional, y $g:\left(\bb{R}^n,\cc{B}^n\right)\to \left(\bb{R}^m,\cc{B}^m\right)$ una función medible. Entonces, $Y=g(X)$ es un vector aleatorio $m-$dimensional.
\end{prop}
\begin{proof}
...
Sea $X=(X_1, \ldots, X_n)$ un vector aleatorio discreto con función de densidad $f_X$, y sea $g:(\bb{R}^n, \cc{B}^n)\to (\bb{R}^m, \cc{B}^m)$ una función medible tal que $Y=g(X)$ es un vector aleatorio discreto con valores en $E_Y\subset \bb{R}^m$. Su función masa de probabilidad se obtiene a partir de $f_X$ como:
\begin{equation*}
...
\begin{equation*}
    f_Y(y) = f_X(g^{-1}(y)) \left|Jg^{-1}(y)\right|, \quad \forall y\in g(E_X).
\end{equation*}