...
El gran uso que tiene $P_X$ es que induce una medida de probabilidad en $\mathbb{R}^n$, y por tanto obtenemos el espacio probabilístico $(\mathbb{R}^n, \mathcal{B}^n, P_X)$.
...
Algunas de las propiedades que la función de distribución cumple son:
\begin{enumerate}
...
Al igual que en el caso unidimensional, podemos clasificar los vectores aleatorios en discretos y continuos.
\begin{observacion}
    En EDIP, ya se vio que, además de variables aleatorias continuas y discretas, había otros tipos de variables aleatorias, las mixtas. No obstante, durante toda esa asignatura el estudio se centró en los dos primeros casos.
    En este caso, el estudio también se limitará a vectores aleatorios de uno de los tipos que describiremos.
\end{observacion}

Esta distinción se hace en función de la naturaleza de los valores que toma el vector aleatorio.
...
Al igual que en el caso de vectores discretos, tenemos el siguiente resultado (aunque en este caso no es una caracterización, ya que la implicación restante no será cierta en general).
...
\subsubsection{Caso continuo}
% // TODO: Entender Condicionadas
% https://www.ugr.es/~cdpye/CursoProbabilidad/pdf/P_T06_CondicionadasContinua.pdf
\begin{definicion}
...
Al igual que ocurría en el caso unidimensional, tenemos un Teorema de Canvio de Variable. Dado un vector aleatorio $X=(X_1, \ldots, X_n)$, podemos estudiar la distribución de otro vector aleatorio $Y=(Y_1, \ldots, Y_m)$ que depende de $X$ mediante una transformación.
\begin{prop}
...
\begin{equation*}
    F_Y(y) = P_Y(\left]-\infty, y\right]) = P_X(g^{-1}(\left]-\infty, y\right])) = F_X(g^{-1}(y))   \qquad \forall y\in \bb{R}^m.
\end{equation*}
...
Como veremos en el siguiente teorema, al aplicarle una transformación a un vector aleatorio discreto, podemos afirmar que llegaremos a otro vector aleatorio discreto. Esto nos permite contemplar todos los casos, algo que no podremos hacer en el caso continuo.
\begin{teo}
...
\end{teo}
\begin{proof}
    La demostración es totalmente análoga al caso unidimensional, disponible en el Temario de EDIP.
\end{proof}
...
En el caso de vectores aleatorios continuos, tras aplicarle una transformación no podemos asegurar que obtengamos un vector aleatorio continuo; ya que pueden suceder distintas combinaciones. Un caso particular es que obtengamos un vector aleatorio discreto, caso que contempla el siguiente teorema.
...
Otro caso especialmente relevante es que, tras aplicar la transformación, obtengamos un vector aleatorio continuo.
\begin{teo}
...
Introducimos además el siguiente resultado, que no pudimos demostrar en la asignatura de EDIP.
\begin{prop}[Linealidad de la esperanza]
...
Este resultado, ya conocido por el lector por usarse en la asignatura de EDIP, tiene importantes corolarios que recomendamos al lector recordar consultando dichos apuntes.
...
Es importante notar que, fijado $i\in \{1,\dots,n\}$, si $k_j=0$ para todo $j\neq i$, entonces se obtienen los momentos de la variable aleatoria $X_i$. Estos se conocen como momentos no centrados marginales.
...
De nuevo, es importante notar que, fijado $i\in \{1,\dots,n\}$, si $k_j=0$ para todo $j\neq i$, entonces se obtienen los momentos centrados de la variable aleatoria $X_i$. Estos se conocen como momentos centrados marginales.\\

Al igual que ocurrió en Estadística Descriptiva Bidimensional, cobran especial importancia los momentos de segundo orden. 
...
La covarianza es invariante frente a cambio de origen, aunque no frente a cambio de escala:
\begin{prop}
    Sea $(X,Y)$ un vector aleatorio, y sean $a_x,b_x,a_y,b_y\in \bb{R}$. Entonces:
    \begin{equation*}
        \Cov[a_xX+b_x,a_yY+b_y] = a_xa_y\Cov(X,Y).
    \end{equation*}
\end{prop}
\begin{proof}
    Tenemos (análogamente para $Y$) que:
    \begin{align*}
        a_xX+b_x - E[a_xX+b_x] &= a_xX+b_x - a_xE[X]-b_x = a_x(X-E[X])
    \end{align*}

    Por tanto:
    \begin{equation*}
        \Cov[a_xX+b_x,a_yY+b_y] = E[a_xa_y(X-E[X])(Y-E[Y])] = a_xa_y\Cov(X,Y).
    \end{equation*}
\end{proof}
...
\begin{lema}
    Sean $X,Y$ dos variables aleatorias tales que $E[X^2],E[Y^2]$. Entonces:
    \begin{itemize}
        \item $\exists E[XY]$.
...
\end{lema}
\begin{proof}
    Supuesto cierto lo primero, tenemos que:
    \begin{align*}
        \Cov(X,Y) &= E[(X-E[X])(Y-E[Y])] = E[XY - XE[Y] - E[X]Y + E[X]E[Y]] =\\
        &= E[XY] - E[X\red{E[Y]}] - E[\red{E[X]}Y] + E[\red{E[X]E[Y]}] =\\
        &= E[XY] - 2E[X]E[Y] + E[X]E[Y] = E[XY] - E[X]E[Y].
    \end{align*}
    donde hemos usado que $\red{E[X]},\red{E[Y]}$ son constantes.
\end{proof}
...
\end{itemize}
Este estudio se denomina correlación, y se entenderá a fondo en Regresión Multivariante, cuando se defina el coeficiente de correlación de Pearson.

\begin{prop}\label{prop:VarSuma}
    Sean $X_1,\dots,X_n$ $n$ variables aleatorias definidas sobre el mismo espacio de probabilidad tales que $\exists E[X_i^2]$ para todo $i=1,\dots,n$. Entonces:
    \begin{equation*}
        \exists \Var\left[\sum_{i=1}^n X_i\right] = \sum_{i=1}^n \Var[X_i] + \sum_{\substack{i,j=1\\i\neq j}}^n \Cov(X_i,X_j).
    \end{equation*}
\end{prop}
\begin{proof}
    Tenemos que:
    \begin{align*}
        ...
        \AstIg \\&\AstIg E\left[\sum_{i=1}^n (X_i - E[X_i])^2 + \sum_{\substack{i,j=1\\i\neq j}}^n (X_i - E[X_i])(X_j - E[X_j])\right]
        =\\&= \sum_{i=1}^n E[(X_i - E[X_i])^2] + \sum_{\substack{i,j=1\\i\neq j}}^n E[(X_i - E[X_i])(X_j - E[X_j])]
        =\\&= \sum_{i=1}^n \Var[X_i] + \sum_{\substack{i,j=1\\i\neq j}}^n \Cov(X_i,X_j).
    \end{align*}
    donde la igualdad $(\ast)$ se demuestra mediante inducción.
    \begin{itemize}
        \item Para $n=2$:
        \begin{equation*}
            \hspace{-2cm}
            (X_1 - E[X_1] + X_2 - E[X_2])^2= (X_1 - E[X_1])^2 + (X_2 - E[X_2])^2 + 2(X_1 - E[X_1])(X_2 - E[X_2])
        \end{equation*}

        \item Supuesto cierto para $n$, veamos que se cumple para $n+1$:
        \begin{align*}
            &\left(\sum_{i=1}^{n+1} (X_i - E[X_i])\right)^2 = \left(\sum_{i=1}^{n} (X_i - E[X_i]) + (X_{n+1} - E[X_{n+1}])\right)^2
            =\\&= \left(\sum_{i=1}^{n} (X_i - E[X_i])\right)^2 + (X_{n+1} - E[X_{n+1}])^2 + 2\left(\sum_{i=1}^{n} (X_i - E[X_i])\right)(X_{n+1} - E[X_{n+1}])
            =\\&= \sum_{i=1}^{n} (X_i - E[X_i])^2+\sum_{\substack{i,j=1\\i\neq j}}^{n} (X_i - E[X_i])(X_j - E[X_j]) +\\&\qquad\qquad + (X_{n+1} - E[X_{n+1}])^2 + 2\sum_{i=1}^{n} (X_i - E[X_i])(X_{n+1} - E[X_{n+1}])
            =\\&= \sum_{i=1}^{n+1} (X_i - E[X_i])^2+\sum_{\substack{i,j=1\\i\neq j}}^{n+1} (X_i - E[X_i])(X_j - E[X_j])
        \end{align*}
    \end{itemize}
...
\begin{definicion}[Función generatriz de momentos]
    ...
    Notemos que, usando el producto escalar en $\bb{R}^n$ o la transpuesta de una matriz, podemos escribir la función generatriz de momentos como:
    \Func{M_X}{N}{\bb{R}}{t}{E[e^{\langle t,X\rangle}]=E[e^{tX^t}]}.
\end{definicion}
...
\end{teo}
% // TODO: Teorema de Unicidad de fgm
