...
\begin{ejercicio}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{align*}
                F_{(X,Y)}(x, y) &= \int_{-\infty}^x \int_{-\infty}^y f(u, v) \, dv \, du = \int_{0}^x \int_{0}^y 1 \, dv \, du = \int_{0}^x \left[v\right]_0^y \, du = \int_{0}^x y \, du = \left[yu\right]_0^x = yx.
            \end{align*}

            \item \ul{Si $x\in \left]0,1\right[$ \quad y \quad $y\in \left]-1,0\right[$ \quad y \quad $x+y\geq 0$} (zona $R_2$):
            \begin{align*}
                F_{(X,Y)}(x, y) &= \int_{-\infty}^x \int_{-\infty}^y f(u, v) \, du \, dv = \int_{0}^x \int_{-1}^y 1 \, dv \, du = \int_{0}^x \left[v\right]_{-1}^y \, du = \int_{0}^x y+1 \, du =\\&= \left[yu+u\right]_0^x = yx+x+1.
            \end{align*}
        \end{itemize}
    \end{enumerate}
\end{ejercicio}
