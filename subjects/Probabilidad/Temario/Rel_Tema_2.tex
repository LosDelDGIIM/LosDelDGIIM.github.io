...
\begin{ejercicio}
    ...
    \begin{enumerate}
        ...
        Calculamos por tanto dicho valor sabiendo $f_{(X,Y)}$. Para $x\in [0,1]$, tenemos que:
        \begin{align*}
            P[(X,Y)\leq (x,x)]&=\int_{0}^{x}\int_{0}^{x} k \, dv \, du = k\int_{0}^{x} x \, du
            = kx\left[u\right]_{0}^{x} = kx^2 = x^2
        \end{align*}

        Por tanto, la función de distribución de probabilidad de $\max(X,Y)$ es:
        \begin{equation*}
            F_{\max(X,Y)}(x) = \begin{cases}
                0 & x\leq 0, \\
                x^2 & 0<x<1, \\
                1 & 1\leq x.
            \end{cases}
        ...
        Respecto del $\min(X,Y)$, dado $x\in \bb{R}$, tenemos que:
        \begin{equation*}
            P[\min(X,Y)\leq x]=1-P[\min(X,Y)>x]=1-P[(X,Y)>x]
        \end{equation*}

        Calculamos dicho valor sabiendo $f_{(X,Y)}$. Distinguimos en función de $x$:
        \begin{itemize}
            \item Si $x\leq 0$, entonces $P[(X,Y)>x]=1$.

            \item Si $0<x<1$, entonces:
            \begin{align*}
                P[(X,Y)>x]&=\int_{x}^{1}\int_{x}^{1} k \, dv \, du = k\int_{x}^{1} (1-x) \, du
                = k(1-x)\left[u\right]_{x}^{1} = (1-x)^2
            \end{align*}

            \item Si $x\geq 1$, entonces $P[(X,Y)>x]=0$.
        \end{itemize}

        Por tanto, la función de distribución de probabilidad de $\min(X,Y)$ es:
        \begin{equation*}
            F_{\min(X,Y)}(x) = 1-P[(X,Y)>x]=\begin{cases}
                0 & x\leq 0, \\
                1-(1-x)^2 & 0<x<1, \\
                1 & 1\leq x.
            \end{cases}
        ...
        Dado $x,y\in \bb{R}$, distinguimos casos:
        \begin{itemize}
            \item Si $x\leq y$, como $\min (X,Y)\leq \max(X,Y)$, entonces:
            \begin{align*}
                P[\max(X,Y)\leq x,\min(X,Y)\leq y]&=P[\max(X,Y)\leq x] = F_{\max(X,Y)}(x)
            \end{align*}

            \item Si $x>y$, entonces:
            \begin{align*}
                P[\max(X,Y)\leq& x,\min(X,Y)\leq y]
                \\&=P[\max(X,Y)\leq x]-P[\max(X,Y)\leq x,\min(X,Y)> y]
                =\\&= P[\max(X,Y)\leq x]-P[y<X\leq x,y<Y\leq x]
            \end{align*}

            Calculamos $P[y<X\leq x,y<Y\leq x]$ sabiendo $f_{(X,Y)}$.
            % // TODO: Terminar
        \end{itemize}
    \end{enumerate}
\end{ejercicio}
...
\begin{ejercicio}
    ...
    Tenemos por tanto la siguiente sutuación:
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}
            \begin{axis}[
    ...
    \begin{itemize}
        \item Si $z\leq 0$ o $t<0$, entonces:
        \begin{equation*}
            P[\min(X,Y)\leq z,T\leq t]=0
        \end{equation*}

        \item Si $z>0$ y $t\in \left[0,1\right[$, entonces:
        \begin{align*}
        ...
        \item Si $z>0$ y $t\geq 1$, entonces:
        % // TODO: Se podría hacer como P[\min (X,Y)\leq <z]???
        \begin{align*}
    ...
    Por tanto, la función de distribución de probabilidad conjunta de $(Z,T)$ es:
    \begin{equation*}
        F_{(Z,T)}(z,t) = \begin{cases}
            0 & z\leq 0 \text{ o } t<0, \\
