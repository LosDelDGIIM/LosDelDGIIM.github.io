...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{equation*}
            P[X = x, Y = y] \leq P[X = x] \qquad \forall x,y
        \end{equation*}
...
Al igual que ocurría con la esperanza condicionada de $X$ a $Y$, para que exista basta con imponer que $E[g(X)]<\infty$.
\begin{coro}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item Dado que $\exists E[X^2]$, entonces $\exists E[X^2\mid Y]$ (de hecho, son iguales). Como la existencia de momentos no centrados implica la de momentos centrados, entonces $\exists E[(X - E[X\mid Y])^2\mid Y] = \Var[X\mid Y]$.
