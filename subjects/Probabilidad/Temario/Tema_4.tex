...
\begin{proof}
    ...
    \begin{itemize}
        \item Si $X$ e $Y$ son discretas, entonces:
        \begin{align*}
            E[|X\mid Y=y|] &= \sum_{x\in E_x} |x|\cdot P[X = x\mid Y = y] = \sum_{x\in E_x} |x|\cdot \dfrac{P[X = x, Y = y]}{P[Y = y]} =\\&= \dfrac{\sum\limits_{x\in E_x} |x|\cdot P[X = x, Y = y]}{P[Y = y]} \leq \dfrac{\sum\limits_{x\in E_x} |x|\cdot P[X = x]}{P[Y = y]} = \dfrac{E[|X|]}{P[Y = y]}<\infty
        \end{align*}
        donde en la desigualdad hemos hecho uso de que
        \begin{equation*}
            P[X = x, Y = y] \leq P[X = x] \qquad \forall x,y\in \bb{R}
        \end{equation*}
        y, al final, hemos usado que $E[|X|]<\infty$ por tener $E[X]$ finita. Por tanto, $E[X\mid Y]$ existe.

        \item Si $X$ e $Y$ son continuas, entonces:
        \begin{align*}
            E[|X\mid Y=y|] &= \int_{-\infty}^{\infty} |x|\cdot f_{X\mid Y=y}(x)~dx = \int_{-\infty}^{\infty} |x|\cdot \dfrac{f_{X,Y}(x,y)}{f_Y(y)}~dx =\\&= \dfrac{\int\limits_{-\infty}^{\infty} |x|\cdot f_{X,Y}(x,y)~dx}{f_Y(y)}
        \end{align*}
        Como $E[|X|]<\infty$, entonces:
        \begin{multline*}
            E[|X|]=\int_{-\infty}^{\infty} |x|\int_{-\infty}^{\infty} f_{X,Y}(x,y)~dy~dx = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} |x|\cdot f_{X,Y}(x,y)~dy~dx<\infty
            \Longrightarrow\\\Longrightarrow
            \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} |x|\cdot f_{X,Y}(x,y)~dx~dy<\infty
            \Longrightarrow
            \int_{-\infty}^{\infty} |x|\cdot f_{X,Y}(x,y)~dx<\infty \qquad \forall y\in \bb{R}
        \end{multline*}
...
Formalmente, dadas dos variables aleatorias $X$ e $Y$ sobre el mismo espacio de probabilidad, y una función $g:\mathbb{R}\to\mathbb{R}$ medible, $g(X)$ es una variable aleatoria, y su esperanza condicionada a $Y$ es:
    \begin{equation*}
        E[g(X)\mid Y](y) := E[g(X)\mid Y = y]
    \end{equation*}

    Veamos en función de si son discretas o continuas:
    \begin{itemize}
        \item Si $X$ e $Y$ son discretas, entonces:
        \begin{equation*}
...
Al igual que ocurría con la esperanza condicionada de $X$ a $Y$, por ser $g(X)$ una variable aleatoria para que exista la esperanza condicionada basta con imponer que $E[g(X)]<\infty$.
\begin{coro}\label{coro:existenciaEsperanzaCondicionadaFuncion}
...
Siguiendo una demostración totalmente análoga a la de la linealidad de la esperanza de variables aleatorias, se tiene la linealidad de la esperanza condicionada.
...
\begin{prop}
    ...
    \begin{equation*}
        E[X\mid Y] \geq 0 \qquad \land \qquad E[X\mid Y] =0\Longleftrightarrow P[X=0\mid Y=y] = 1~~~\forall y\in \bb{R}
    \end{equation*}
\end{prop}
\begin{proof}
    Como $X\geq 0$, entonces se tiene de forma directa el primer resultado por la definición de esperanza condicionada. Para el segundo resultado, tenemos que:
    \begin{align*}
        E[X\mid Y] = 0 &\Longleftrightarrow E[X\mid Y = y] = 0 \quad \forall y\in \bb{R}
    \end{align*}

    En la asignatura de EDIP se vio que la esperanza de una variable aleatoria no negativa es cero si y solo si la variable aleatoria es degenerada en el cero. Por tanto, tenemos que:
    \begin{align*}
        E[X\mid Y] = 0 &\Longleftrightarrow P[X = 0\mid Y = y] = 1 \quad \forall y\in \bb{R}
    \end{align*}
\end{proof}
...
\end{prop}
\begin{proof}
    Se tiene de forma directa por la linealidad y la propiedad de conservación del orden, siguiendo una demostración análoga a la de la conservación del orden de la esperanza de variables aleatorias.
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{equation*}
            P[X = x\mid Y = y] = P[X = x] \quad \forall x\in E_x,~y\in E_y
        \end{equation*}
        Por tanto, tenemos para todo $y\in E_Y$:
        \begin{align*}
        ...
        \begin{equation*}
            f_{X\mid Y=y}(x) = f_X(x) \quad \forall x,y\in \bb{R}
        \end{equation*}
        Por tanto, tenemos para todo $y\in \bb{R}$:
        \begin{align*}
...
Usando el Corolario~\ref{coro:existenciaEsperanzaCondicionadaFuncion}, tenemos que $\exists E[X^k]$ implica que exista $E[X^k\mid Y]$, y se calcula como:
\begin{itemize}
    \item Si $X$ e $Y$ son discretas, entonces:
    \begin{equation*}
...
\end{prop}
\begin{proof}
    En primer lugar, tenemos que $\exists E[X^2]$ implica $\exists E[E[X^2\mid Y]]$. Por tanto, del segundo apartado de la proposición anterior, tenemos que:
    \begin{equation*}
...
\begin{teo}
    Sea $Y$ una variable aleatoria sobre el espacio de probabilidad $(\Omega, \mathcal{A}, P)$, y $X$ una variable aleatoria sobre el mismo espacio de probabilidad. Entonces, la función $E[Y\mid X]$ minimiza el E.C.M. Es decir:
    \begin{equation*}
...
\end{teo}
%https://www.ugr.es/~cdpye/CursoProbabilidad/pdf/P_T08_ProblemaRegresion.pdf
...
\begin{align*}
    ...
    =\\&=
    \int_\bb{R} \int_\bb{R} (y - E[Y\mid X = x])^2\cdot f_{X,Y}(x,y)~dx~dy
    =\\&=
    \int_\bb{R} \int_\bb{R} (y - E[Y\mid X = x])^2\cdot f_{Y\mid X=x}(y)f_X(x)~dx~dy
    =\\&=
    \int_\bb{R} E[(Y - E[Y\mid X = x])^2\mid X = x]\cdot f_X(x)~dx
    =\\&=
...
\end{definicion}
% // TODO: Definir dependencia funcional
\begin{prop}
    Sean $X,Y$ dos variables aleatorias sobre el mismo espacio de probabilidad.
    Equivalen:
    \begin{enumerate}
        \item $\Var[Y\mid X] = 0$.
        \item $E[\Var[Y\mid X]] = 0$.
        \item $Y$ depende funcionalmente de $X$.
        \item $\text{E.C.M.}(\varphi_{\text{opt}}) = 0$.
    \end{enumerate}
\end{prop}
\begin{proof}
    Demostramos mediante distintas equivalencias:
    \begin{description}
        \item[$(1)\Longleftrightarrow (2)$] Sabemos que $\Var[Y\mid X]\geq 0$, por lo que la equivalencia se tiene de forma directa.
        \item[$(3)\Longrightarrow (4)$] Tenemos que $Y=\varphi_{\text{opt}}(X)$, por lo que:
        \begin{equation*}
            \text{E.C.M.}(\varphi_{\text{opt}}) = E[(Y - \varphi_{\text{opt}}(X))^2] = E[(Y - Y)^2] = 0
        \end{equation*}

        \item[$(4)\Longrightarrow (3)$] Aunque se ve intuitivamente, veámoslo formalmente.
        
        Dado que $\text{E.C.M.}(\varphi_{\text{opt}}) = 0$, entonces:
        \begin{equation*}
            E[(Y - \varphi_{\text{opt}}(X))^2] = 0
        \end{equation*}
        Esta es la esperanza de una variable aleatoria no negativa, por lo que:
        \begin{equation*}
            Y-\varphi_{\text{opt}}(X) = 0 \Longrightarrow Y = \varphi_{\text{opt}}(X)
        \end{equation*}

        \item[$(1)\Longrightarrow (3)$] Partimos de:
        \begin{equation*}
        ...
        \item[$(3)\Longrightarrow (1)$] Partimos de que $Y$ depende funcionalmente de $X$, es decir, $\exists f:E_X\to E_Y$ tal que:
        \begin{equation*}
            Y = f(X)
        \end{equation*}
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\centering\arraybackslash}m{3cm} |>{\centering\arraybackslash}m{3cm} >{\centering\arraybackslash}m{3cm} >{\centering\arraybackslash}m{3cm}}
        ...
        E.C.M. & $\Var[Y]$ & $E[\Var[Y\mid X]]$ & $\Var[Y\mid X=x]$\\
        \bottomrule
    \end{tabular}
...
\end{definicion}
\begin{observacion}
    En el caso de que, por ejemplo, $Y$ sea constante, tenemos $\Var[Y]=0$. Se define en este caso $\eta_{Y/X}^2 = 1$.
    Análogamente, si $X$ es constante, se define $\eta_{X/Y}^2 = 1$.
\end{observacion}
...
En esta última proposición, vemos que nos falta estudiar el caso en el que se da $\eta_{Y/X}^2 = \eta_{X/Y}^2 = 0$. Este lo veremos más adelante, porque se trata de ejemplos de rectas de regresión.
...
Por tanto, razonando de forma idéntica a como hicimos en la sección anterior, tenemos que el problema de regresión se reduce a encontrar los valores de $a$ y $b$ que minimizan el E.C.M. asociado a la aproximación. Es decir, la recta de regresión de $Y$ sobre $X$ es $\varphi_{\text{opt}}^L(X)=aX+b$ tales que $a,b\in\bb{R}$ minimizan:
\begin{equation*}
    E[(Y - aX - b)^2]
\end{equation*}
...
\begin{lema}
    Dadas dos variables aleatorias $X$ e $Y$ sobre el mismo espacio de probabilidad, el vector de medias $(E[X], E[Y])$ es el punto de las rectas de regresión de $Y$ sobre $X$ y de $X$ sobre $Y$.
\end{lema}
\begin{proof}
    Para ello, basta con comprobar que el punto $(E[X], E[Y])$ pertenece a las rectas de regresión de $Y$ sobre $X$ y de $X$ sobre $Y$. Estas rectas son:
    \begin{align*}
        Y &= E[Y] + \dfrac{\Cov(X,Y)}{\Var[X]}(X - E[X])\\
        X &= E[X] + \dfrac{\Cov(X,Y)}{\Var[Y]}(Y - E[Y])
    \end{align*}

    Al evaluar en $X=E[X]$ y $Y=E[Y]$, tenemos que dicho punto pertenece a ambas rectas de regresión. Este resultado técnico nos será útil en la práctica.
\end{proof}
...
Como primer resultado, de la definición deducimos que $\rho_{X,Y}=\rho_{Y,X}$, por lo que nos referiremos a $\rho_{X,Y}$ como coeficiente de correlación lineal de Pearson.
...
Como resultado directo de esta última definición, tenemos la siguiente caracterización, que en muchos casos es la que se da como definición de incorrelación.
\begin{prop}
    Sean $X$ e $Y$ dos variables aleatorias sobre el espacio de probabilidad $(\Omega, \mathcal{A}, P)$. Entonces:
    \begin{equation*}
