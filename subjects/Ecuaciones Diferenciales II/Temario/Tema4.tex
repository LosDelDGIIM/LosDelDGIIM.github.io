...
\subsection{Demostración del Teorema} % // TODO: Copiar el enunciado del Tª de aquí
\noindent
Recordemos que teníamos un campo $X:G\subset\mathbb{R}\times\mathbb{R}^d\times\mathbb{R}^m\to\mathbb{R}^d$ continuo con $G$ abierto, conexo y $\exists \frac{\partial X}{\partial x}(t,x,\lm),\frac{\partial X}{\partial \lm}(t,x,\lm)\quad \forall (t,x,\lm)\in G$, de forma que las aplicaciones:
\begin{equation*}
    (t,x,\lm)\in G\longmapsto \frac{\partial X}{\partial x}(t,x,\lm)\in \mathbb{R}^{d\times d},\quad \frac{\partial X}{\partial \lm}(t,x,\lm)\in \mathbb{R}^{d\times m}
\end{equation*}
...
\begin{equation*}
    \dot{x} = X(t,x,\lm), \qquad x(t_0) = x_0
\end{equation*}
definido en alguna componente conexa de $G_\lm$. Para un punto $(t_0,x_0)\in \mathbb{R}\times\mathbb{R}^d$ fijo con $(t_0,x_0)\in G_\lm$. Consideramos:
\begin{equation*}
    \cc{G} = \{(t,\lm)\in \mathbb{R}\times\mathbb{R}^m~:~(t_0,x_0,\lm)\in G,\quad \alpha(\lm)<t<\omega(\lm)\}
\end{equation*}
que nos permite definir
\Func{x}{\cc{G}}{\bb{R}^d}{(t,\lm)}{x(t,\lm)}
La tesis es que $\cc{G}\subset \mathbb{R}\times\mathbb{R}^m$ es abierto, que $x\in C^1(\cc{G},\mathbb{R}^d)$ y además que si escribimos $y(t) = \frac{\partial x}{\partial \lm}(t,\lm)$ es solución del PVI:
\begin{equation*}
    \dot{y} = \frac{\partial X}{\partial x}(t,x(t,\lm),\lm)\ y + \frac{\partial X}{\partial \lm}(t,x(t,\lm),\lm),\qquad y(t_0) = 0
\end{equation*}
\begin{proof}
    ...
    \begin{equation*}
        \left\{\begin{array}{ll}
                \dot{x} = X(t,x,y), & x(t_0) = x_0 \\
                \dot{y} = 0, & y(t_0) = \lm
        \end{array}\right.
    \end{equation*}
    Es claro que si consideramos $\xi = (x,y)\in \mathbb{R}^d\times\mathbb{R}^m$ tenemos que:
    \begin{equation*}
        \dot{\xi} = \cc{X}(t,\xi), \qquad \xi(t_0) = \xi_0 = (x_0,\lm)
    \end{equation*}
    Para un campo $\cc{X}:G\to\mathbb{R}^d\times\mathbb{R}^m$ dado por:
    \begin{equation*}
        \cc{X}(t,\xi) = (X(t,x,y), 0)
    \end{equation*}
    que es continuo y existe la parcial respecto $\xi$, por lo que podemos aplicarle el Teorema de diferenciabilidad respecto a condiciones iniciales, de donde si consideramos:
    \begin{equation*}
        \cc{D} = \{(t,t_0,\xi_0) : (t_0,\xi_0)\in G, \alpha(t_0,\xi_0)<t<\omega(t_0,\xi_0)\}
    \end{equation*}
    Sabemos ya que $\cc{D}\subset \mathbb{R}\times\mathbb{R}\times\mathbb{R}^d\times\mathbb{R}^m$ es un abierto y que $\xi \in C^1(\cc{D},\mathbb{R}^d\times\mathbb{R}^m)$. De aquí podemos deducir ya que $\cc{G}$ es abierto y que $x\in C^1(\cc{G},\mathbb{R}^d)$, ya que: 
    \begin{equation*}
        \left(\begin{array}{cc}
            x(t,\lm) \\
            \lm 
        \end{array}\right) = \xi(t;t_0,\xi_0) 
    \end{equation*}
    Obtenemos $x$ de $\xi$ fijando dos variables y tomando sus primeras $d$ coordenadas. De manera formal, podemos ver $x$ como composición con la ayuda de la proyección $\pi:\mathbb{R}^d\times\mathbb{R}^m\to\mathbb{R}^d$ y la inclusión $i:\mathbb{R}\times\mathbb{R}^m\to\mathbb{R}\times\mathbb{R}\times\mathbb{R}^d\times\mathbb{R}^m$:
    \begin{equation*}
        x = \pi \circ \xi \circ i
    \end{equation*}
    Ahora, obtenemos $\cc{G}$ como un corte de $\cc{D}$:
    \begin{equation*}
        \cc{G} = \left\{(t,\lm) : \left(t,t_0,\left(\begin{array}{cc}
            x_0 \\
            \lm 
        \end{array}\right)\right)\in \cc{D}\right\}
    \end{equation*}
    Falta ahora probar que si tomamos $y(t) = \frac{\partial x}{\partial \lm}(t,\lm)$ entonces se cumple el PVI enunciado. Para ello, observemos primero que:
    \begin{equation*}
        \frac{\partial \xi}{\partial \xi_0}(t;t_0,\xi_0)\in \mathbb{R}^{(d+m)\times(d+m)}, \quad \frac{\partial x}{\partial \lm}(t,\lm)\in \mathbb{R}^{d\times m}
    \end{equation*}
    La relación existente es que la segunda es submatriz de la primera:
    \begin{equation*}
        \frac{\partial \xi}{\partial \xi_0} = \left(\begin{array}{cc}
            \frac{\partial x}{\partial \xi_0} \\
            \hline 
             \frac{\partial y}{\partial \xi_0}
        \end{array}\right) = \left(\begin{array}{c|c}
            \frac{\partial x}{\partial x_0} & \frac{\partial x}{\partial \lm} \\
            \hline
            \frac{\partial y}{\partial x_0} & \frac{\partial y}{\partial \lm} 
        \end{array}\right) = \left(\begin{array}{c|c}
            \frac{\partial x}{\partial x_0} & \frac{\partial x}{\partial \lm} \\
            \hline
             0 &  I_m
    ...
    \begin{equation}\label{eq:producto_matricial}
        \dot{Y} = \frac{\partial \cc{X}}{\partial \xi}(t,\xi(t;t_0,\xi_0))\ Y, \qquad Y(0) = I_{d+m}
    \end{equation}
    Calculamos primero:
    \begin{equation*}
        \frac{\partial \cc{X}}{\partial \xi}(t,\xi) = \left(\begin{array}{c|c}
            \frac{\partial X}{\partial x}(t,x,\lm) & \frac{\partial X}{\partial \lm}(t,x,\lm) \\
            \hline
            0 & 0 
        \end{array}\right)
    \end{equation*}
    Así, si realizamos el producto matricial de la ecuación~\eqref{eq:producto_matricial} con las 4 cajas para buscar la caja correspondiente a $y(t)$ obtenemos:
    \begin{equation*}
        \dot{y} = \frac{\partial X}{\partial t}(t,x(t,\lm),\lm)\ y + \frac{\partial X}{\partial \lm}(t,x(t,\lm),\lm)
    \end{equation*}
    Y la caja $2,1$ de $I_{d+m}$ es $0$, por lo que $y(t_0) = 0$.
\end{proof}

\subsection{Motivación histórica}
\noindent
¿Cómo hacían los clásicos para utilizar esto si no usaban el concepto de ``matriz''?
\begin{ejemplo}
    Consideramos la ecuación:
    \begin{equation*}
        \ddot{x} + x + \varepsilon x^3 = 0, \qquad x(0) = 0, \quad \dot{x}(0) = 1
    \end{equation*}
    Que reescribimos como:
    \begin{equation*}
        \frac{\partial^2 x}{\partial t^2}(t,\varepsilon) + x(t,\varepsilon) + \varepsilon x(t,\varepsilon)^3 = 0, \qquad x(0,\varepsilon) = 0, \quad \frac{\partial x}{\partial t}(0,\varepsilon) = 1
    \end{equation*}
    Lo que haremos ahora será derivar respecto $\varepsilon$ y conmutar las derivadas (este cálculo es totalmente riguroso, bien por el Teorema o bien si se sabe mucho tenemos que $x(t,\varepsilon)$ es de clase $C^\infty$, de hecho es analítica):
    \begin{multline*}
        \frac{\partial^2 }{\partial t^2}\left(\frac{\partial x}{\partial \varepsilon}(t,\varepsilon)\right) + \frac{\partial x}{\partial \varepsilon}(t,\varepsilon) + x(t,\varepsilon)^3 + 3\varepsilon x(t,\varepsilon)^2 \frac{\partial x}{\partial \varepsilon}(t,\varepsilon) = 0 \\
        \frac{\partial x}{\partial \varepsilon}(0,\varepsilon) = 0, \quad \frac{\partial }{\partial t}\left(\frac{\partial x}{\partial \varepsilon}\left(0,\varepsilon\right)\right) = 0
    \end{multline*}
    Si ahora llamamos $y(t) = \frac{\partial x}{\partial \varepsilon}(t,\varepsilon)$, tenemos entonces que podemos reescribirlo como:
    \begin{equation*}
        \ddot{y} + y + x(t,\varepsilon)^3 + 3\varepsilon x(t,\varepsilon)^2 y = 0, \qquad y(0) = 0, \quad \dot{y}(0) = 0
    \end{equation*}
    Sabemos que $x(t,0) = \sen t$ puesto que para $\varepsilon=0$ la ecuación es sencilla. Una vez conocido esto tenemos que:
    \begin{equation*}
        \ddot{y} + y + \sen^3 t = 0, \qquad y(0) = 0,\quad \dot{y}(0) = 0
    \end{equation*}
...
% // TODO: De esto caen muchos ejercicios
