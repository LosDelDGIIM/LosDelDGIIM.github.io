...
% // TODO: Hasta aquí entra en el parcial, pero en el final es todo el temario
% Esto es solo para el curso 25/26, próximos cursos depende del tiempo que tenga

    \noindent
    Para elevar $A = \lm I + N$ usaremos el binomio de Newton, que podemos usarlo puesto que la matriz identidad $I$ conmuta con cualquier otra. Vemos que $A\in \mathbb{R}^{r\ŧimes r}$ con $\lm\in \mathbb{C}$. Sabemos ya que $N^r = 0$. En primer lugar, debemos observar que no siempre se tiene que:
    \begin{equation*}
        e^{A+B} = e^Ae^B
    \end{equation*}
    puesto que puede suceder que $AB\neq BA$. Sin embargo, si $AB = BA$ sí que se da la igualdad, puede demostrarse con el binomio de Newton (hágase). % // TODO: Pedir a Irina
\end{proof}
