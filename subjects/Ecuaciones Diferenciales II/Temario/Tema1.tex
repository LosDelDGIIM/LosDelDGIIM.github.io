...
\begin{ejercicio}
    ...
    \noindent
    La condición $\sigma(\mathbb{N})\subset \tau(\mathbb{N})$ se traduce a que:
    \begin{equation*}
        \forall n\in \mathbb{N}~~\exists m\in \mathbb{N} \quad :\quad \sigma(n) = \tau(m)
    \end{equation*}
    Para ver que $\sigma(n)\geq \tau(n)$, dado $n\in \mathbb{N}$ tenemos que existe $m_n\in \mathbb{N}$ de forma que $\sigma(n) = \tau(m_n)$. ¿Puede ser $m_n<n$? No, ya que para cada $0\leq i<n$ debemos poder encontrar $m_i \in \mathbb{N}$ cumpliendo que $\sigma(i) = \tau(m_i)$ con $\sigma$ y $\tau$ estrictamente crecientes, por lo que:
    \begin{align*}
        \sigma(n) &> \sigma(n-1) > \ldots > \sigma(1) > \sigma(0) \\
        \tau(m_n) &> \tau(m_{n-1}) > \ldots > \tau(m_1) > \tau(m_0)
    \end{align*}
    Donde para la primera cadena de desigualdades hemos usado que $\sigma$ es estrictamente creciente y ahora tenemos que $\tau$ es estrictamente creciente, por lo que:
    \begin{equation*}
        m_n > m_{n-1} > \ldots > m_1 > m_0 \geq 0
    \end{equation*}
    Así, debemos tener forzosamente $m_n\geq n$. En este punto, vemos que:
    \begin{equation*}
        \sigma(n) = \tau(m_n) \geq \tau(n)
    \end{equation*}
    por ser $\tau$ estrictamente creciente. Ahora, si consideramos la aplicación $\delta:\mathbb{N}\to \mathbb{N}$ dada por la correspondencia $n\longmapsto m_n$ ($\delta$ está bien definida por ser $\tau$ estrictamente creciente y por tanto inyectiva) acabamos de probar que $\delta$ es estrictamente creciente, así como que $\tau(\delta(n)) = \tau(m_n) = \sigma(n)$ para cada $n\in \mathbb{N}$ por definición de $\delta$, con lo que:
    \begin{equation*}
...
\begin{teo}[Unicidad]\label{teo:unicidad}
    ...
    \begin{proof}
        ...
        \noindent
        Conviene ahora observar el Lema~\ref{lema:1unicidad} y posteriormente el Lema~\ref{lema:2unicidad}
    \end{proof}
\end{teo}
...
\begin{lema}\label{lema:2unicidad}
    Para todo punto $(t_\ast,x_\ast)\in D$ existe $\cc{U}\subset D$ entorno del mismo y $\exists L>0$ de manera que:
    \begin{equation*}
        \|X(t,x_1) - X(t,x_2)\| \leq L \|x_1-x_2\| \qquad \forall (t,x_1), (t,x_2)\in \cc{U}
    \end{equation*}
    \begin{proof}
         % // TODO: Se seguirá por aquí, por la desigualdad del TVM
    \end{proof}
\end{lema}
