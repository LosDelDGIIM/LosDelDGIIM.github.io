...
Además, si $\sigma,\tau:\mathbb{N}\to \mathbb{N}$ son dos funciones estrictamente crecientes con $\sigma(\mathbb{N})\subset \tau(\mathbb{N})$, tenemos entonces que $\{f_{\sigma(n)}\}$ es una parcial de $\{f_{\tau(n)}\}$ y $\sigma(n)\geq \tau(n)$. % // TODO: Probar
    ...
    \begin{proof} % // TODO: Repasar prueba
        ...
        \begin{enumerate}
            \item Primero buscaremos $D\subset I$ denso y una parcial $\{f_{\sigma(n)}\}$ de forma que $\{f_{\sigma(n)}(t)\}$ es convergente $\forall t\in D$.
                ...
                \end{equation*}
                Con $\{f_{\sigma(n)}\}_{n\geq m}$ parcial de $\{f_{\sigma_m(n)}\}_{n\geq m}$, con lo que es convergente en todo punto de $D$.
            \item Ahora vamos a ver que $\{f_{\sigma(n)}\}$ es uniformemente de Cauchy. Para ello, dado $\varepsilon>0$, buscamos $N\in \mathbb{N}$ de forma que si $n,m\geq N$ obtenemos que:
                \begin{equation*}
