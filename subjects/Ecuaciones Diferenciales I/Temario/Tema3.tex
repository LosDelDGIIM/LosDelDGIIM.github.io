...
\begin{equation}\label{eq:exacta}
    P(x,y)+Q(x,y)y' = 0
\end{equation}
Con $P,Q\in C^1(\Omega)$ con $\Omega\subseteq \mathbb{R}^2$ un conjunto abierto y conexo.

La ecuación se dice que es exacta cuando se verifica la condición de exactitud.
\begin{equation*}
    \dfrac{\partial P}{\partial y} = \dfrac{\partial Q}{\partial x}
\end{equation*}
No son una familia de ecuaciones que se vean directamente. 
Por tanto, cuando veamos un tipo de ecuaciones que parezca no ser de ningún tipo, probablemente sea ecuaciones exactas.\\

Vamos a probar que si tenemos una ecuación exacta y fijamos un punto en el que $Q(x_0,y_0)\neq 0$, entonces siempre vamos a poder encontrar una solución de la ecuación diferencial, definida en un intervalo $I$ abierto.

Sea $(x_0,y_0)\in \Omega$. Supongamos que $Q(x_0,y_0)\neq 0$.
Existe una solución que cumple $y(x_0)=y_0$.

\begin{proof}
    Para demostrarlo, la primera dificultad es que no hemos dicho nada sobre el dominio (notemos que no tiene por qué tener forma de estrella), con lo que la condición de exactitud no nos asegura que tengamos un potencial en $\Omega$.\\

    Sin embargo, lo que sí podemos hacer es localizar, es decir, tomar un conjunto estrellado pequeño dentro de $\Omega$ (donde usamos que $\Omega$ es abierto).

    Sea por tanto $R$ un cuadrado (en los apuntes está hecho con circunferencia) de forma que esté dentro de $\Omega$, con centro en $(x_0,y_0)$.\\

    De esta forma, podemos encontrar un potencial en $R$ (aunque no en todo $\Omega$), dado que es estrellado y se cumple la condición de exactitud, luego $\exists U\in C^2(R)$ de forma que:
    \begin{equation*}
        \dfrac{\partial U}{\partial x} = P \qquad \dfrac{\partial U}{\partial y} = Q \qquad \text{en\ } R
    \end{equation*}
    Ahora, donde pone $P$ y $Q$ podemos poner los parciales, con lo que la ecuación diferencial se queda en una diferencial exacta:
    \begin{equation*}
        P(x,y) + Q(x,y)y' = \dfrac{\partial U}{\partial x}(x,y) + \dfrac{\partial U}{\partial y}y' = \dfrac{d}{dx}\left[U(x,y)\right] = 0 \Longleftrightarrow U(x,y) = c \qquad c\in \mathbb{R}
    \end{equation*}
    Donde usamos un abuso de notación, estamos pensando en $y$ como función de $x$. 
    Como $U(x,y)$ es constante y tenemos que $y(x_0)=y_0$, dicha constante será $c=U(x_0,y_0)$.

    De esta forma, buscamos una función $y$ de forma que cumpla:
    \begin{equation*}
        U(x,y) = U(x_0,y_0) \qquad \text{y} \qquad y(x_0) = y_0
    \end{equation*}
    Y buscamos aplicar el Teorema de la Función Implícita, para obtener $y$ en función de $x$. Sea $F:R\rightarrow\mathbb{R}$
    \begin{equation*}
        F(x,y) = U(x,y) - U(x_0,y_0)
    \end{equation*}
    es una función de clase $F\in C^2(\mathbb{R})$, que cumple que $F(x_0,y_0)= 0$ y:
    \begin{equation*}
        \dfrac{\partial F}{\partial y}(x_0,y_0) =\dfrac{\partial U}{\partial y}= Q(x_0,y_0) \neq 0
    \end{equation*}
    Por tanto, sabemos que existe una función $y:I\rightarrow\mathbb{R}$ en un intervalo abierto $I$ de forma que:
    \begin{gather*}
        y(x_0) = y_0 \\
        F(x,y(x)) = 0
    \end{gather*}
...
Notemos que la condición $Q(x,y)\neq 0$ significa que podemos despejar $y'$, de forma que pongamos la ecuación en forma normal:
\begin{equation*}
    y' = \dfrac{-P(x,y)}{Q(x,y)}
\end{equation*}
En la práctica, no es difícil resolver ecuaciones diferenciales exactas, sino averiguar que sean exactas. Lo difícil es detectarlas. Una vez que las detectamos, es fácil resolverla, integrando para buscar el potencial, buscar la condición inicial y aplicar función implícita.

La recomendacion es que si vemos una ecuación que no es de ningún tipo, sospechemos que es exacta.
...
\begin{ejemplo}
    ...
    \begin{gather*}
        ...
        Q(x,y) = 5y^4 + 2xy
    \end{gather*}
    $P,Q\in C^2(\mathbb{R}^2)$, veamos si cumplen la condición de exactitud:
    \begin{equation*}
        \dfrac{\partial P}{\partial y} = 2y = \dfrac{\partial Q}{\partial x}
    \end{equation*}
    Por tanto, la ecuación~(\ref{eq:exacta_ejm}) es una ecuación exacta.
    \begin{equation*}
        Q(0,3) = 405 \neq 0
    \end{equation*}
    Ahora:
    \begin{equation*}
        \dfrac{\partial U}{\partial x} = y^2 + 2x
    \end{equation*}
    \begin{equation*}
        U(x,y) = \int (y^2 + 2x)~dx  = y^2 x + x^2 + \varphi(y)
    \end{equation*}
    \begin{gather*}
        \dfrac{\partial U}{\partial y} = 5y^4 + 2xy \\
        \dfrac{\partial U}{\partial y} = 2xy + \varphi'(y)
    \end{gather*}
    con lo que $\varphi'(y) = 5y^4$ y podemos tomar, por ejemplo, $\varphi(y)=y^5$.

    Tenemos finalmente:
    \begin{equation*}
        U(x,y) = y^2x+x^2+y^5 \qquad (x,y)\in \mathbb{R}^2
    \end{equation*}
    Y sabemos que la ecuación~(\ref{eq:exacta_ejm}) podemos ponerla como la derivada de dicha expresión en $x$ igualada a 0. Por tanto, sabemos que las soluciones se pueden escribir como:
    \begin{equation*}
        U(x,y) = c 
    \end{equation*}
    Sin embargo, como no queremos todas las soluciones, sino la que en $0$ vale $3$:
    \begin{equation*}
        U(x,y) = y^2x + x^2+y^5 = U(x_0,y_0) =  243
    \end{equation*}
    Y como $Q(0,3)\neq 0$, no hace falta aplicar el Teorema de la Función Implícita, porque acabamos de ver teóricamente que sí nos da una función $y$:
    \begin{equation*}
...
\end{ejemplo}
Como hemos visto, lo más difícil es darnos cuenta de que la ecuación diferencial que nos solicitan sea o no exacta.\\

% // TODO: Quizas meter en la siguiente seccion

Es una idea totalmente distinta del cambio de variable y toca bastantes otros contextos, aunque desde el punto de vista de las ecuaciones diferenciales es muy pobre (ya que hay muy pocas ecuaciones diferenciales que sean exactas).

La gracia está en que esto de las ecuaciones exactas se puede llevar más lejos usando el factor integrante, ya que puede que nos den una ecuación que no sea exacta y que si la multiplicamos por un número, la convertimos en exacta.

Resulta que este método es aplicable a todas las ecuaciones diferenciales:
Todas las ecuaciones se pueden multiplicar por algo que hace que se pueda expreasr como la derivada de algo igualada a 0.

\section{Factor integante}
En la notación clásica, integrar una ecuación diferencial es resolverla, con lo que el factor queda resuelto.\\

Veamos primero ejemplos que nos motiven el factor integrante:
\begin{ejemplo}
    Dada la ecuación:
    ...
    \end{equation*}
    Y resulta que~(\ref{eq:factori_ejm}) no es una ecuación exacta, ya que:
    \begin{equation*}
        \dfrac{\partial P}{\partial y} = 1 \neq 0 = \dfrac{\partial Q}{\partial x}
    \end{equation*}
    Sin embargo, hay varios métodos que hacen que sí sea exacta:
    \begin{equation*}
        e^{x}y + e^{x}y' = 0
    \end{equation*}
    Que ya sí es exacta, ya que:
    \begin{equation*}
        e^{x}y + e^{x}y' = \dfrac{d}{dx}(e^{x}y) = 0
    \end{equation*}
    Por tanto, $e^x$ es un factor integrante para~(\ref{eq:factori_ejm}).
    
    Sin embargo, una ecuación diferencial no tiene un único factor integrante, sino que tiene muchos. Si ahora multiplicamos por $\frac{1}{y}$:
    \begin{equation*}
        1+\dfrac{y'}{y} = 0
    \end{equation*}
    Y vuelve a ser una ecuación exacta:
    \begin{equation*}
        1+\dfrac{y'}{y} = \dfrac{d}{dx}(x+\ln y) = 0 \qquad y>0
    \end{equation*}
    (Tomamos $\ln y$ o $\ln(-y)$ dependiendo de dónde se mueva la $y$)

    El peligro de los factores integrantes que en algunos sitios se hace 0 o tiene singularidades, en dichos sitios perdemos a la ecuación diferencial.
    Por tanto, nos interesarán factores integrantes con mayor dominio de definición.
\end{ejemplo}

\begin{definicion}[Factor integante]
    Dada una ecuación diferencial definida en un conjunto $D$. Sea $\Omega\subseteq D$, un factor integante es una función $\mu\in C^1(\Omega)$ de forma que:
    \begin{enumerate}[label=\arabic*)]
        \item
            \begin{equation}\label{eq:condicion_fi}
                \dfrac{\partial }{\partial y}(\mu P) = \dfrac{\partial }{\partial x}(\mu Q) \qquad \text{en\ } \Omega
            \end{equation}
            Es decir, que si tenemos una ecuación de la forma~(\ref{eq:exacta_ejm}) y la multiplicamos por $\mu$, entonces obtenemos una ecuación exacta.
...
\begin{observacion}
    Notemos que si tenemos una ecuación diferencial exacta, factores integrantes de la misma son:
    \begin{equation*}
        \mu_c(x,y) = c \qquad c\neq 0, \quad (x,y)\in \Omega
    \end{equation*}
\end{observacion}
La idea para buscar un factor integante es ver la ecuación~(\ref{eq:condicion_fi}) como una ecuación en derivadas parciales, o también, que este cumple (donde los subíndices son las parciales). Usando la fórmula de la ecuación de la derivación del producto:
\begin{align*}
...
% \end{gather*}
Con lo que obtenemos una ecuacion en derivadas parciales del factor integrante.

Las ecuaciones en derivadas parciales tienen bastantes soluciones. Dependen de infinitas constantes, con lo que sus espacios tienen dimensión infinita.
La gracia es que no necesitamos todas, sólamente una, con lo que buscar todas puede ser difícil pero encontrar una puede llegar a ser fácil (notemos que no nos sirve $\mu = 0$).\\

Buscamos ahora el truco para buscar el factor integrante.
Buscamos funciones $\mu$ dependientes de dos variables, pero buscaremos que esta sea una función solo de una variable:
\begin{equation*}
    ...
    \end{array}\right.
\end{equation*}
Nota: para los problemas difíciles, buscar un factor integante dependiente de $x^2+y^2$. El origen de esto está en que muchas ecuaciones diferenciales provienen del siglo XIX de problemas geométricos, donde dichos problemas han sido olvidados.\\

Lo que hay que entender es el papel que juegan $\mu$ y $m$ (en la notación clásica, llaman también $\mu$ a $m$).

% // TODO: Hacer esto bien
\begin{ejemplo}
    Por ejemplo, cuando teníamos el factor integrante
    \begin{equation*}
        \mu(x,y) = e^x
    \end{equation*}
    Lo que teníamos era la función
    \begin{equation*}
        m(\xi) = e^\xi
    \end{equation*}
    Con lo que usamos la función:
    \begin{gather*}
        \mathbb{R}^2 \rightarrow\mathbb{R}\rightarrow\mathbb{R}\\
        (x,y) \longmapsto x \\
        \qquad \xi \longmapsto e^\xi
    \end{gather*}
    ...
    \begin{gather*}
        \mathbb{R}^2\rightarrow\mathbb{R}\rightarrow\mathbb{R} \\
        (x,y)\longmapsto x+y \longmapsto \sen(x+y)
    \end{gather*}
    ...
    \begin{gather*}
        \mathbb{R}^2 \rightarrow\mathbb{R}\rightarrow\mathbb{R} \\
        (x,y) \longmapsto x^2+y^2 \longmapsto \dfrac{1}{x^2+y^2}
    \end{gather*}
\end{ejemplo}

Vamos a hacer un par de factores integantes y vamos a pillar la mecánica de esto.

\begin{ejemplo}
        Factor integante que solo depende de $x$: $\mu(x,y)=m(x)$.
        Sustituimos y:
        \begin{equation*}
            \mu_y P-\mu_x Q = \mu(Q_x - P_y)
        \end{equation*}
        ...
        y obtenemos (suponiendo que $m\neq 0$, condición que obtenemos si $\mu$ será un factor integante y que $Q\neq 0$ en $\Omega$):
        \begin{gather*}
            -m'(x)Q = m(x) (Q_x -P_y) \\
            \dfrac{m'(x)}{m(x)} = \dfrac{P_y - Q_x}{Q}
        \end{gather*}
        Y lo que vemos es que no cualquier ecuación va a poder tener un factor integante de este tipo, ya que muchas veces tendremos a la izquierda una función de $x$ y a la derecha una función de dos variables:
        \begin{equation*}
            \dfrac{m'(x)}{m(x)} = \dfrac{e^{y+x}}{x^3}
        \end{equation*}
        No es posible para todas las ecuaciones diferenciales. Sin embargo, sí que será para una clase de ecuaciones diferenciales, para las cuales el cociente
        \begin{equation*}
            \dfrac{P_y-Q_x}{Q}
        \end{equation*}
        Dependa solo de $x$, con lo que tendríamos:
        \begin{equation*}
            \dfrac{m'(x)}{m(x)} = f(x)
        \end{equation*}
        Que es una ecuación lineal homogénea, de soluciones:
        \begin{equation*}
        ...
        De esta forma:
        \begin{equation*}
            \exists \text{factor integante\ } \mu(x,y) = m(x) \Longleftrightarrow \dfrac{P_y-Q_x}{Q} = f(x)
        \end{equation*}
        En cuyo caso, el factor integrante será:
        \begin{equation*}
            m(\xi) = e^{F(\xi)} \qquad F' = f
            \end{equation*}
\end{ejemplo}
...
\begin{ejemplo}
    ...
    De esta forma, tenemos funciones:
    \begin{gather*}
        P(t,x) = b(t) + a(t)x \\
        Q(t,x) = -1
    \end{gather*}
    ...
    \begin{equation*}
        \dfrac{\partial P}{\partial x}(t,x) = \dfrac{\partial Q}{\partial t}(t,x) \qquad \forall (t,x)\in \mathbb{R}^2
    \end{equation*}
    Veremos que esta ecuación no es exacta, pero que acepta un factor $\mu(t,x) = m(t)$. Esto pasrá si:
    \begin{equation*}
        \dfrac{P_x-Q_t}{Q} = f(t)
    \end{equation*}
    ...
    \begin{equation*}
        \dfrac{a(t)}{-1} = f(t)
    \end{equation*}
    Obtenemos un cociente que es función de $t$, con lo que sabemos que existe el factor integrante $\mu$, que sólo depende de $t$:
    \begin{equation*}
        m(\xi) = e^{-A(\xi)} \qquad A' = a
    \end{equation*}
    De esta forma, podemos multiplicar la ecuación por $\mu$, obteniendo una ecuación exacta:
    \begin{equation*}
        b(t)e^{-A(t)} - \dfrac{d}{dt}\left(e^{-A(t)}x\right) = 0
    \end{equation*}
\end{ejemplo}

Veamos ahora el factor integante del tipo $\mu(x,y)=x^2+y^2$:
...
\end{gather*}
\begin{equation*}
    \my_y P - \mu_x Q = \mu (Q_x - P_y) 
\end{equation*} 
Con lo que:
\begin{equation*}
    \left[2yP(x,y)-2xQ(x,y)\right]m'(x^2+y^2) = m(x^2+y^2)(Q_x(x,y)-P_y(x,y))
\end{equation*}
Habrá un factor integante cuando podamos encontra una $m$ que cumpla esto. Para verlo más claro, movemos a un lado lo que depende de $m$ y a otro lo que depende de $P$ y $Q$ (abreviando notacion):
\begin{equation*}
    \dfrac{Q_x-P_y}{2yP-2xQ} = \dfrac{m'(x^2+y^2)}{m(x^2+y^2)}
\end{equation*}
Para que esto pase, $m$ tiene que ser distinto de $0$ (algo que sucederá, ya que buscamos que sea factor integante), así como que $2yP-2xQ \neq 0$, lo que a partir de ahora supondremos como hipótesis:
\begin{equation*}
...
\begin{equation*}
    \exists \text{factor integante\ } \mu(x,y) = m(x^2+y^2) \Longleftrightarrow \dfrac{Q_x-P_y}{2yP-2xQ} = f(x^2+y^2)
\end{equation*}
En caso afirmativo:
\begin{equation*}
...
\begin{ejemplo}
    ...
    \begin{enumerate}
        \item En $\Omega = \mathbb{R}^2\setminus\{(0,0)\}$
            ...
            \end{equation*}
            Función de $x^2+y^2$, con lo que admite un factor integante en función de $x^2+y^2$:
            \begin{equation*}
                f(\xi) = \dfrac{-1}{\xi}
            \end{equation*}
            \begin{equation*}
            ...
            \end{equation*}
            Con lo que el factor integante vale:
            \begin{equation*}
                \mu(x,y) = \dfrac{1}{x^2+y^2}
            \end{equation*}
        \item Con la otra
    ...
    \end{enumerate}
    Por tanto, si una ecuacion diferencial adminte un factor integante, la ecuacion diferencial cuyas soluciones son las inversas de la original tienen el mismo factor integante.
