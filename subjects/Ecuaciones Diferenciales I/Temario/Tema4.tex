...
\begin{ejemplo}
    ...
    \begin{equation*}
        Z = \{k_1 \cosh t + k_2 \senh t \mid k_1,k_2 \in \mathbb{R}\}
    \end{equation*}
    ...
    \begin{equation*}
        \Phi_1(k_1\psi_1 + k_2\psi_2) = CALCULAR \qquad \forall k_1,k_2\in \mathbb{R}
    \end{equation*}
\end{ejemplo}

% // TODO: Definir determinante de k vectores
\begin{definicion}
\end{definicion}
...
\begin{prop}
    ...
    \begin{proof}
        ...
        \begin{description}
            ...
            \item [$i) \Rightarrow ii)$] Vamos a tener en cuenta que si tenemos un isomorfismo entre dos espacios vectoriales $\Phi:V\rightarrow W$, entonces si tenemos una base $\{v_1,\ldots,v_k\}$ de $V$, entonces $\{\Phi(v_1),\ldots,\Phi(v_k)\}$ forma una base de $W$.\\

                Además, si tenemos $v_1,\ldots,v_k\in \mathbb{R}^k$, estos vectores formarán una base si y solo si el $\det(v_1|v_2|\ldots|v_k) \neq 0$.\\

                Fijado $t_0\in I$, tenemos $\Phi_{t_0}:Z\rightarrow\mathbb{R}^k$ dado por el Teorema~\ref{teo:dim}, por lo que sabemos que $\{\varphi_1,\ldots,\varphi_k\}$ son base de $Z$ si y solo si $\{\Phi_{t_0}(\varphi_1),\ldots,\Phi_{t_0}(\varphi_k)\}$ es una base de $\mathbb{R}^k$, si y solo si:
                \begin{equation*}
                    det(\Phi_{t_0}(\varphi_1) | \ldots| \Phi_{t_0}(\varphi_k)) = W(\varphi_1,\ldots,\varphi_k) \neq 0
                \end{equation*}
                Como $t_0$ era arbitrario, esto funciona para cada $t\in I$.
        \end{description}
    \end{proof}
\end{prop}
...
\begin{ejemplo}
    ...
    Probar que son soluciones de una ec lineal homogenea de segundo orden.
\end{ejemplo}
...
Dada una ecuación diferencial lineal homogénea de la forma~(\ref{eq:linealsup_homogenea}). Supongamos que tenemos $\varphi_1,\ldots,\varphi_k\in Z$, y tomamos $t,t_0\in I$. Entonces:
\begin{equation*}
    W(\varphi_1,\ldots,\varphi_k)(t) = W(\varphi_1,\ldots,\varphi_k)(t_0) \cdot e^{\displaystyle-\int_{t_0}^{t} a_{k-1}(s)~ds }
\end{equation*}
Notemos que nos da la equivalencia entre $ii)$ y $iii)$ en la última Proposición.

\begin{proof}
    Método de Cauchy, probar una identidad entre funciones construyendo una ecuación diferencial de la cual ambas son solución y comprobamos que cumplen la misma condición inicial, luego son la misma por el Teorema~\ref{teo:existencia_unicidad}.\\

    Antes, hay que ver cómo se deriva un determinante.
\end{proof}
\end{teo}

Supongamos que tenemos $f_{ij}:I\rightarrow\mathbb{R}$ funciones con $1\leq i,j\leq k$ derivables, definimos $D:I\rightarrow\mathbb{R}$ dada por
\begin{equation*}
    D(t) = det(f_{ij}(t)) \qquad t\in I
\end{equation*}
...
\begin{equation*}
    ...
    \end{array}\right|
\end{equation*}
El teorema también es cierto derivando columnas, que es equivalente porque $det(A) = det(A^t)$.
Nos recomienda probar con 2x2.\\

Recordamos la definicion de determinante, a partir del grupo de permutaciones:
\begin{definicion}[Grupo de permutaciones]
    \begin{equation*}
        S_2 = \{(1,2),(2,1)\}
    \end{equation*}
\end{definicion}
...
$S_n$ es el conjunto de aplicaciones biyectivas con $n$ elementos.\\
$|S_n| = n!$.
Esto da un grupo.

Las permutaciones se dividen en:
\begin{itemize}
    \item Pares, me las llevo a la identidad dando un número par de saltos.
    \item Impares, me las llevo a la identidad dando un número par de saltos.
\end{itemize}
Un salto es mover dos piezas de uno en uno, grupos alternados

\begin{definicion}
    Signatura de uan permutación, que es 1 (par) o -1, denotada con $\veps$.
\end{definicion}

A partir de la regla de Sarrous:
\begin{definicion}
    \begin{equation*}
        A = (a_{ij})_{1\leq i,j\leq k}
    \end{equation*}
    \begin{equation*}
        \det A = \sum_{\sigma\in S_k} \veps(\sigma) a_{1\sigma(1)} a_{2\sigma(2)} \ldots a_{k\sigma(k)}
    \end{equation*}
\end{definicion}
Luego hay que derivar un producto.
