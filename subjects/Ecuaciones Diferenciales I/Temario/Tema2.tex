...
\begin{tikzpicture}
    ...
    % Recta y = -x
    \draw[thick, dashed] (-4,4) -- (0,0) node {};
...
\begin{tikzpicture}
    ...
    % Recta y = -1
    \draw[thick, dashed] (0,-1) -- (-5,-1) node {};
...
Tenemos por tanto una ecuación diferencial en forma normal dada por:
\begin{gather*}
    f:D=J\times \mathbb{R} \rightarrow \mathbb{R} \\
    f(t,x) = a(t)x^2+b(t)x+c(t)
\end{gather*}

A diferencia de la ecuación lineal, esta ecuación no puede resolverse.\\

Sin embargo, si se sabe una solución para dicha ecuación, entonces se saben todas las demás.
Suponemos, por tanto, que conocemos una solución $\phi(t)$ explícita definida en un cierto intervalo abierto $I \subseteq J$.\\

Se resuelve mediante un cambio
\begin{equation*}
    \left\{\begin{array}{rl}
            s &= t \\
            y &= \dfrac{1}{x-\phi(x)}
    \end{array}\right.
\end{equation*}
Por tanto, en nuestro dominio tendremos que quitar la recta $x-\phi(x) = 0$. Es decir, nos quitamos la solución que ya conocemos. Por tanto, al resolver una ecuación de Riccati, hay que añadir la solución que ya nos dan al final.

% // TODO: Graficar
Notemos además que esta solución (graficar como $x^2+10$) nos divide el dominio en dos posibles dominios:
\begin{gather*}
    D_+ = \{(t,x)\in \mathbb{R}^2 \mid t\in I, x> \phi(t)\} \\
    D_- = \{(t,x)\in \mathbb{R}^2 \mid t\in I, x< \phi(t)\} \\
\end{gather*}

Tenemos como codominio de $\phi$: 
\begin{itemize}
    \item $\mathcal{D}_+ = I\times \mathbb{R}^+$ si elegimos el dominio $D_+$, ya que si $x>\phi(x)$, entonces $x-\phi(x)> 0 \Longrightarrow \frac{1}{x-\phi(x)}>0$
    \item $\mathcal{D}_- = I\times \mathbb{R}^-$ si elegimos el dominio $D_-$, ya que si $x<\phi(x)$, entonces $x-\phi(x)< 0 \Longrightarrow \frac{1}{x-\phi(x)}<0$.
\end{itemize}
Además, tenemos que $\phi(t)$ se va a infinito y que el infinito se va a al 0.\\

Veamos que $\varphi$ es un difeomorfismo:
\begin{itemize}
    \item En primer lugar, si miramos sus componentes:
        \begin{equation*}
            \varphi(t,x) = \left(t,\dfrac{1}{x-\phi(t)}\right) = (s,y)
        \end{equation*}
        Como $\phi$ era solución de l ecuación diferencial, entonces es de clase $C^1$. Por tanto, las dos componentes de $\varphi$ son de clase $C^1$, luego $\varphi$ es de clase $C^1$.
    \item Buscamos ahora la inversa:
        \begin{equation*}
            t = s \qquad x = \dfrac{1}{y}+\phi(t)
        \end{equation*}
        \begin{equation*}
...
Preferimos no derivar el codiente, luego derivamos:
\begin{equation*}
    x = \dfrac{1}{y} + \phi
\end{equation*}
\begin{equation*}
    x' = \dfrac{-y'}{y^2} + \phi'
\end{equation*}
Además, como $\phi$ era solución de Riccati:
\begin{align*}
    x' &= a{(\left(\dfrac{1}{y}+\phi\right))}^{2} + b{\left(\dfrac{1}{y}+\phi\right)}^{2} + c\\
       &= \dfrac{a}{y^2} + \dfrac{2a\phi}{y} + a\phi^2 + \dfrac{b}{y} + b\phi + c
\end{align*}
Volvemos a usar que $\phi$ era solución y:
\begin{equation*}
    \phi' = a\phi^2 + b\phi + c
\end{equation*}

\begin{equation*}
    \dfrac{-y'}{y^2} = \dfrac{a}{y^2} + \dfrac{2a\phi + b}{y}
\end{equation*}
Multiplicando por $y^2$:
\begin{equation*}
    y' = -(2a(t)\phi(t)+b(t)) y - a(t)
\end{equation*}
Y llegamos a una ecuación lineal, la cual tenemos que resolver bien en $\mathcal{D}_+$ o bien en $\mathcal{D}_-$. Puede pasar por tanto que alguna solución corte $y=0$, por lo que tengamos que tratarla como dos soluciones, una de una ecuación diferencial y otra de otra.

Además, tenemos que agregar al conjunto final de soluciones la solución inical de Riccati, $\phi$.
...
\begin{ejemplo}
    ...
    A simple vista, observamos que una solución particular es $x(t) = 0$. Se deja como ejercicio.\\

    En este ejemplo, tomaremos como solución:
    \begin{equation*}
        \phi(t) = \dfrac{-1}{t}
    \end{equation*}
    Que definiremos en el intervalo $I = \left]-\infty, 0\right[$.\\

    Teníamos por tanto $J=\mathbb{R}$ y ahora $I=\mathbb{R}^-$.\\
    ...
    \begin{gather*}
        \mathcal{D}_+ = \mathbb{R}^- \times \mathbb{R}^+\\
        \mathcal{D}_- = \mathbb{R}^- \times \mathbb{R}^-
    \end{gather*}
    Derivamos pensando que tanto $x$ como $y$ dependen de $t$:
    \begin{equation*}
    ...
    \begin{equation*}
        y' = \dfrac{2}{t}y-1
    \end{equation*}
    Y llegamos a una ecuación lineal que pasamos a resolver, mediante otro cambio de variable:
    \begin{equation*}
        z = l(t)y
    \end{equation*}
    ...
    \begin{equation*}
        l(t) z = y
    \end{equation*}
    para distinta función $l$. Derivamos:
    \begin{gather*}
        y' = l' z + lz' \\
        y' = \dfrac{2}{t}lz - 1
    \end{gather*}
    Y tenemos $lz' = \left(\dfrac{2l}{t}-l'\right)z-1$. Buscamos que el paréntesis sea 0:
    \begin{equation*}
        l' = \dfrac{2l}{t}
    \end{equation*}
    Y cogemos como solución:
    \begin{equation*}
        l(t) = e^{2\ln(-t)} = e^{\ln(t^2)} = t^2 \qquad t\in \mathbb{R}
    \end{equation*}
    Luego:
    \begin{equation*}
        t^2 z' = -1 \Longrightarrow z' = \dfrac{-1}{t^2}
    \end{equation*}
    Luego tenemos como soluciones:
    \begin{equation*}
        z(t) = \dfrac{1}{t} + k \qquad k\in \mathbb{R} \quad t\in \mathbb{R}
    \end{equation*}
    Deshaciendo un cambio de variable:
    \begin{equation*}
...
\section{Relación con Teoría de Grupos}

% // TODO: Definir grupos otra vez

Con objetivo a dar sentido teórico a los cambios de variable.

Obra de Sophus Lie, hay una teoría en álgebra que a las ecuaciones polinómicas se les asocia un grupo. Lie hizo lo mismo con las ecuaciones diferenciales, lo que permite explicar qué cambio de variable hay que hacer para resolverlas. Conectar los grupos con la solubilidad de las ecuaciones diferenciales.\\

Los grupos tienen la misión de detectar simetrías. Si tenemos un cuadrado con vértices numerados y lo rotamos 45º sobre el centro, es otro cuadrado. Si lo rotamos 90º, tenemos el mismo cuadrado pero con los vértices cambiados de posición.

Hay una serie de rotaciones especiales que hace que el cuadrado sea invariante. El conjunto de rotaciones que deja invariante un cuadrado forma un grupo.
Este es similar a $\{1, -1, i, -i\}$.
Cada figura simétrica tiene un grupo cíclico asociado que las dejan invariantes. Grupo de simetrías.

Podemos hacer una analogía entre las figuras y las ecuaciones diferenciales y los giros y los cambios de variable. De esta forma, buscamos los cambios de variable que nos dejan las ecuaciones diferenciales invariantes (puede suceder que una solución vaya en otra).\\

La idea es, que dada una ecuación diferencial
\begin{equation}\label{eq:relacion_grupos}
    x' = f(t,x)
\end{equation}
definida en un dominio $D$ dentro del plano $t$, $x$. Buscaremos un cambio de variable 
\begin{equation*}
    \varphi: \left\{\begin{array}{rl}
        s = \varphi_1(t,x) \\
        y = \varphi_2(t,x) 
...
\begin{ejemplo}
    ...
    \end{equation*}
    Le haremos el cambio de variable:
    \begin{equation*}
        \varphi: \left\{\begin{array}{rl}
                s &= -t \\
                y^3 + y &= x^3+x+8
        \end{array}\right.
    \end{equation*}
    Daremos por ahora por bueno que es efectivamente un cambio de variable (que $y$ viene dada de forma implícita por dicha ecuación).\\

    Vemos en primer lugar que la ecuación diferencial está definida en $D = \mathbb{R}^2$. Por ahora, suponemos que $\varphi:\mathbb{R}^2\rightarrow\mathbb{R}^2$ es un difeomorfismo admisible para dicha ecuación diferencial.

    Veamos que tras aplicar el cambio de variable obtenemos la misma ecuación:
    \begin{equation*}
    ...
    \begin{enumerate}
        \item Veamos primero que las ecuaciones 
            ...
            \begin{equation*}
                \sigma'(\xi) = 3\xi^2 + 1 > 0
            \end{equation*}
            Luego es estrictamente creciente, luego inyectiva.\\

            Notemos además que es sobreyectiva, ya que
            \begin{equation*}
                \lim_{\xi\to\pm\infty}\sigma(\xi) = \pm \infty
            \end{equation*}
            Y como $\sigma$ es una función continua, por el Teorema de Bolzano tenemos que tiene que tomar todos los valores, luego $\sigma$ es sobreyectiva. En definitiva, $\sigma$ es biyectiva y además tiene inversa, la cual es derivable, gracias al Teorema de la Función Inversa.
...
Lo que vamos a hacer ahora es considerar todos los cambios de variable que dejan a una ecuación diferencial invariante, y veremos que es un grupo.\\

Sea $Diff(\mathbb{R}^2)$ el conjunto de todos los difeomorfismos de $\mathbb{R}^2$:
\begin{equation*}
    Diff(\mathbb{R}^2) = \{\varphi:\mathbb{R}^2\rightarrow\mathbb{R}^2 \mid \varphi \text{\ difeomorfismo}\}
\end{equation*}
Tomaremos como operación interna en dicho conjunto la composición de funciones, lo cual es natural. Recordemos que la composición de funciones no es conmutativa.\\

Para ver que $Diff(\mathbb{R}^2)$ sea un grupo, buscamos ver que la composición sea una operación interna (es decir, que es cerrada para dicho conjunto).
\begin{itemize}
    \item La composición de funciones de clase $C^1$ es de clase $C^1$.
    \item La composición de funciones biyectivas resulta en una función biyectiva.
    \item Dados dos difeomorfismos, si tomamos el inverso de la composición:
        \begin{equation*}
...
\begin{equation*}
    \Phi, \phi \in Diff(\mathbb{R}^2) \Longrightarrow \Phi \circ \phi \in Diff(\mathbb{R}^2)
\end{equation*}
Además, tenemos que:
\begin{enumerate}
    \item La composición es asociativa\footnote{Visto en Álgebra I.}.
