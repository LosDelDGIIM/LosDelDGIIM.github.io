...
\begin{tikzpicture}
    % Ejes coordenados
    \draw[->] (-5,0) -- (1,0) node[right] {$u$};
    \draw[->] (0,-1) -- (0,3) node[above] {$v$};
...
\begin{tikzpicture}
    % Dibujar los ejes
    \draw[->] (5,0) -- (-1,0) node[left] {$t$};
    \draw[->] (0,-4) -- (0,1) node[above] {$x$};
...
\end{equation*}
Que es una ecuación homogénea, por ser cociente de polinomios homogéneos de grado 1. Obtenemos la función $h$:
\begin{equation*}
...
% // TODO: Clase de 15-10
% // TODO: Cambiar subsections por sections
...
para ciertas funciones $a,b:J\rightarrow\mathbb{R}$ continuas en un intervalo abierto $J$.\\
...
\end{equation*}
Que se trata de una banda vertical.

\begin{ejemplo}
    El dominio de la ecuación diferencial
    \begin{equation*}
        x' = \sqrt{1-t^2}x + \ln t
    \end{equation*}
    es $D = \left]0,1\right[\times \mathbb{R}$
\end{ejemplo}
Las ecuaciones lineales pueden clasificarse en dos tipos:
\begin{itemize}
    \item Si $b=0$, entonces estamos ante una ecuación lineal homogénea (no tiene nada que ver con las ecuaciones homogéneas).
    \item Si $b$ no es la función constantemente igual a cero, decimos que es una ecuación lineal completa.
\end{itemize}
Observemos que $x' = \frac{x}{t}$ es de variables separadas, homogenea y reducible a homogenea.

\subsubsection{Ecuaciones lineales homogeneas}
Las ecuaciones de la forma
\begin{equation*}
    x' = a(t) x
\end{equation*}
Sabemos ya resolverlas, son ecuaciones de variables separadas.

Las soluciones constantes son $x(t) = 0 \forall t\in I$.
...
\begin{equation*}
    x(t) = e^c e^{A(t)}
\end{equation*}
Si $c$ es una constante, tenemos que $e^c$ es una constante positiva.
...
\begin{equation*}
    x(t) = -e^c e^{A(t)}
\end{equation*}
Si $c$ es una constante, tenemos que $-e^c$ es uan constante negativa.

Por tanto, dada una ecuacion de la forma
\begin{equation*}
    x' = a(t)x
\end{equation*}
Entonces, sus soluciones son de la forma:
\begin{equation*}
    x(t) = k\cdot e^{A(t)} \qquad k\in \mathbb{R}
\end{equation*}
con
\begin{equation*}
    A(t) = \int_{t_0}^{t} a(s)~ds 
...
\begin{ejemplo}
    Resolvamos
    \begin{equation*}
        x' = \dfrac{x}{t} \qquad D_- = \left]-\infty,0\right[ \times \mathbb{R}
    \end{equation*}
    Tenemos por tanto el semiplano en el que $t<0$.\\
    ...
    \begin{equation*}
        x(t) = k\cdot e^{\ln(-t)} = -kt \qquad k\in \mathbb{R}, \quad t\in \mathbb{R}^-
    \end{equation*}
\end{ejemplo}

\subsubsection{Ecuaciones lineales completas}
Son de la forma
\begin{equation*}
    x' = a(t)x + b(t)
\end{equation*}
de forma que la función $b$ no es constantemente igual a 0. Probaremos con el cambio de variable:
...
\begin{equation*}
    ...
    \end{array}\right.
\end{equation*}
Con $l\in C^1(J)$ y $l(t)\neq 0$ $\forall t\in J$.

Lo primero es ver el cambio de variable $\varphi:D\rightarrow D$, ya que $D=J\times \mathbb{R}$, por lo que lleva la banda vertical en la misma.
\begin{itemize}
    \item $\varphi$ es de clase $C^1$, ya que sus componentes son de clase $C^1$.
    \item Tiene inversa, que es:
        \begin{equation*}
            \left\{\begin{array}{rl}
                    t &= s \\
                    x &= \dfrac{y}{l(s)}
            \end{array}\right.
        \end{equation*}
        Que podemos hacerlo porque exigimos anteriormente que $l(t)\neq 0$ $\forall t\in J$.
    \item Que también es de clase $C^1$, luego tenemos un difeomorfismo.
\end{itemize}
Resulta un cambio de variable admisible, ya que no se cambia la variable $t$.\\

A continuación, aplicaremos dicha familia de cambios de variable a la ecuación del inicio:
...
\begin{equation*}
    ...
    \end{array}\right.
\end{equation*}
entendiendo que todo depende de $t$:
\begin{equation*}
    y' = l' x + lx' = l'x + l(ax+b)
\end{equation*}
sustituimos $x$:
...
\end{equation*}
Y obtenemos otra ecuación lineal:
\begin{equation*}
...
\begin{equation*}
    l' = -a(t) l
\end{equation*}
Que es una ecuación lineal homogénea. Si escogemos $l(t) = e^{-A(t)}$, tenemos una solución de dicha ecuación.\\
...
\end{equation*}
Una vez obtenida $y$, sacamos las soluciones de las ecuaciones lineales completas:
\begin{equation*}
...
\begin{ejemplo}
    ...
    \begin{equation*}
        x' = \dfrac{x}{t} + 1
    \end{equation*}
    Podemos tomar como dominio $t>0$ o $t<0$. Tomemos $t<0$, con dominio $D=\mathbb{R}^-\times \mathbb{R}$.
    ...
    \end{equation*}
    \begin{equation*}
        y(t) = k - \ln(-t) \qquad k\in \mathbb{R}
    \end{equation*}
    Luego:
    \begin{equation*}
...
Riccati tuvo la ocurrecia de decir que una ecuación lineal es un polinomio de primer grado en $x$, luego trató de resolver un polinomio de segundo grado en $x$:
\begin{equation*}
    x' = a(t) x^2+b(t)x+c(t+b(t)x+c(t))
\end{equation*}
