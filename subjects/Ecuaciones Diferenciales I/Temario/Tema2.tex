...
\begin{tikzpicture}
    ...
    \def\alph{0.4}   % Pendiente de la recta x = alpha * t
    \def\bet{0.8}    % Pendiente de la recta x = beta * t
    
    % Recta x = \alpha t (tanto para t > 0 como t < 0)
    \draw[thick] (-4,\alph*-4) -- (4,\alph*4) node[above right] {$x = \alpha t$};

    % Recta x = \beta t (tanto para t > 0 como t < 0)
    \draw[thick] (-4,\bet*-4) -- (4,\bet*4) node[above right] {$x = \beta t$};
    
    % Relleno del sector angular entre las rectas (t > 0)
    \fill[blue!20, opacity=0.5] (0,0) -- (4,\alph*4) -- (4,\bet*4) -- cycle;
    
    % Relleno del sector angular entre las rectas (t < 0)
    \fill[green!20, opacity=0.5] (0,0) -- (-4,\alph*-4) -- (-4,\bet*-4) -- cycle;
...
La ecuación diferencial estará, por tanto, definida en un conjunto abierto y conexo $D=J\times \mathbb{R}$ y vendrá dada por la función
\Func{f}{D}{\mathbb{R}}{(t,x)}{a (t) x^2 +  b (t) x+c (t)}
...
\begin{tikzpicture}
    ...
    \draw[thick, domain=1:3, smooth, variable=\x, black] plot ({\x}, {(\x-2)^2+1});

    % Color para la zona por encima de la parábola
    \fill[blue!20, domain=1:3, smooth, variable=\x]
        (1,4) -- plot ({\x}, {(\x-2)^2+1}) -- (3,4) -- cycle;

    % Color para la zona por debajo de la parábola
    \fill[red!20, domain=1:3, smooth, variable=\x]
        (1,-0.5) -- plot ({\x}, {(\x-2)^2+1}) -- (3,-0.5) -- cycle;

    % Ejes
    \draw[-Stealth] (-0.5,0) -- (4,0) node[below] {$t$};
    \draw[-Stealth] (0,-0.5) -- (0,4) node[left] {$x$};

    % Líneas punteadas para marcar los límites del intervalo
    \draw[dashed] (1,-0.5) -- (1,{(1-2)^2+3});
    \draw[dashed] (3,-0.5) -- (3,{(3-2)^2+3});

    \node at (2,2.5) {$D_+$};
    \node at (4,1.5) {$x=\phi(t)$};
    \node at (2,0.5) {$D_-$};
\end{tikzpicture}
...
\centering
\begin{tikzpicture}
    \fill[blue!20] (1,2) rectangle (3,0);
    \fill[red!20] (1,0) rectangle (3,-2);
...
En resumen, dada una ecuación de Riccati de la forma:
\begin{equation*}
    x' = a(t)x^2 + b(t)x + c(t)
\end{equation*}
...
\begin{ejemplo}
    ...
    \begin{enumerate}
        \item Veamos primero que las ecuaciones 
            ...
            \Func{\sigma}{\mathbb{R}}{\mathbb{R}}{\xi}{\xi^3 + \xi}
...
\begin{prop}
    ...
    \begin{proof}
        ...
        \end{equation*}
        % // TODO: Hacer. Está hecho en GeoIII, por si quieres dejarlo
...
La primera observación es que $\varphi_\lm$ es siempre admisible para cualquier $\lm \in \mathbb{R}$.
...
\subsection{Ecuaciones diferenciales invariantes por homotecias}
