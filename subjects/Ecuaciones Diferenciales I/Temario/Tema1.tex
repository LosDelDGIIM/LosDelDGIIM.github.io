...
\begin{ejemplo}
    ...
    % // TODO: Soy Arturo. No me queda muy claro qué es x_1 y x_2, por si quieres poner referencia o algo. Si no, lo borras y ale.
...
% // TODO: Apuntes del 30-9

\subsubsection{Recta normal a una curva}
Si $m_1$ y $m_2$ son las pendientes de la recta tangente y normal, respectivamente, tenemos que
\begin{equation*}
    m_1 \cdot m_2 = -1
\end{equation*}
En un punto $(x,y)$, la recta normal a una curva viene dada por la ecuación
\begin{equation*}
...
Otra forma de expresar la ecuación es:
\begin{equation*}
    y'(x) (v-y(x)) = x-u
\end{equation*}
Una forma mejor, ya que no debemos distinguir el caso en el que $y'(x) = 0$.
...
\begin{ejemplo}
    ...
    Para ello, buscamos expresar $y$ como función de $x$: $y=y(x)$. Cuando vamos considerando todas las rectas normales, lo que hacemos es mover la ecuación de la recta normal por los puntos que cumplen la curva. 

    Debemos imponer que las rectas pasen por el origen, es decir, que la ecuación de las rectas normales se cumpla por $(0,0)$ siempre. Esto sucede cuando $u=v=0$, para cada punto $(x,y)$ de la curva.

Luego se cumple que:
\begin{equation*}
    -y'(x)y(x) = x \qquad \forall x
\end{equation*}
Ecuación diferencial que no está en forma normal:
\begin{equation*}
    y'y+x=0
\end{equation*}
Pasaremos a resolverla con un truco del Capítulo 3:

\begin{equation*}
    y'(x)y(x) + x = 0
\end{equation*}
(Escribiremos esta expresión como la derivada de algo igual a 0, luego dicho algo es constante)
\begin{align*}
    \dfrac{d}{dx}\left(\dfrac{1}{2}{(y(x))}^{2}+\dfrac{1}{2}x^2\right)&= 0 \\
    \dfrac{1}{2}{\left(y(x)\right)}^{2}+\dfrac{1}{2}x^2&=k \quad k\in \mathbb{R}^+\\
    {\left(y(x)\right)}^{2}+x^2 &= k
\end{align*}
Dicha curva es una circunferencia centrada en el origen. Estamos buscando la solución en explícitas, luego despejamos y obtenemos:
\begin{equation*}
    y(x) = \pm \sqrt{k-x^2} \qquad x\in \left]-\sqrt{k},\sqrt{k}\right[
\end{equation*}
(Hemos puesto el intervalo abierto por ser solución de ecuación diferencial, luego debe estar definida en un abierto).
\end{ejemplo}

\subsection{Trayectorias ortogonales}
Tenemos una familia $\mathcal{F}$ de curvas uniparamétricas en el plano, de forma que por cada punto del plano pasa un única curva.

Queremos buscar otra familia uniparamétrica de curvas cuyas curvas corten de forma ortogonal a las otras.\\

Dada la familia $\mathcal{F}$ por una ecuación con un parámetro $c$:
\begin{equation*}
    F(x,y,c) = 0 \qquad c\in \mathbb{R}
\end{equation*}

derivando para obtener la ecuación diferencial:
\begin{equation*}
    \dfrac{\partial F}{\partial x}(x,y,c) + \dfrac{\partial F}{\partial y}(x,y,c)y' = 0
\end{equation*}
Ahora juntamos las dos ecuaciones y suponemos que somos capaces de eliminar el parámetro $c$ y que podemos poner en forma normal, obteniendo una ecuación
\begin{equation*}
    y' = f(x,y)
\end{equation*}
que nos describe la familia $\mathcal{F}$. A partir de la ecuación de la familia obtenemos la ecuación diferencial. Para obtener las trayectorias ortogonales, sacamos la ecuación diferencial y pasamos a la ecuación de dicha familia.

Suponiendo que podemos dividir, la ecuación de la familia de trayectorias ortogonales será:
\begin{equation*}
    y' = -\dfrac{1}{f(x,y)}
\end{equation*}
Ahora, bastará resolver la ecuación difernecial para obtener una ecuación implícita que nos define la familia uniparamétrica de funciones:
\begin{equation*}
    G(x,y,c) = 0
\end{equation*}

\begin{ejemplo}
    Las trayectorias ortogonales a la familia:
    \begin{equation*}
...
\begin{ejemplo}
    Dada la familia uniparamétrica $\mathcal{F}$:
    \begin{equation*}
    ...
    No nos preocupamos al dividir, sacamos ecuación y comprobamos luego

    En este caso, $F(x,y,c) = y-cx^2$

    \begin{align*}
        y &= cx^2 \\
        y' &= 2cx
    \end{align*}
    y eliminamos la $c$ despejando:
    \begin{equation*}
        c = \dfrac{y}{x^2}
    \end{equation*}
    \begin{equation*}
        y' = \dfrac{2y}{x}
    \end{equation*}
    Ahora, escribimos la ecución diferencial para la familia de trayectoriaas ortogonales:
    \begin{equation*}
        y' = \dfrac{-x}{2y}
    \end{equation*}
    ...
    \end{align*}
    Que son las elipses de eje mayor $\sqrt{2}$ (en el de abscisas) y eje menor 1.
    % // TODO: Pintar

    Poner nota de que tenemos familia $F$, damos familia $G$ y se comprueba que son ortogonales.
\end{ejemplo}

\section{Sistemas de ecuaciones diferenciales}
Nos centramos ahora en el estudio de los sistemas de ecuaciones diferenciales (supuesto que tenemos $d\in \mathbb{N}$ ecuaciones):
\begin{equation*}
    x_i' = f_i(t,x_1,\ldots,x_d) \qquad i=1,\ldots d
\end{equation*}
Y nuestras incógnitas son:
\begin{equation*}
    x_1 = x_1(t), x_2 = x_2(t), \ldots, x_d = x_d(t)
\end{equation*}
El conjunto de soluciones dependerá ahora de $d$ parámetros.
...
\begin{ejemplo}
    ...
    \begin{align*}
        x_1(t) &= c_1 \sen t - c_2 \cos t \\
        x_2(t) &= c_1 \cos t + c_2 \sen t \\
               & c_1, c_2 \in \mathbb{R}
    \end{align*}
\end{ejemplo}

\begin{ejercicio}
    Demostrar que ambas familias son las mismas:
    \begin{align*}
        x_1(t) &= b \sen (t+a) \\
        x_2(t) &= b \cos (t+a) \\
               &a,b \in \mathbb{R}
...
Volveremos a la notación $x$ e $y$ para las variables. Y nos centraremos en las ecuaciones paramétricas. Esto es, dar $x$ e $y$ en función de una variable independiente $t$:
\begin{equation*}
    x=x(t) \qquad y=y(t) \quad t\in \mathbb{R}
\end{equation*}
Ahora, tendremos dos funciones
\Func{x,y}{I}{\mathbb{R}}{t}{x(t),y(t)}
Ahora pensamos en la curva como un movimiento, en vez de como un lugar geométrico: dado un instante $t$, estaremos en un punto $(x(t), y(t))$.\\

Una cosa es describir una curva en implícitas (es dar un circuito de carreras)
\begin{equation*}
    F(x,y) = 0
\end{equation*}
Y otra es describir una curva en paramétricas (es recorrer el circuito), describimos la trayectoria de un móvil en el plano.\\


\begin{definicion}[Órbita]
Dada una curva en paramétricas
\Func{x,y}{I}{\mathbb{R}}{t}{x(t),y(t)}
(No tenemos un lugar geométrico, sino un movimiento en el plano)
La curva como lugar geométrico, será:
\begin{equation*}
    \{(x(t),y(t)) \mid t\in I\}
\end{equation*}
al que llamaremos órbita
\end{definicion}
Encontramos muchas parametrizaciones para una misma curva (ya que puede recorrerse de distintas formas).

\begin{ejemplo}
    Tomamos la curva en paramétricas:
    \begin{align*}
        x(t) &= \cos t \\
        y(t) &= \sen t \\
             & t \in I=\left]0, \dfrac{3\pi}{2}\right[
    \end{align*}
    que parametriza a la circunferencia en los 3 primero cuadrantes.
    % // TODO: Dibujar con una flecha en sentido levórigo
\end{ejemplo}


\subsection{Órbitas de un sistema autónomo en el plano}
Tendremos un sistema de dos ecuaciones y dos incógnitas autónomo:
\begin{equation*}
    \left\{\begin{array}{c}
            x' = f(x,y) \\
            y' = g(x,y)
    \end{array}\right.
\end{equation*}
En el que tratamos de buscar soluciones
\begin{equation*}
    x=x(t) \qquad y=y(t) \quad t\in \mathbb{R}
\end{equation*}
Las funciones $f$ y $g$ serán:
\Func{f,g}{D\subseteq \mathbb{R}^2}{\mathbb{R}}{(x,y)}{f(x,y),g(x,y)}
Con $D$ un conjunto abierto y conexo, $f$ y $g$ funciones continuas.\\

A continuacion, escribe algunos sistemas y vemos si son de la categoria
\begin{ejemplo}
    \begin{equation*}
        \left\{\begin{array}{c}
            x' = 3tx + y^2 \\
        ...
        \end{array}\right.
    \end{equation*}
    No: dependencia de $t$, no es autónomo
    \begin{equation*}
        \left\{\begin{array}{c}
            x' = t-y\\
            y' = t+x
    ...
    \begin{equation*}
        ...
        \end{array}\right.
    \end{equation*}
    Este sí: $f(x,y) = y+2$ y $g(x,y)=-x^3$
\end{ejemplo}

Se dice que un sistema es autónomo cuando las ecuaciones de los sistemas no dependen de $t$. Aunque luego tanto $x$ como $y$ dependerán de $t$.\\

Conseguir las soluciones de cualquier sistema del tipo es demasiado difícil. Nos centramos simplemente en calcular las órbitas del sistema. 
Va a ser un proceso largo:
- Primero de forma intuitiva
- Después de forma rigurosa
...
\begin{equation*}
    ...
    \end{array}\right.
\end{equation*}
La órbita es poner $y$ en función de $x$. Simplemente dividimos (suponiendo que podemos):

\begin{equation*}
    \dfrac{\frac{dy}{dt}}{\frac{dx}{dt}} = \dfrac{g(x,y)}{f(x,y)} \Longrightarrow \dfrac{dy}{dx} = \dfrac{g(x,y)}{f(x,y)}
\end{equation*}
Vamos a aprender por qué se puede tachar y cuándo podemos tachar.
...
\begin{ejemplo}
    ...
    \end{align*}
    Trabajamos en un intervalo donde $g$ no se anule ($g$ es la $x$)\\

    \begin{equation*}
        \dfrac{dx}{dt} = y \qquad \dfrac{dy}{dt} = -x \qquad \dfrac{dy}{dx} = \dfrac{-x}{y}
    \end{equation*}
    Que es la ecuación
    \begin{equation*}
        y' = -\dfrac{x}{y}
...
Con $I\subseteq \mathbb{R}$ un intervalo abierto

- Forma 1:
Sea $f:I\rightarrow J$ función biyectiva, podemos encontrar su inversa: $f^{-1}:J\rightarrow I$ tal que $f^{-1}\circ f = id_I$ y $f\circ f^{-1} = id_J$.

- Forma clásica:
Tenemos $x=f(t)$ $x$ en función de $t$, buscamos poner $t=g(x)$.

Hablaremos de la inversa de una función siempre que sea inyectiva, restringiendo su codominoi a la imagen de la función.

% // TODO: Poner como proposición
Toda función continua e inyectiva es estrictamente monótona. Luego al buscar funciones inversas, buscaremos funciones inversas de funciones estrictamente monótonas.\\

% // TODO: Poner como proposición
Sea
\Func{f}{I}{\mathbb{R}}{x}{f(x)}
Que suponemos derivable en $I$ con:
\begin{equation*}
    f'(t) > 0 \qquad \forall t\in I
\end{equation*}
Notemos que es pedir más a que tenga inversa, pero nos basta con esto.

En este caso, dicha función tiene inversa y es derivable: $f:I\rightarrow J$ con $J=f(I)$.
Se tiene que $J$ es un intervalo abierto (imagen de una funcion continua de un intervalo) y la inversa $f^{-1}$ es derivable en $J$.

Nos centramos en cómo calcular la derivada de la inversa, llamando $g=f^{-1}$.
Tendremos $g:J\rightarrow I$. y convendremos que $x=f(t)$ y $t=g(x)$ (de forma clásica):

\begin{align*}
    f(g(x)) &= x \qquad \forall x\in J \Longrightarrow  f'(g(x))g'(x) = 1  \\
    g(f(t)) &= t \qquad \forall t\in I 
\end{align*}
De donde:
\begin{equation*}
    g'(x) = \dfrac{1}{f'(g(x))}
...
%\begin{ejemplo}
%    La función seno es creciente desde $\frac{-\pi}{2}$ a $\frac{\pi}{2}$:
%    \Func{f}{\left]\frac{-\pi}{2},\frac{\pi}{2}\right[}{\mathbb{R}}{t}{\sen t}
%    Tendremos que $J = f(I) = \left]-1, 1\right[$.
%    Luego podemos definir:
%    \Func{\arcsen}{\left]-1,1\right[}{\left]\frac{-\pi}{2}, \frac{\pi}{2}\right[}{x}{\arcnsen x}
%    \begin{align*}
%        \arcsen (\sen t) = t \qq
%    \end{align*}
%\end{ejemplo}
...
\begin{ejercicio}
    Sacar la regla de derivación del arcoseno.
\end{ejercicio}

Análogo para el arcocoseno, arcotangente, argumentos senos y cosenos hiperbólicos.
