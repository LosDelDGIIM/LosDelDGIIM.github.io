...
\begin{ejemplo} Las proyecciones definidas en el espacio producto son identificaciones:
...
Vamos a ver un lema técnico que nos ayudará a saber, en algunos casos, que una aplicación es cerrada. Lo generalizaremos en el tercer tema.
\begin{lema}
...
\begin{ejemplo}
    ...
    \begin{enumerate}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
              \begin{axis}[
                ...
                \addplot3[domain=0:360,y domain=0:320, samples=8,surf,z buffer=sort]
                ({(12 + 3 * cos(x)) * cos(y)} ,
                {(12 + 3 * cos(x)) * sin(y)},
                {3 * sin(x)});
        ...
        \begin{figure}[H]
            ...
            \begin{subfigure}[c]{0.25\linewidth}
                ...
                \begin{tikzpicture}
                    ...
                    \addplot3[domain=0:360, y domain=0:360, samples=8, surf, z buffer=sort]
                    ({(12 + 3 * cos(x)) * cos(y)} ,
                    {(12 + 3 * cos(x)) * sin(y)},
                    {3 * sin(x)});
                    ...
                    % Línea roja
                    \addplot3[domain=-20:140, samples=5, surf, z buffer=sort, faceted color=red, opacity=1]
                     ({12+3*cos(x)} ,
                    {0},
                    {3*sin(x)});
                    \addplot3[domain=140:340, samples=5, surf, z buffer=sort, faceted color=red!20, opacity=1]
                     ({12+3*cos(x)} ,
                    {0},
                    {3*sin(x)});
                
                    % Línea azul
                    \addplot3[domain=200:360, samples=5, surf, z buffer=sort, faceted color=blue, opacity=1]
                     ({15*cos(x)} ,
                    {15*sin(x)},
                    {0});
                    \addplot3[domain=0:40, samples=5, surf, z buffer=sort, faceted color=blue, opacity=1]
                     ({15*cos(x)} ,
                    {15*sin(x)},
                    {0});
                    \addplot3[domain=40:200, samples=5, surf, z buffer=sort, faceted color=blue!50, opacity=1]
                     ({15*cos(x)} ,
                    {15*sin(x)},
                    {0});
        ...
        \Func{F}{\left(\bb{R}^{n+1}\setminus \left\{\vec{0}\right\}/\cc{R}, \T_u/\cc{R}\right)}{(\bb{S}^n, \T_u)}{x}{\dfrac{x}{\|x\|}}
