...
\begin{coro}
    Sea $(X,\T)$ un espacio topológico de Hausdorff (T2).
    \begin{equation*}
        A,B\subset X \text{ compactos } \Longrightarrow A\cap B \text{ compacto}
    \end{equation*}
\end{coro}
\begin{proof}
    ...
    Como $A\cap B\subset A$ es un subconjunto cerrado de un compacto $(A)$, entonces $A\cap B$ es compacto por la proposición \ref{prop:CerradoCompacto}.
\end{proof}
...
\end{definicion}
\begin{lema}
    Sea $(X,d)$ un espacio métrico. Entonces, $A\subset X$ es acotado si y solo si, para cada $x\in X$, $\exists r_x\in \bb{R}^+$ tal que $A\subset B(x,r)$.
\end{lema}
\begin{proof}
    Demostramos por doble implicación:
    \begin{description}
