...
\begin{notacion}
    Notamos $\bb{S}^r=S(0,r)$, para todo $r\in \bb{R}^+$.
\end{notacion}
...
\begin{prop}
    Todo espacio normado $(V,\|\cdot\|)$ es también un espacio métrico con $d(u,v)=\|v-u\|,\quad \forall u,v\in V$. A $d$ se le denomina distancia asociada a (o inducida por) la norma, y se denota por $d_{\|~\|}$.
\end{prop}
\begin{proof}
...
\begin{ejemplo}
    En el espacio vectorial $V=\cc{C}^0([a,b], \bb{R})$ de las funciones reales continuas definidas en el intervalo $[a,b]$, con $a<b$ tenemos la métrica euclídea
    \begin{equation*}
...
\end{prop}
\begin{proof}
    Sea la bola abierta $B(x,r)$. Consideramos $y\in B(x,r)$. Veamos si $\exists \veps >0\mid B(y,\veps)\subset B(x,r)$.

    Como $d(x,y)<r$ por la elección de $y$, tenemos que $\exists \veps >0\mid d(x,y)+\veps<r$. Veamos que $B(y,\veps)\subset B(x,r)$. Considerando $z\in B(y,\veps)$, se tiene que:
    \begin{equation*}
        d(z,x)\leq d(z,y)+d(y,x)\leq \varepsilon + d(x,y)< r
    \end{equation*}

    Por tanto, tenemos que $\exists \veps >0\mid B(y,\veps)\subset B(x,r)$, por lo que la bola $B(x,r)$ es un abierto.
\end{proof}
...
\begin{ejemplo} Algunos ejemplos de espacios topológicos son:
    \begin{enumerate}
        ...
        Si $X$ es finito, se tiene que $\cc{T}_{CF}=\cc{T}_D$.
        ...
        Si $X$ es numerable, se tiene que $\cc{T}_{CN}=\cc{T}_D$.
        ...
        \begin{equation*}
            U\in \cc{T}_S \Longleftrightarrow \forall x\in U,~~\exists \veps> 0 \mid [x,x+\veps[~\in U
        \end{equation*}
    \end{enumerate}
\end{ejemplo}
...
\begin{proof}
    ...
    Por tanto, por la desigualdad triangular, $d(x,y)\leq d(x,z) + d(z,y)<2r$. No obstante, por la elección de $r$ tenemos que $d(x,y)>2r$, por lo que llegamos a un absurdo y tenemos, por tanto, que $U\cap V=\emptyset$.
\end{proof}
...
\begin{enumerate}
    ...
    Buscamos demostrar que $X\setminus \ol{A}\in \T$. Supongamos $x\in X\setminus \ol{A}\neq \emptyset$. Entonces, como $x\notin \ol{A}$, $\exists U\in \T\cap N_x$ tal que $x\in U,~U\cap A=\emptyset$. 
    
    Veamos que $U\cap \ol{A}=\emptyset$. Por reducción al absurdo, supongamos $y\in U\cap \ol{A}\in N_y$. Entonces, $U\cap A\neq \emptyset$, llegando a una contradicción.

    Por tanto, sea $x\in U\subset X\setminus \ol{A}$. Entonces, $X\setminus \ol{A}\in N_x~\forall x\in X\setminus \ol{A}$. Entonces, $X\setminus\ol{A}\in \T$, lo que implica que $\ol{A}\in C_\T$.
    ...
    Sea $C\in C_\T$, $A\subset C$. Por reducción al absurdo, supongamos que $\ol{A} \not \subset C$, por lo que $\exists x\in \ol{A}\cap (X\setminus C)$, es decir, $\ol{A}\cap (X\setminus C)\neq \emptyset$. Por tanto, $A\not \subset C$, llegando a una contradicción.
...
\begin{enumerate}
    ...
    \item $\ol{A}=A^\circ \amalg\footnote{Símbolo de unión de disjunta.}~ \partial A$.
    \begin{gather*}
    ...
    Para la segunda, demostramos mediante doble implicación:
    \begin{description}
        \item[$\subset)$] $A^\circ = \ol{A}\setminus \partial A \subset A\setminus \partial A$.
...
\begin{proof}Demostramos cada resultado por separado:
    \begin{enumerate}
        ...
        \begin{description}
            ...
            \item[$\Longleftarrow)$] Supongamos que  $\forall x,y\in X,~x\neq y$ entonces $\exists U\in \T$ con $x\in U,~y\notin U$. Entonces, por definición de entorno, se tiene que $\exists N\in N_x$ con $y\notin N$. Intercambiando $x$ e $y$, tenemos que $\exists M\in N_y$ con $x\notin M$. Por tanto, tenemos que $(X,\T)$ es T1.
        \end{description}
...
\begin{enumerate}
    ...
    \item $(\bb{R}^n,\T_u)$ es 2AN, ya que una base numerable es $$\cc{B}=\{B(x,r)\mid x\in \bb{Q}^n,~r\in \bb{Q}^n\}$$
    ...
    \item $(\bb{R},\T_S)$ no es 2AN. Veámoslo.

    Tenemos que $\cc{B}=\{[x,y[\mid x<y\}$ es una base de $\T_S$. Por reducción al absurdo, supongamos que $\exists \cc{B}'=\{[x_n,y_n[~\mid n\in \bb{N}\}$ base de $\T_S$.

    Sea $x\in \bb{R}\setminus\{x_n\mid n\in \bb{N}\}\neq \emptyset$. Tenemos que $x\in [x,x+1[~\in \T_S$, y por ser $\cc{B}'$ base tenemos que $\exists n\in \bb{N}$ tal que $x\in [x_n,y_n[\subset [x,x+1[$, por lo que $x_n=x$, llegando a una contradicción.

    \item $(\bb{R},\T_S)$ es 1AN, ya que $\beta_x=\left\{\left[x,x+\frac{1}{n}\right[~\mid n\in \bb{N}\right\}$ es una base de entornos de $x$, $\forall x\in \bb{R}$.
\end{enumerate}
\end{ejemplo}
...
\begin{proof}
    ...
    \end{equation*}
    Tenemos que $\Gamma$ es numerable, y por ser $\cc{B}$ una base tenemos que $\Gamma\neq \emptyset$. Sea la aplicación $f:\Gamma\to I$ tal que $f(n)=i$ si y solo si $B_{m}\in U_{i},~\forall n\in \Gamma$.\\
