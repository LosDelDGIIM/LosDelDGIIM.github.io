...
\begin{ejercicio} \label{ej:8}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[node distance=6.3 em]
            ...
            \node[entidad] (ordenador) [below right of=tipo, node distance=9 em, xshift=9em] {Ordenador} edge(tipo);

            \node[herencia] (tipo_pc) [below of=ordenador] {Tipo} edge node[midway, right] {Disjunta} (ordenador);
            \node[entidad] (pc) [below left of=tipo_pc, xshift=-2em] {PC} edge(tipo_pc);
            \node[entidad] (portatil) [below right of=tipo_pc, xshift=2em] {Portátil} edge(tipo_pc);
            ...
            \node[relacion] (contiene) [right of=procesador, xshift=1.8em] {Contiene} edge[-Stealth](procesador) edge(ordenador);
