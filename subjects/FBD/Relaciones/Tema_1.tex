...
\begin{ejercicio} \label{ej:12}
    ...
    El Diagrama Entidad-Relación correspondiente se encuentra en la Figura~\ref{fig:ej12}.
    \begin{figure}
        \centering
        \begin{tikzpicture}[node distance=6.3 em]
            \node[entidad] (agente) {Agente};
            \node[herencia] (tipo) [below of=agente] {Tipo} edge[participacion obligatoria] node[midway, right] {Disjunta} (agente);
            \node[entidad] (oficial) [below left of=tipo, xshift=-2em] {Oficial} edge(tipo);
            \node[entidad] (policia) [below right of=tipo, xshift=2em] {Policía} edge(tipo);

            \node[relacion] (responsable) [below of=tipo, yshift=-2em] {Responsable} edge(oficial) edge(policia);
            \node[relacion] (dirige) [below of=oficial] {Dirige} edge(oficial);
            \node[entidad] (comisaria) [below of=dirige] {Comisaría} edge(dirige);

            \node[relacion] (participa) [right of=agente] {Participa} edge(agente);
            \node[entidad] (detencion) [right of=participa] {Detención} edge(participa);
            \node[relacion] (sufre) [right of=detencion] {Sufre} edge(detencion);
            \node[entidad] (delincuente) [right of=sufre] {Delincuente} edge(sufre);

            \node[relacion] (realiza) [below of=delincuente] {Realiza} edge(delincuente);
            \node[entidad] (comision_deltito) [below of=realiza] {$\text{Hola}$} edge(realiza);
        \end{tikzpicture}
