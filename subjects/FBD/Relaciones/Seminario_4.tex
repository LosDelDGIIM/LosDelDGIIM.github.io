...
\begin{ejercicio}\label{ej:AR_1}
    ...
    \begin{enumerate}
        ...
        \begin{multline*}
            \sigma_{\text{fecha\_matriculación} > \text{fecha}(1/1/22)}(\text{Vehículo}) \cup\\ \cup \pi_{\text{matrícula},\text{id\_modelo}, \text{fecha\_matriculación}}(\text{Vehículo}\bowtie \sigma_{\text{fecha}>\text{fecha}(1/1/22)}(\text{Repara}))
        \end{multline*}
        \item Vehículos con fecha de matriculación posterior al $1/1/22$ que han sido reparados alguna vez.
        \begin{multline*}
            \pi_{\text{matrícula},\text{id\_modelo}, \text{fecha\_matriculación}}(\sigma_{\text{fecha\_matriculación} > \text{fecha}(1/1/22)}(\text{Vehículo})\bowtie \text{Repara})
        \end{multline*}
        \item Marca de los vehículos que no han tenido ninguna reparación en el año $2022$.
        \begin{multline*}
            \pi_{\text{marca}}[(\text{Vehículo} - \pi_{\text{matrícula},\text{id\_modelo}, \text{fecha\_matriculación}}(\text{Vehículo} \bowtie\\\bowtie \sigma_{\text{fecha}(01/01/22) \leq \text{fecha}\leq \text{fecha}(31/12/22)}(\text{Repara})))\bowtie \text{Modelo}]
        \end{multline*}
        ...
        \begin{equation*}
            \rho\left(\pi_{\text{id\_mecánico},\text{marca}}(\text{Repara}\bowtie \text{Vehículo}\bowtie \text{Modelo})\right) = A
        \end{equation*}

        Usando además $\rho(A)=B$ para reducir la notación, la consulta sería:
        \begin{equation*}
            \pi_{\text{id\_mecánico}}\left(\sigma_{A.\text{marca} \neq B.\text{marca}}(A \bowtie B)\right)
        \end{equation*}

        Notamos que, usando el operador de reunión normal, la consulta también se puede poner como:
        \begin{equation*}
            \pi_{\text{id\_mecánico}}\left(A \bowtie_{A.\text{id\_mecánico} = B.\text{id\_mecánico} \land A.\text{marca} \neq B.\text{marca}} B\right)
        \end{equation*}
