...
\section{Crossover}
% // TODO: Memorizar crossover methods
...
\begin{itemize}
    ...
    % // En el ejemplo no es como dice internet. No hay ciclos
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{r|cccccccccc}
            ...
            Parent 3 & A & A & A & B & A & A & C & C & A & A \\ \hline
            Offspring & {A} & {A} & A & {B} & {A} & A & C & C & {A} & A
        \end{tabular}
        \caption{\centering Example of Voting Crossover (VX) with three parents.}
    \end{table}
    ...
    \begin{table}[ht]
        \centering
        \begin{tabular}{r|cc|cccccc|cc}
            Parent 1 & A & A & {B} & {B} & {A} & {A} & {C} & {C} & A & A \\
            Parent 2 & C & C & \textbf{A} & \textbf{B} & \textbf{A} & \textbf{B} & B & A & A & A \\ \hline
            Offspring & \textbf{A} & \textbf{B} & {B} & {B} & {A} & {A} & {C} & {C} & \textbf{A} & \textbf{B}
        \end{tabular}
        \caption{\centering Example of Maximal Preservative Crossover (MPX), where the selected interval is 2-7.}
    \end{table}
    ...
    \begin{itemize}
        \item Positions should be kept. If a gene is in the position $i$ in offspring, it should be in the same position $i$ in the parent from which it was taken.
        \item Genes can't be repeated in a chromosome.
    \end{itemize}
    
    This results in the following procedure to create the offspring:
    \begin{enumerate}
        \item The first gene of the offspring is taken from the first position of the first parent.
        \item For every gene in the offspring, the next gene is taken from the other parent. If that gene is already in the offspring, the order of the parents is switched, and the next gene for that position is chosen from the other parent.
        \begin{itemize}
        ...
        \item This process is repeated until the offspring is complete.
    \end{enumerate}
...
\begin{itemize}
    \item \ul{Scramble Mutation}: A segment of the chromosome is selected and its genes are randomly shuffled.
    \item \ul{Swap Mutation}: Two genes in the chromosome are selected and their positions are swapped.
\end{itemize}
...
% // TODO: Memorizar selection methods
