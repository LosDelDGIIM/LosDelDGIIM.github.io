...
\begin{equation*}
    f(n) = g(n) + h(n)
\end{equation*}
Where $g(n)$ is the actual cost from the root node to the current node $n$, and $h(n)$ is the heuristic estimate of the cost from $n$ to the goal. A* Search is guaranteed to find the optimal solution if the heuristic function is:
\begin{itemize}
    \item Admissible: It never overestimates the cost to reach the goal.
...
\begin{enumerate}
    ...
    \item If $\Delta E < 0$ (the neighbor is better), move to that neighbor. Otherwise, move to the neighbor with a probability of $e^{-\Delta E / T}$.
