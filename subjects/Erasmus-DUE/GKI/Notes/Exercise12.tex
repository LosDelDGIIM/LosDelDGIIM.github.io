...
\begin{ejercicio}
    ...
    Explainability refers to the ability of a model to provide understandable explanations for its decisions or predictions, often in a way that is accessible to non-experts. Interpretability, on the other hand, refers to the degree to which a human can understand the cause of a decision made by an AI model, which is often related to the complexity of the model and the transparency of its decision-making process. In summary, explainability focuses on providing explanations for model behavior, while interpretability focuses on understanding the underlying mechanisms of the model.
\end{ejercicio}

\begin{ejercicio}
    Name three explainability methods.

    Three explainability methods are Grad-CAM (Gradient-weighted Class Activation Mapping), LIME (Local Interpretable Model-agnostic Explanations), and SHAP (SHapley Additive exPlanations). These methods help provide insights into the decision-making process of machine learning models, allowing users to understand and trust the model's outputs.
\end{ejercicio}