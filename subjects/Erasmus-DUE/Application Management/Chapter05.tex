...
\begin{itemize}
    ...
    \item \ul{Automation}: It should be done in order to remove human error from the deployment process. It also improves security, as helps avoiding attackers to introduced malicious code during the deployment.
...
For asymetric cryptography, a proof of the authenticity of the public key is needed. This is done through certificates, which are made of:
\begin{itemize}
...
When a person or an organization wants to obtain a certificate, they first need to create a pair of public and private keys, and then they need to generate a Certificate Signing Request (CSR) that contains the public key and the identity information. The CSR is then sent to the CA, which verifies the identity of the requester and issues a certificate if everything is correct. The certificate is then signed with the CA's private key, and it can be used to prove the authenticity of the public key. It is important to note that the verification process is essential to ensure the security of the system, as it prevents attackers from obtaining fraudulent certificates.
...
\begin{itemize}
    ...
    % // TODO: Memorizar: OWASP SAMM
    \begin{itemize}
        \item {Governance}: Strategy and metrics.
        \item {Design}: Threat modeling and secure architecture.
        \item {Implementation}: Secure coding and code review.
        \item {Verification}: Security testing and vulnerability management.
        \item {Operations}: Incident detection and response.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}
        % // TODO: Memorizar: Fogg Behavior Model
        \centering
        \begin{tikzpicture}
