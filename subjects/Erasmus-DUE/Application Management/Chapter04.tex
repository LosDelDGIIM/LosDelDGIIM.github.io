...
\begin{itemize}
    \item Introducing new bugs while fixing the issue, known as Regression Bugs.
    \item The system could be in an unknown state after the fix (e.g., inconsistent data), as it was not tested or committed properly.
\end{itemize}
...
\subsection{Guidelines for a Deployment Pipeline}
...
\begin{itemize}
    \item Look \& Feel Testing: Ensures that the application meets the desired aesthetic and usability standards.
    \item Worst Case Testing: Evaluates the application's performance and stability under extreme conditions (e.g., for instance, the application is closed while performing a critical operation).
\end{itemize}
...
\begin{observacion}
    Both CD and Continuous Delivery focus on automating the deployment process on the production environment, while CI focuses on automating the build and testing processes in the earlier stages of development, in a test environment.
\end{observacion}


% // TODO: Qué es Rapid Incremental Deployment
...
\begin{itemize}
    ...
    \item \textbf{Consulted}: The person or team that provides input or expertise for a task or decision. They are usually consulted before a decision is made or a task is completed.
    \item \textbf{Informed}: The person or team that needs to be kept informed about the progress or outcome of a task or decision. They are usually informed after a decision is made or a task is completed.
\end{itemize}
...
The \verb|pivot\_root| command is a Linux system call similar to \verb|chroot|, but it provides a more complete isolation mechanism. It moves the current root filesystem to a new location and mounts a new root filesystem in its place. This allows for better isolation, as the process cannot see the original root filesystem. It is what Docker uses. It is used as follows: \verb|pivot\_root <new\_root\_directory> <put\_old\_root\_here>|. For example, \verb|pivot\_root /home/user/new\_root /home/user/old\_root| would move the current root filesystem to \texttt{/home/user/old\_root} and set \texttt{/home/user/new\_root} as the new root filesystem.
...
\begin{itemize}
    \item \textbf{PID Namespace}: Isolates process IDs, allowing processes in different namespaces to have the same PID. A child namespace will have its PID in its father namespace and a different one in its own namespace.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \verb|/sys/fs/cgroup/system.slice/|: This directory contains \verb|cgroups| for system services managed by the init system (e.g., systemd). For instance, the \verb|docker.service| \verb|cgroup| is located in \verb|/sys/fs/cgroup/system.slice/docker.service|, and it defines the resource limits for the Docker service.
