...
\begin{itemize}
    ...
    \item \verb|git add <file>|: Stages a file for commit. It also lets tracking new files and indicating when a file conflict has been resolved. Some important options are the following ones:
    \begin{itemize}
        \item \verb|-A|: Stages all the changes (new, modified and deleted files).
        \item \verb|-p|: Stages changes interactively, allowing to review and select specific changes to stage.
    \end{itemize}
    ...
    \item \verb|git log|: Displays the commit history of the repository. Some important options are the following ones:
    \begin{itemize}
        \item \verb|-p|: Shows the differences introduced in each commit.
        \item \verb|--oneline|: Displays each commit in a single line, showing only the commit hash and the commit message.
        \item \verb|--graph|: Displays the commit history as a graph, showing the branching and merging of commits.
        \item \verb|--all|: Shows the commit history of all branches, not just the current branch.
    \end{itemize}
    ...
    \item \verb|git blame <file>|: Shows the last commit that modified each line of the specified file, along with the author and the commit message. It is useful to identify who introduced a specific change in the codebase and when it was made. Some important options are the following ones:
    \begin{itemize}
        \item \verb|-L <start>,<end>|: Limits the output to a specific range of lines in the file. For instance, \verb|git blame -L 10,20 <file>| will show the last commit that modified each line between lines 10 and 20 of the specified file.
        \item \verb|--since <date>|: Shows only the commits that were made after the specified date. For instance, \verb|git blame --since "2024-01-01" <file>| will show the last commit that modified each line of the specified file, but only considering the commits that were made after January 1, 2024.
        \item \verb|--until <date>|: Shows only the commits that were made before the specified date.
        \item \verb|-c|: The output is in a more compact format. It is useful when you want to quickly identify the commits that modified specific lines without needing all the details.
    \end{itemize}

    \item \verb|git config <setting>|: It is used to configure Git settings. Some important options are the following ones:
    \begin{itemize}
        \item \verb|--global|: Sets the configuration for the current user, applying to all repositories on the machine. The configuration is stored in the \verb|~/.gitconfig| file.
        \item \verb|--local|: Sets the configuration for the current repository only. The configuration is stored in the \verb|.git/config| file of the repository.
        \item \verb|--system|: Sets the configuration for all users on the system. The configuration is stored in the \verb|/etc/gitconfig| file.
    \end{itemize}

    Some common settings that can be configured are the following ones:
    \begin{itemize}
        \item \verb|user.name|: Sets the name of the user that will be associated with the commits.
        \item \verb|user.email|: Sets the email of the user that will be associated with the commits.
        \item \verb|core.editor|: Sets the default text editor that will be used for writing commit messages and other Git-related tasks.
        \item \verb|alias.<alias_name>|: Creates a custom alias for a Git command. For instance, \verb|git config --global alias.co checkout| will allow to use \verb|git co| as a shorthand for \verb|git checkout|.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \verb|git switch <branch_name>|: It is an alternative to \verb|git checkout| for switching branches. It is more intuitive and focused on branch switching, without the additional functionalities of \verb|git checkout| (e.g., checking out specific commits, creating new branches, etc). With the option \verb|-c|, it creates a new branch and switches to it in a single command.
\end{itemize}
...
% // TODO: Memorizar: Collaboration workflows: Centralized, Integration Manager, Dictator and Lieutenants.
...
The project has a single central shared repository. All developers clone this repository, make changes in their local copies, and then push their changes back to the central repository. When more than one developer changes the same lines of code, conflicts may arise during the push operation, which need to be resolved before the changes can be successfully pushed.\\
