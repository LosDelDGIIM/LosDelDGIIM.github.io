...
\begin{definicion}[\emph{HEAD} Pointer]
    The \verb|HEAD| pointer is a reference to the current commit that the working directory is based on. It indicates the current position in the repository's history.
\end{definicion}
...
\begin{itemize}
    ...
    \item \verb|git checkout <branch_name>|: Switches to the specified branch, updating the working directory to reflect the state of that branch. It then also updates the \verb|HEAD| pointer to point to the new branch (the latest commit of that branch).
\end{itemize}
