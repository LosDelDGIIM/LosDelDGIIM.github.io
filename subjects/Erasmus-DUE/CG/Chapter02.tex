...
It should be noted that the first equation represents three equations, one for each coordinate. Therefore, we have a system of four equations with four unknowns. That system can be solved using Cramer's rule, but calculating the determinants can be computationally expensive. Instead, we can use the triple product of vectors to calculate the determinants more efficiently\footnote{Only two vector products need to be calculated.}. After solving this system, we have to check if the solution is valid:
\begin{itemize}
    ...
    \item $0 \leq \lambda_i \leq 1$ for $i = 1, 2, 3$ and $\lambda_1 + \lambda_2 + \lambda_3 = 1$, as we are only interested in the intersection points that are inside the triangle (not in the whole plane defined by the triangle).
\end{itemize}
...
\subsection{Spatial subdivision}\label{sec:spatial-subdivision}
