...
\begin{itemize}
    \item \ul{Time until discovery ($T_d$)}: the time it takes for a bug to be discovered since it was introduced.
    \item \ul{Time until found ($T_{\text{find}}$)}: the time it takes for the reason for the bug to be found since it was discovered.
    \item \ul{Time until fixed ($T_{\text{fix}}$)}: the time it takes for the bug to be fixed since its reason was found.
\end{itemize}
