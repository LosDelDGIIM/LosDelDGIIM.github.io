...
This pattern is shown in Figure~\ref{fig:command}. An important disavantage of this pattern is that it may be difficult to detect who and why started an action. However, it allows an elegant approach to different problems, as already explained. In addition, the OCP is respected by simply adding new command classes, and the SRP is also respected decoupling classes that invoke the action from the classes that actually perform them.
\begin{figure}
    \centering
    \begin{tikzpicture}
