...
\begin{ejercicio}
    ...
    \begin{enumerate}[label=(\alph*)]
        \item The situation is:
        \begin{enumerate}[label=\arabic*)]
            \item $C1$ subscribes to topic \texttt{home/groundfloor/temp} with the retain flag set to ``1''.
            \item $C2$ subscribes to topic \texttt{home/secondfloor/temp} with the retain flag set to ``0''.
            \item $C3$ subscribes to topic \texttt{home/thirdfloor/livingroom/temp} with the retain flag set to ``1''.
        \end{enumerate}
        After all publications are done, $C4$ subscribes to topic \texttt{home/secondfloor/temp} and $C5$ subscribes to topic \texttt{home}.\\

        In this case:
        \begin{itemize}
            \item $C4$ would not receive any message, as the retain flag for the message published by C2 was set to ``0'', which means that the broker does not store the message and does not send it to new subscribers.
            \item $C5$ would not receive any message, as its topic filter \texttt{home} does not match any of the topics that $C1$, $C2$, and $C3$ published messages for.
        \end{itemize}
        \item The situation is:
        \begin{enumerate}[label=\arabic*)]
            \item $C1$ subscribes to topic \texttt{home/thirdfloor/+/temp}.
            \item $C2$ publishes a message for topic \texttt{home/groundfloor/temp} with the retain flag set to ``1''.
            \item $C3$ publishes a message for topic \texttt{home/secondfloor/temp} with the retain flag set to ``0''.
            \item $C4$ publishes a message for topic \texttt{home/thirdfloor/livingroom/temp} with the retain flag set to ``0''.
        \end{enumerate}
        After all publications are done, $C5$ subscribes to topic \texttt{home/thirdfloor/livingroom/temp}.\\

        In this case:
        \begin{itemize}
            \item $C1$ would receive the message published by $C4$, as its topic filter \texttt{home/thirdfloor/+/temp} matches the topic of the message published by $C4$.
            \item $C5$ would not receive any message, as the retain flag for the message published by $C4$ was set to ``0''.
        \end{itemize}
        \item The situation is:
        \begin{enumerate}[label=\arabic*)]
            \item $C1$ subscribes to topic \texttt{home/+/temp}.
            \item $C2$ subscribes to topic \texttt{home/secondfloor/temp}.
            \item $C3$ publishes a message for topic \texttt{home/+/temp}.
        \end{enumerate}

        It is not possible to publish a message for topic \texttt{home/+/temp}, as the topic filter \texttt{home/+/temp} contains a wildcard character, which is not allowed in the topic of a published message. Therefore, no client would receive any message in this case.
        
        \item The situation is:
        \begin{enumerate}[label=\arabic*)]
            \item $C1$ subscribes to topic \texttt{home/\#} with QoS level 0.
            \item $C2$ subscribes to topic \texttt{home/+/temp} with QoS level 0.
            \item $C3$ publishes a message for topic \texttt{home/thirdfloor/temp} with QoS level 2.
        \end{enumerate}

            In this case:
            \begin{itemize}
                \item $C1$ would receive the message published by $C3$, as its topic filter \texttt{home/\#} matches the topic of the message published by $C3$. However, it would receive the message with QoS level 0, as that is the QoS level of its subscription, which means that the broker would not guarantee the delivery of the message to $C1$.
                \item $C2$ would also receive the message published by $C3$, as its topic filter \texttt{home/+/temp} matches the topic of the message published by $C3$. However, it would also receive the message with QoS level 0, as that is the QoS level of its subscription, which means that the broker would not guarantee the delivery of the message to $C2$.
            \end{itemize}
            
            % // TODO: Es correcto lo del downgrade?
    \end{enumerate}
\end{ejercicio}

\begin{ejercicio}
    Client $C2$ subscribes to the topic \texttt{a/b/c} with QoS level 0. Afterwards, client $C1$ publishes a message to the topic \texttt{a/b/c} with QoS level 2. Looking at the message headers:
    \begin{enumerate}
        \item What is the QoS level between $C1$ and the broker?
        ...
        \item What is the QoS level between the broker and $C2$?
