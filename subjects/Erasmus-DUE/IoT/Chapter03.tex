...
\begin{itemize}
    ...
    \begin{itemize}
        \item The Network Prefix is a fixed value that is defined by the 6LowPAN standard (given that it is only used for communication within the PAN, there is no need for a global unique prefix). It is set to \verb|FE80::|\footnote{In IPv6, hexadecimal notation is usually used. The usual convention is to split the address in 2-bytes groups, using ``:'' to differenciate them. Two consecutive colon ``::'' is used to filled with as many zeros as needed.}, which is the prefix for link-local addresses in IPv6.
        
        \item The IID is created using the EUI-64 address of the device, but just flipping the U/L bit.
    \end{itemize}
    \item \ul{16-bit Short Address}:
    
    The Network Prefix is the same as in the previous case, but the IID is created using the 16-bit short address of the device. The SADDR is ``converted'' to the following 64-bit address: \verb|<PAN ID>:00FF:FE00:<SADDR>|, where \verb|<PAN ID>| is the PAN ID of the device, and \verb|<SADDR>| is the 16-bit short address of the device. The IID is then created using that 64-bit address, but just setting the U/L bit to 0.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \emph{Fragment Type} (5 bits): \verb|11000| for the first fragment.
        ...
        It should be noted that the maximum value for this field is $2^{11}-1=2047$ bytes, which is larger than the minimum MTU required by IPv6. Therefore, the MTU for communication over IEEE 802.15.4 using 6LowPAN is effectively 2048 bytes, and therefore for PDUs larger than 2047 bytes, the fragmentation mechanism of IPv6 needs to be used in addition to the fragmentation mechanism of 6LowPAN.
        % // TODO: Is the MTU then 2048, or 1028
    ...
    \begin{itemize}
        \item \emph{Fragment Type} (5 bits): \verb|11100| for the rest of fragments.
        ...
        It should be noted that the maximum value for this field is $2^8-1=255$, which means that the maximum size of each fragment is $255 \cdot 8 = 2040$ bytes. Given that the maximum length of the original IPv6 PDU is 2047 bytes, that offset is enough to ensure that the fragments can be reassembled correctly.
...
\begin{itemize}
    \item \ul{IPv6 Header Compression (HC1)}: This is a simple compression mechanism that is used for compressing the IPv6 header. In the Dispatch Header, \verb|0x42| is used to indicate that the IPv6 header is compressed using HC1. After that, the HC1 encoding indicates which fields of the IPv6 header are compressed and which fields are not compressed. The HC1 encoding fields are:
    \begin{itemize}
        ...
        \begin{itemize}
            \item The first bit indicates if the network prefix is assumed to be the LL IPv6 prefix (which is \verb|FE80::|) or not. If assumed, a 1 is set in that bit and the network prefix is not included in the compressed header. If not assumed, a 0 is set in that bit and the network prefix is included in the uncompressed fields.
        ...
        % // TODO: Menciona que esta asunción no funciona, porque en muchos casos se usarán otras. Es cierto? No decíamos que la PGW les cambiaba? Como hacer NAT
