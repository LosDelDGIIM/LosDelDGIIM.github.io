...
\begin{itemize}
    % // TODO: Reasons for not using TCP/IP in IoT systems
...
\begin{itemize}
    % // TODO: Approaches to connect the IoT Things to the Internet
...
\begin{itemize}
    % // TODO: Types of devices in a PAN
...
\begin{itemize}
    % // TODO: Roles of devices in a PAN
...
The Physical Layer (PHY) of the IEEE 802.15.4 standard defines the physical characteristics and parameters of the wireless communication, such as the frequency bands where the communication can take place.
\begin{comment}
It defines three different frequency bands that can be used for communication:
\begin{itemize}
...
\begin{itemize}
    % // TODO: Types of MAC addresses in IEEE 802.15.4
...
% // TODO: Explain CSMA/CA and variants
...
\begin{itemize}
    % // TODO: Type of Deployments in NB-IoT
    ...
    \item \ul{Guard Band Deployment}: This deployment is used when there is an existing LTE network, and it uses the guard band (which is a portion of the spectrum that is reserved for preventing interference between different channels) for communication.
...
% // TODO: How to join/change a cell in NB-IoT
...
\begin{observacion}
    This local star topology should not be confused with the topology of the underlying network, which can be any topology. The publishers and subscribers can be connected to the broker through any type of network topology, as long as they can reach the broker to send and receive messages.
\end{observacion}
...
This has however a big problem, which is how much time the broker should keep the subscription information for a client with a permanent session when it goes offline. To solve this issue, MQTT provides three different fields that can be used:
\begin{itemize}
    \item \emph{Clean Start}: This field is used by the client when it connects to the broker to indicate if it wants to start a new session or use the existing one.
    \item \emph{Session Expiry Interval}: This field is used by the client when it connects to the broker to indicate how long the broker should keep the session information (including the subscription information) for that client when it goes offline.
    \item \emph{Message Expiry Interval}: This field is used by the publisher when it publishes a message to indicate how long the broker should keep that message in its queue for delivery to clients with permanent sessions that are subscribed to the topic of that message.
\end{itemize}
