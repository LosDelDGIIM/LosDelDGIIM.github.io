\chapter{Transparency}\label{chap:transparency}
...
\begin{itemize}
    ...
    \item The source blending factor is the alpha value of the source fragment, and the destination blending factor is one minus the alpha value of the source fragment. This allows for the creation of semi-transparent effects, where the color of the fragment is blended with the color of the background based on the alpha value of the fragment in front of it.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Disable the depth buffer updates, but enable the depth test and blending.
...
\begin{enumerate}
    ...
    \item In the following passes, the reference depth buffer (which had been updated in the previous pass) is used to peel away fragments with depth values less than or equal to the stored depth values. This allows to ignore the fragments that have already been stored in the previous passes.
    
    All other fragments are rendered using the working depth buffer. When finished, the depth values of the next closest fragments are stored in the reference depth buffer.
...
\begin{itemize}
    \item The ``logical'' approach. Rendering the scene with the depth test disabled and incrementing the stencil buffer for each fragment. The maximum value in the stencil buffer will give the depth complexity of the scene.
...
An important drawback of the depth peeling algorithm is that it requires multiple rendering passes, which can be computationally expensive. However, the MSAA can be abused to achieve the same effect requiring much less rendering passes.\\
...
The initial stencil buffer values start from $2$ in order to control that no more layers are stored than the number of samples available. When all the samples have been used, the stencil buffer values will $0$ for all the samples but the last one, which will have a stencil buffer value of $1$. If all the values are $0$, it means that an overflow has occurred.\\
