...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        In order to achieve bilinear interpolation in a rectangle, it is simply interpolated in both dimensions. Firstly, $\alpha$ is used to interpolate between in the $x$ direction, and then $\beta$ is used to interpolate between the results in the $y$ direction. Let's suppose $4$ points $(x_i, y_i)$, $(x_{i+1}, y_i)$, $(x_i, y_{i+1})$, and $(x_{i+1}, y_{i+1})$ with their corresponding values $f_{i,j}$, $f_{i+1,j}$, $f_{i,j+1}$, and $f_{i+1,j+1}$. The situacion is shown in Figure~\ref{fig:bilinear-interpolation}.
        \begin{figure}
            \centering
            \begin{tikzpicture}
                % Draw the grid
                ...
                % Draw the points
                \filldraw[black] (0,0) circle (4pt) node[below left] {$f_{i,j}$};
                \filldraw[black] (4,0) circle (4pt) node[below right] {$f_{i+1,j}$};
                \filldraw[black] (0,2) circle (4pt) node[above left] {$f_{i,j+1}$};
                \filldraw[black] (4,2) circle (4pt) node[above right] {$f_{i+1,j+1}$};
                
                
                % Draw the interpolation lines
                \draw[thick, dashed] (0,1) -- (4,1);
                \draw[thick, dashed] (1.3,0) -- (1.3,2);

                % Alpha and beta
                \node[below] (alpha) at (1.3,0) {$\alpha$};
                \node[left] (beta) at (0,1) {$\beta$};

                % Draw the interpolation point
                \filldraw[red] (1.3,1) circle (4pt) node[above right] {$f(x,y)$};
            \end{tikzpicture}
            \caption{Bilinear interpolation in a rectangle.}
            \label{fig:bilinear-interpolation}
        \end{figure}

        Therefore, the value of $f(x,y)$ can be calculated as follows:
        \begin{align*}
            f(x,y) &= (1-\beta)\left[(1-\alpha)f_{i,j} + \alpha f_{i+1,j}\right] + \beta\left[(1-\alpha)f_{i,j+1} + \alpha f_{i+1,j+1}\right]\\
        ...
        The values of $\alpha,\beta\in [0, 1]$ are calculated based on the position of the fragment relative to the texels.
        \begin{equation*}
            \alpha = \frac{x - x_i}{x_{i+1} - x_i}, \quad \beta = \frac{y - y_j}{y_{j+1} - y_j}
        \end{equation*}
    \end{itemize}
