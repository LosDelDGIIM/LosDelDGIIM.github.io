...
\begin{ejercicio}
    Demuestra que si $p:R\to B$ es un homeomorfismo local con $R$ compacto, $B$ conexo y ambos Hausdorff, entonces $p$ es una aplicación recubridora.\\
    ...
    \end{itemize}
    Como $p$ es abierta y cerrada tenemos que $p(R)\subseteq B$ es abierto y cerrado, como $B$ es conexo ha de ser $p(R) = B$, por lo que $p$ es sobreyectiva. Falta probar que todo punto $b\in B$ tiene un entorno abierto regularmente recubierto mediante $p$. Para ello, la estrategia a seguir será demostrar primero que la preimagen de cada punto es finita, luego que cada punto de $B$ tiene el mismo número de preimágenes, y finalmente la existencia de los entornos abiertos regularmente recubiertos.
    \begin{description}
        \item [$p^{-1}(b)$ es finito para cada $b\in B$.] Por reducción al absurdo, si $p^{-1}(B)\subseteq R$ fuera infinito como $R$ es compacto debería existir $r_0\in R$ un punto de acumulación de $p^{-1}(b)$, ya que:
            \begin{center}
                Sea $X$ un espacio topológico compacto y $A\subseteq X$ un conjunto infinito entonces $A$ tiene algún punto de acumulación, es decir, existe $x\in X$ de forma que
                \begin{equation*}
                    (N\setminus \{x\})\cap A \neq \emptyset 
                \end{equation*}
                para todo entorno $N$ de $x$.
            \end{center}
            \begin{proof}
                Por reducción al absurdo, si $A\subseteq X$ es un conjunto infinito sin puntos de acumulación tenemos entonces que para cada $x\in X$ podemos encontrar $O_x$ un entorno abierto suyo de forma que $(O_x\setminus \{x\})\cap A = \emptyset $. Si consideramos el recubrimiento:
                \begin{equation*}
                    X = \bigcup_{x\in X}O_x
                \end{equation*}
                como $X$ es compacto, podemos encontrar una cantidad finita de puntos $x_1,\ldots,x_n$ de forma que:
                \begin{equation*}
                    X = O_{x_1} \cup \ldots \cup O_{x_n}
                \end{equation*}
                De esta forma, tenemos que:
                \begin{equation*}
                    A = (O_{x_1}\cap A) \cup \ldots \cup (O_{x_n}\cap A)
                \end{equation*}
                con $O_{x_m}\cap A$ bien vacío o igual a $\{x_m\}$, para cada $m \in \{1,\ldots,n\}$. Tenemos así que $A$ tiene a lo mucho $n$ elementos, hemos llegado a una contradicción.
            \end{proof}
            Como $B$ es T2 tenemos que $\{b\}$ es cerrado, de donde $p^{-1}(b)$ también es cerrado, con lo que $r_0\in p^{-1}(b)$. Sin embargo, esto es imposible, ya que al ser $p$ un homeomorfismo local existe un entorno $U_0$ de $r_0$ de forma que $p\big|_{U_0}$ es inyectiva, de donde:
            \begin{equation*}
                p^{-1}(b) \cap U_0 = \{r_0\}
            \end{equation*}
            pero esto contradice que $r_0$ sea un punto de acumulación, contradicción que viene de suponer que $p^{-1}(b)$ es infinito para algún $b\in B$.
        \item [El cardinal de $p^{-1}(b)$ no depende del punto $b\in B$.] Vimos en el Ejercicio~\ref{ej:k_hojas} que si $p^{-1}(b_0)$ tiene $k$ elementos para $b_0\in B$ entonces $p^{-1}(b)$ tiene $k$ elementos, para todo $b\in B$.
        \item [Cada punto $b\in B$ tiene un entorno abierto regularmente recubierto.] Dado $b\in B$, podemos escribir:
            \begin{equation*}
                p^{-1}(b) = \{r_1,\ldots,r_k\} \subseteq R
            \end{equation*}
            Y tenemos que existe un entorno abierto $W$ de $b$ y $W_i$ abiertos disjuntos (aquí usamos la hipótesis de que $R$ sea T2) de $R$ de forma que cada $W_i$ contiene a $r_i$, para cada $i \in \{1,\ldots,k\}$; y de forma que $p\big|_{W_i}$ es un homeomorfismo desde $W_i$ en $W$.

            Para probar que $W$ es el entorno regularmente recubierto de $b$ basta ver que:
            \begin{equation*}
                p^{-1}(W) = \bigcup_{i = 1}^k W_i
            \end{equation*}
            La inclusión $\supseteq$ la tenemos ya garantizada. Para la otra inclusión, por reducción al absurdo supongamos que existe:
            \begin{equation*}
                r \in p^{-1}(W)\setminus \left(\bigcup_{i=1}^k W_i\right)
            \end{equation*}
            Tendríamos así que el punto $p(r)$ tendría al menos $k+1$ preimágenes (cada una en cada $W_i$ y además en $r$), pero esto es una contradicción, pues anteriormente vimos que cada punto debía tener $k$ preimágenes.
    \end{description}
\end{ejercicio}
    ...
    \begin{enumerate}[label=\alph*)]
        \item $G=\{f_{n,m}:n,m\in \mathbb{Z}\}$ es un grupo de homeomorfismos de $\mathbb{R}^2$ y para cada $x\in \mathbb{R}^2$ existe un entorno suyo $U_x$ tal que $f_{n,m}(U_x)\cap U_x=\emptyset $ para todo $n,m\in \mathbb{Z}$ con $(n,m)\neq (0,0)$.
            ...
            \begin{itemize}
                ...
                \item Además, dada $f_{n,m}\in G$ tomando $s=-n$, $t = m{(-1)}^{n+1}$ obtenemos que $f_{n,m}\circ f_{s,t} = f_{0,0}$.
            \end{itemize}
