...
\begin{lema}
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            (x,y) & \mapsto (x,y)
        \end{align*}
        que es continua. Tenemos además que $f\circ j : \bb{S}^1 \to \bb{S}^1$ conserva antípodas (salvo rotación de $\bb{S}^1$ podemos suponer que $(f\circ j)(1,0)=(1,0)$). Por el lema anterior tenemos que existe un lazo $\alpha$ tal que $\deg((f\circ j)(\alpha))$ es impar. En particular se tiene que 
        \begin{align*}
            (f\circ j)_*([\alpha]) &\neq [\veps_{(1,0)}]\\
