...
\begin{definicion}
    ...
    A la aplicación $H$ la llamaremos \textbf{homotopía} de $f$ en $g$.
\end{definicion}
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Representación de una homotopía $H$ entre $f$ y $g$.}
    \label{fig:homotopia}
\end{figure}
\begin{figure}
    \centering
    % --- Subfigura izquierda: Homotopía posible ---
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}[>=latex]
            % Ejes
            \draw[thick, black] (-2.5, 0) -- (-0.75, 0);
            \draw[->, thick, black] (0.75, 0) -- (3.3, 0) node[below] {$x$};
            \draw[thick, black] (0, -3.8) -- (0, -0.75);
            \draw[->, thick, black] (0, 0.75) -- (0, 3.3) node[left] {$y$};

            % Agujero central
            \fill[black!12] (0,0) circle (0.75);
            \draw[dashed, thick, black] (0,0) circle (0.75);
            \fill[black] (0,0) circle (1.5pt);

            % f(S^1)
            \draw[very thick, blue!85!black] 
                plot [smooth cycle, tension=0.75] coordinates {
                    (1.7, 3.0) (1.1, 2.5) (1.25, 1.6) (1.05, 0.9) 
                    (1.6, 0.45) (2.3, 1.2) (2.2, 2.4)
                };
            \node[blue!85!black] at (2.8, 2.3) {$f(\mathbb{S}^1)$};

            % Etapas intermedias
            \draw[thick, black!50, dashed] (1.55, 0.30) ellipse (0.62 and 0.75);
            \draw[thick, black!50, dashed] (1.40, -0.85) ellipse (0.48 and 0.58);
            \draw[thick, black!50, dashed] (1.30, -1.75) ellipse (0.38 and 0.44);

            % Flecha de homotopía
            \draw[->, thick, black!60] (2.35, 0.3) to[bend left=20] 
                node[right=1pt, font=\scriptsize] {$H(s, t)$} (2.05, -1.5);

            % g(S^1)
            \draw[very thick, red!85!black] (1.20, -2.55) circle (0.30);
            \node[red!85!black] at (2.1, -2.55) {$g(\mathbb{S}^1)$};
        \end{tikzpicture}
        \caption{Homotopía $H$ entre $f$ y $g$.}
        \label{fig:homotopia2}
    \end{subfigure}
    \hfill
    % --- Subfigura derecha: Homotopía no posible ---
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}[>=latex]
            % Ejes
            \draw[thick, black] (-3.2, 0) -- (-0.75, 0);
            \draw[->, thick, black] (0.75, 0) -- (3.5, 0) node[below] {$x$};
            \draw[thick, black] (0, -3.2) -- (0, -0.75);
            \draw[->, thick, black] (0, 0.75) -- (0, 3.2) node[left] {$y$};
            ...
            \draw[very thick, blue!85!black] 
                plot [smooth cycle, tension=0.8] coordinates {
                    (1.5, 2.9) (0.9, 2.3) (1.2, 1.4) (1.0, 0.9) 
                    (1.7, 0.6) (2.5, 1.2) (2.4, 2.4)
                };
            \node[blue!85!black] at (1.5, 3.3) {$f(\mathbb{S}^1)$};

            % g(S^1)
            \draw[very thick, red!85!black] 
                plot [smooth cycle, tension=0.75] coordinates {
                    (0.0, 1.4) (-1.1, 0.8) (-1.4, -0.6) (-0.8, -1.5) 
                    (0.3, -1.2) (1.2, -0.5) (1.1, 0.5)
                };
            \node[red!85!black] at (1.5, -1.3) {$g(\mathbb{S}^1)$};
        \end{tikzpicture}
        \caption{Situación no homotópica.}
        \label{fig:no_homotopia}
    \end{subfigure}
    
    \caption{Comparación de lazos en el plano perforado $\mathbb{R}^2 \setminus \{0\}$.}
    \label{fig:comparacion_homotopias}
\end{figure}
Veamos ahora la intuición de la definición de homotopía, mostrada en la Figura~\ref{fig:homotopia}. La idea es que $H$ nos permite deformar $f(X)$ en $g(X)$ de manera continua a lo largo del tiempo. Es decir, para cada $t\in [0,1]$ tenemos una aplicación continua $H_t:X\to Y$ dada por $H_t(x) = H(x,t)$ que nos permite ir deformando $f$ en $g$. En particular, tenemos que $H_0 = f$ y $H_1 = g$. Otro ejemplo intuitivo de homotopía es el que se muestra en la Figura~\ref{fig:homotopia2}, donde se puede ver cómo un lazo $f(\mathbb{S}^1)$ contenido en el primer cuadrante de $\bb{R}^2$ se puede deformar continuamente en otro lazo $g(\mathbb{S}^1)$, siempre evitando pasar por el agujero central. Por otro lado, en la Figura~\ref{fig:no_homotopia} se muestra un ejemplo de dos lazos que no son homotópicos, ya que uno de ellos rodea al agujero central y el otro no, por lo que no es posible deformar uno en otro sin pasar por el agujero. Notemos que por el momento tan solo se busca que el lector obtenga conocimientos intuitivos.
...
\begin{definicion}
    ...
    A la aplicación $H$ la llamaremos \textbf{homotopía por arcos} de $\alpha$ en $\beta$.
\end{definicion}

Notemos que, en el caso de la homotopía por arcos, la condición de que $H(0,t) =~x$ y $H(1,t) = y$ para todo $t\in [0,1]$ nos asegura que los extremos de los arcos se mantienen fijos a lo largo de la homotopía.

\begin{lema}
    Sea $X$ un e.t. y $x,y\in X$.
    Ser homotópico por arcos da lugar a una relación de equivalencia en $\Omega(X;x,y)$.\\
    
    Como es costumbre, a la clase de equivalencia de un arco $\alpha\in\Omega(X;x,y)$ la denotaremos por $[\alpha]$.
    \begin{proof}\
        \begin{enumerate}[label=(\roman*)]
            \item Dado $\alpha \in \Omega(X;x,y)$ queremos ver que $\alpha$ es homotópica por arcos con $\alpha$. Para ello podemos definir la homotopía por arcos
            ...
            \item Dados $\alpha, \beta\in \Omega(X;x,y)$ tales que existe $H:[0,1]\times [0,1]\to X$ continua verificando
            \begin{align*}
            ...
            \end{align*}
            Tomando $\tilde{H}(s,t):=H(s, 1-t)$ cumple claramente con lo que buscamos. Es decir, para todo $s,t\in [0,1]$:
            \begin{gather*}
            ...
            \item Dado $\alpha, \beta, \gamma\in \Omega(X;x,y)$ y $H_1,H_2:[0,1]\times[0,1]\to X$ continuas tales que 
            \begin{gather*}
                \begin{array}{c c c c}
...
Un ejemplo intuitivo de esta relación de equivalencia se da en la Figura~\ref{fig:homotopia_arcos}, donde se considera el e. t. $X$ dado por un cilindro hueco. En la Figura~\ref{subfig:equivalentes} se muestran dos arcos equivalentes, mientras que en la Figura~\ref{subfig:no_equivalentes} los arcos no son equivalentes puesto que van por caras opuestas del cilindro, por lo que no se podría transformar la una en la otra manteniendo los extremos fijos y sin atravesar el interior hueco del cilindro.
\begin{figure}
    \centering
    % Subfigura 1: Caminos equivalentes
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \begin{tikzpicture}
            ...
            % Puntos x e y
            \node[draw, circle, fill=black, inner sep=0.5pt, label=below:$x$] at (-0.75,1) {};
            \node[draw, circle, fill=black, inner sep=0.5pt, label=below:$y$] at (0.75,1) {};

            % Arcos
            \draw[blue, thick] plot[smooth] coordinates {
                (-0.75,1) (-0.25,0.7) (0.4,1) (0.75,1)
            };
            \draw[red, thick] plot[smooth] coordinates {
                (-0.75,1) (0,1.3) (0.7,1.2) (0.75,1)
            };
            \node[red] at (0,1.5) {$\alpha$};
            \node[blue] at (0,0.5) {$\beta$};
        \end{tikzpicture}
        \caption{Equivalentes}
        \label{subfig:equivalentes}
    \end{subfigure}
    \hspace{1cm}
    % Subfigura 2: Caminos no equivalentes
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \begin{tikzpicture}
            \shorthandoff{>}
            ...
            % Arcos
            \draw[blue, dashed] (1,0.65) arc (0:-180:1 and 0.5);
            \draw[blue, thick] (1,0.65) arc (0:45:1 and 0.5);
            \draw[blue, thick] (-1,0.65) arc (180:135:1 and 0.5);
            \draw[red, thick] (-0.75,1) arc (135:45:1 and 0.5);
            \node[red, thick] at (0,1.4) {$\alpha$};
            \node[blue, thick] at (0,-0.2) {$\beta$};
        \end{tikzpicture}
        \caption{No equivalentes}
        \label{subfig:no_equivalentes}
    \end{subfigure}
    
    \caption{Ejemplos de homotopía por arcos sobre un cilindro hueco.}
    \label{fig:homotopia_arcos}
\end{figure}
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \item Sea $X$ un e.t. y $x,y\in X$. Consideramos $\alpha\in \Omega(X; x,y)$ un arco y una aplicación $h:[0,1] \to [0,1]$ continua con $h(0)=0$ y $h(1)=1$. Entonces $\dot{\alpha}(s) = \alpha(h(s))$ es homotópica por arcos a $\alpha(s)$.
        
        \begin{proof}
            Es claro que $\dot{\alpha}$ es continua por ser composición de aplicaciones continuas, y existe una homotopía por arcos entre $\alpha$ y $\dot{\alpha}$ dada por:
            \Func{H}{[0,1]\times [0,1]}{X}{(s,t)}{\alpha((1-t)s + th(s))}

            Esta es claramente continua y verifica que:
            \begin{align*}
                H(s,0) = \alpha(s) \hspace{1cm} & H(s,1) = \alpha(h(s)) = \dot{\alpha}(s)\\
                H(0,t) = \alpha(0)=\dot{\alpha}(0) \hspace{1cm} & H(1,t) = \alpha(1) = \dot{\alpha}(1)
            \end{align*}
        \end{proof}
        Intuitivamente, podemos entender este ejemplo como que no importa a qué velocidad se recorra un arco para ser homotópico por arcos.\label{ej:homotopia_velocidad}
    \end{enumerate} 
\end{ejemplo}

Nuestro objetivo es definir el grupo fundamental, que será el grupo cociente con la operación $\ast$. Para ello, en primer lugar hemos de ver que la aplicación $\ast$ definida para arcos está bien definida en el espacio cociente.
\begin{lema}
    Sea $X$ un e.t. y $x,y,z\in X$.
    Dados dos arcos $\alpha_1, \alpha_2$ en $\Omega(X;x,y)$ y $\beta_1, \beta_2\in \Omega(X; y,z)$. Se verifica que, si $[\alpha_1] = [\alpha_2]$ y $[\beta_1] = [\beta_2]$, entonces se tiene que $[\alpha_1\ast\beta_1] = [\alpha_2\ast\beta_2]$.
    ...
    \begin{proof}
        ...
        \begin{gather*}
            ...
            \right.
        \end{gather*}
        Además, sabemos que existen $H_1, H_2$ continuas tal que, para todo $s,t\in [0,1]$:
        \begin{gather*}
            \begin{array}{c c c c}
            ...
            \end{array}
        \end{gather*}
        Ahora, vamos a construir una homotopía entre $\alpha_1\ast\beta_1$ y $\alpha_2\ast\beta_2$.
        Tomamos la aplicación $H:[0,1]\times [0,1] \to X$ dada por: 
        \begin{gather*}
            H(s,t) = \left\{
                \begin{array}{l c c}
            ...
            \right.
        \end{gather*}
        Es fácil ver que $H$ es continua y tenemos que: 
        \begin{gather*}
            H(s,0) = \left\{
                \begin{array}{l c c}
        ...
        \end{gather*}
        Análogamente se tiene que $H(s,1)=(\alpha_2\ast\beta_2)(s)$. Por último, vemos que:
        \begin{align*}
            H(0,t) &= H_1(0,t) = x\\
            H(1,t) &= H_2(1,t) = z
        \end{align*}
        Por tanto, $H$ es una homotopía por arcos entre $\alpha_1\ast\beta_1$ y $\alpha_2\ast\beta_2$, lo que nos da que $[\alpha_1\ast\beta_1] = [\alpha_2\ast\beta_2]$.
    \end{proof}
\end{lema}

\begin{definicion}
    Sea $X$ un e.t. y $x,y,z\in X$. Consideramos $\alpha\in \Omega(X;x,y)$, $\beta \in \Omega(X;y,z)$. A partir del lema anterior, podemos definir:
    \begin{gather*}
        [\alpha] \ast [\beta] := [\alpha \ast \beta]
    \end{gather*}
\end{definicion}

El siguiente teorema nos proporcionará la estructura de grupo junto con la operación $\ast$. Notemos que, en la definición de grupo que se vio en Álgebra II, tan solo se impone que haya elemento neutro \emph{por la izquierda} y, para cada elemento, que exista su opuesto \emph{por la izquierda}, puesto que el resto se puede deducir. No obstante, por claridad, demostraremos también que lo son por la derecha.
\begin{teo}
    Sea $X$ un e.t. y $x,y,z,w\in X$. Consideramos $\alpha\in \Omega(X;x,y)$, $\beta\in\Omega(X;y,z)$ y $\gamma\in \Omega(X;z,w)$. Entonces, se tiene que: 
    \begin{enumerate}[label=(\roman*)]
        \item $[\alpha] \ast ([\beta] \ast [\gamma]) = ([\alpha] \ast [\beta]) \ast [\gamma]$
        \item $[\alpha]\ast [\veps_y] = [\alpha]$ y $[\veps_x]\ast [\alpha] = [\alpha]$
        \item $[\alpha]\ast \left[\tilde{\alpha}\right] = [\veps_x]$ y $\left[\tilde{\alpha}\right]\ast [\alpha] = [\veps_y]$
    \end{enumerate}
    \begin{proof}\
        \begin{enumerate}[label=(\roman*)]
            \item Desarrollemos cada expresión:
            \begin{align*}
            ...
            Usando el ejemplo~\ref{ej:homotopia_velocidad} de la página~\pageref{ej:homotopia_velocidad} se tiene que $[\alpha] \ast ([\beta] \ast [\gamma]) = ([\alpha] \ast [\beta]) \ast [\gamma]$, puesto que realmente se están recorriendo las mismas curvas pero a distinta velocidad. Esta idea se muestra en la Figura~\ref{fig:comparacion_asociatividad}.
            ...
            \begin{figure}[h]
                ...
                \end{tikzpicture}
                \caption{Comparación de $(\alpha\ast\beta)\ast\gamma$ y $\alpha\ast(\beta\ast\gamma)$.}
                \label{fig:comparacion_asociatividad}
            \end{figure}


            \item Veamos que $\alpha \ast \veps_y$ se relaciona por una homotopía con arcos con $\alpha$. Recordemos que
            \begin{gather*}
                ...
                \right.
            \end{gather*}
            De nuevo, usando el ejemplo~\ref{ej:homotopia_velocidad} de la página~\pageref{ej:homotopia_velocidad}, se tiene que son homotópicas, por lo que $[\alpha] \ast [\veps_y] = [\alpha]$. De manera análoga, se tiene que $[\veps_x]\ast [\alpha] = [\alpha]$.

            \item Veamos ahora que $\alpha \ast \tilde{\alpha}$ es homotópico por arcos a $\veps_x$. Describamos ambas curvas.
            \begin{gather*}
            ...
            \end{gather*}
            Pensamos ahora una transformación $H$ que haga que cada vez la curva se vaya quedando más cerca de $x$ (cada vez vuelve antes de llegar a $y$). Intuitivamente, podríamos pensar en la gráfica de la Figura~\ref{fig:homotopia_arcos_opuesto}, en la que vamos reduciendo la distancia desde la curva $\alpha\ast \tilde{\alpha}$.
            \begin{figure}[h]
                \centering
                \begin{tikzpicture}[scale=0.75]
                    \begin{axis}[
                        ...
                        % Líneas
                        \addplot[very thick,red] coordinates {(0,0) (0.5,1) (1,0)};
                        \addlegendentry{$\alpha\ast\tilde{\alpha}$}
                        \addplot[very thick,olive, forget plot] coordinates {(0,0) (0.5,0.8) (1,0)};
                        \addplot[very thick,yellow, forget plot] coordinates {(0,0) (0.5,0.3) (1,0)};
                        \addplot[very thick,blue] coordinates {(0,0.01) (0.5,0.01) (1,0.01)};
                        \addlegendentry{$\veps_x$}
                    \end{axis}
                \end{tikzpicture}
                \caption{Homotopía por arcos entre ambos elementos.}
                \label{fig:homotopia_arcos_opuesto}
            \end{figure}

            Con el objetivo de definir la homotopía $H$, definimos la siguiente función auxiliar:
            \begin{gather*}
                h(s) = \left\{
                    \begin{array}{l c c}
                        2s & \text{ si } & s\in [0,\nicefrac{1}{2}]\\
                ...
                \right. 
            \end{gather*}
            Construimos entonces la homotopía $H(s,t) = \alpha((1-t)h(s))$, que claramente es continua por ser composición y producto de funciones continuas y verifica:
            \begin{align*}
            ...
            \end{align*}
            Por tanto, $H$ es una homotopía por arcos entre $\alpha\ast\tilde{\alpha}$ y $\veps_x$, lo que nos da que $[\alpha]\ast[\tilde{\alpha}] = [\veps_x]$. Análogamente se tiene que $[\tilde{\alpha}]\ast[\alpha] = [\veps_y]$.
        \end{enumerate}
    \end{proof}
\end{teo}
...
\begin{coro}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            ...
            \end{gather*}
            Esto se tiene debido al teorema anterior.
            ...
            \begin{gather*}
                [\alpha] \ast \left[\tilde{\alpha}\right] = [\veps_{x_0}] =  \left[\tilde{\alpha}\right] \ast [\alpha] 
            \end{gather*}
        \end{enumerate}
...
\begin{definicion}
    Dados un e.t. $X$ y un punto $x_0\in X$, el grupo algebraico dado por el corolario anterior lo llamaremos el \textbf{grupo fundamental} de $X$ en $x_0$, y lo denotaremos por $\pi_1(X,x_0)$.
\end{definicion}

\begin{ejemplo} Veamos por el momento un ejemplo intuitivo. Sea el e.t. $\bb{R}^2\setminus \{0,0\}$ y un punto cualquiera $x_0\in \bb{R}^2\setminus \{0,0\}$. Consideramos las curvas mostradas en la gráfica de la Figura~\ref{fig:lazos_R2_menos_0}.
    \begin{figure}[h]
        \centering
        \begin{tikzpicture}
            \shorthandoff{>}
        ...
        \end{tikzpicture}
        \caption{Ejemplos de lazos en $\bb{R}^2\setminus \{0,0\}$.}
        \label{fig:lazos_R2_menos_0}
    \end{figure}%
    
    En este espacio, vemos intuitivamente que $[\veps_{x_0}] \neq [\alpha] \neq \left[\tilde{\alpha}\right] \neq [\beta]$. Intuitivamente, podríamos intentar identificar este espacio con $\bb{Z}$ de la siguiente forma:
    \begin{itemize}
        ...
        \item Para los números negativos tomaremos los opuestos de los anteriores, es decir, las curvas recorridas en sentido contrario. Por ejemplo $\left[\tilde{\alpha}\right]$ será el $-1\in \bb{Z}$, $[\tilde{\beta}]$ será el $-2$ y así sucesivamente. 
    \end{itemize}
...
\begin{ejemplo}\label{ej:grupo_fundamental_estrellado}
    Sea $A\subseteq \bb{R}^n$ un subconjunto estrellado desde un punto $x_0\in A$. Entonces $\pi_1(A;x_0) = \{[\veps_{x_0}]\}$; es decir, es trivial.

    \begin{proof}
        Dado $\alpha\in \Omega(A; x_0)$, consideramos: 
        \begin{gather*}
            H(s,t) = (1-t)\alpha(s)+tx_0
        \end{gather*}

        Esta aplicación está bien definida puesto que $A$ es estrellado desde $x_0$, luego $H(s,t)\in A$ para todo $s,t\in [0,1]$. Además, es continua y cumple:
        \begin{align*}
            H(s,0) = \alpha(s) \hspace{0.5cm} &\text{ y } \hspace{0.5cm} H(s,1) = x_0 = \veps_{x_0}(s) \ \ \ &\forall s\in [0,1]\\
            H(0,t) = (1-t)x_0 + tx_0  \hspace{0.5cm} &\text{ y } \hspace{0.5cm} H(1,t) = x_0\ \ \ &\forall t \in [0,1]
        \end{align*}
        Por tanto, $H$ es una homotopía por arcos entre $\alpha$ y $\veps_{x_0}$, lo que nos da que $[\alpha] = [\veps_{x_0}]$. Como $\alpha$ era arbitraria, se tiene que $\pi_1(X;x_0) = \{[\veps_{x_0}]\}$.
    \end{proof}
...
\begin{teo}
    Sea $X$ un e.t. y $x,y\in X$. Si $X$ es arcoconexo, entonces $\pi_1(X,x)$ y $\pi_1(X,y)$ son iguales salvo isomorfismo.
    \begin{proof}
        Como $X$ es arcoconexo podemos considerar $\alpha$ un arco uniendo $x$ con $y$ y la aplicación
        \Func{F_\alpha}{\pi_1(X,y)}{\pi_1(X,x)}{[\gamma]}{[\alpha]\ast[\gamma]\ast\left[\tilde{\alpha}\right]}
        Esta está bien definida por lo visto anteriormente sobre el operador $\ast$. Queremos ver ahora que $F_\alpha$ es un isomorfismo de grupos. Para ello comencemos por ver que $F_\alpha$ es un homomorfismo, es decir, que se verifica
        \begin{gather*}
            F_\alpha([\gamma_1] \ast [\gamma_2]) = F_\alpha[\gamma_1]\ast F_\alpha([\gamma_2])
        \end{gather*}
        Desarrollamos el segundo término de la expresión:
        \begin{align*}
            F_\alpha[\gamma_1]\ast F_\alpha([\gamma_2]) &= ([\alpha] \ast [\gamma_1] \ast \left[\tilde{\alpha}\right]) \ast ([\alpha] \ast [\gamma_2] \ast \left[\tilde{\alpha}\right]) = [\alpha] \ast [\gamma_1] \ast [\gamma_2] \ast \left[\tilde{\alpha}\right] =\\
            &= F_\alpha([\gamma_1] \ast [\gamma_2])
        \end{align*}
        y tenemos la igualdad buscada. Veamos ahora que tiene inversa considerando $F_{\tilde{\alpha}}$, que por definición es $F_{\tilde{\alpha}}([\beta]) = \left[\tilde{\alpha}\right] \ast [\beta] \ast [\alpha]$ y que además verifica:
        \begin{gather*}
            (F_{\tilde{\alpha}} \circ F_{\tilde{\alpha}})([\gamma]) = F_{\tilde{\alpha}}([\alpha]\ast [\gamma]\ast \left[\tilde{\alpha}\right]) = \left[\tilde{\alpha}\right] \ast ([\alpha]\ast [\gamma]\ast \left[\tilde{\alpha}\right]) \ast [\alpha] = [\gamma] = Id_{\pi_1(X,y)}([\gamma])\\
            %
            (F_\alpha \circ F_{\tilde{\alpha}})([\gamma]) = F_\alpha(\left[\tilde{\alpha}\right] \ast [\gamma] \ast [\alpha]) = [\alpha] \ast (\left[\tilde{\alpha}\right] \ast [\gamma] \ast [\alpha]) \ast \left[\tilde{\alpha}\right] = [\gamma] = Id_{\pi_1(X,x)}([\gamma])
        \end{gather*}
        Por tanto, esta es su inversa. Hemos encontrado por tanto un homomorfismo biyectivo, luego $\pi_1(X,x) \cong \pi_1(X,y)$
    \end{proof}
\end{teo}
...
\begin{ejemplo}
    Si $A\subset \bb{R}^n$ es estrellado desde un punto $x_0\in A$, entonces $A$ es arcoconexo y, por el ejemplo de la página~\pageref{ej:grupo_fundamental_estrellado}, su grupo fundamental es el trivial. Por tanto, $A$ es simplemente conexo.
\end{ejemplo}
...
\begin{lema}
    ...
    \begin{proof}
        Para ver que $(f_x)_*$ está bien definida tomamos $\alpha_1,\ \alpha_2$ lazos basados en $x$ tales que $[\alpha_1] = [\alpha_2]$. Queremos comprobar que $[f\circ \alpha_1] = [f\circ \alpha_2]$. Para ello, sabemos que si $[\alpha_1] = [\alpha_2]$, entonces existe $H:[0,1]\times [0,1]\to X$ continua tal que
        \begin{gather*}
        ...
        \begin{gather*}
            (f_x)_* ([\alpha] \ast [\beta] ) = (f_x)_*([\alpha \ast \beta]) = [f\circ(\alpha \ast \beta)] = \left[\left\{
                ...
                \end{array}
            \right.\right]=\\
            =[(f\circ \alpha) \ast (f\circ \beta)] = (f_x)_* ([\alpha]) \ast (f_x)_* ([\beta])
        \end{gather*}
        Por tanto, queda demostrado que $(f_x)_*$ es un homomorfismo.
    \end{proof}
\end{lema}

\begin{definicion}
    A la aplicación $f_*$ dada por el lema anterior la llamaremos \textbf{homomorfismo inducido} por $f$.
\end{definicion}

\begin{propiedades}
    Algunas propiedades básicas de $f_*$ cuya demostración es inmediata son:
    \begin{enumerate}
        ...
        \item  Se verifica que $(Id_X)_*=Id_{\pi_1(X,x_0)}$; es decir,
...
\begin{coro}
    Si $h:X\to Y$ es un homeomorfismo entre espacios topológicos, entonces la aplicación dada por
    \begin{gather*}
        (h_x)_*:\pi_1(X,x) \to \pi_1(Y,h(x))
    \end{gather*}
    es un isomorfismo de grupos. Es decir, el grupo fundamental se conserva (salvo isomorfismo) por homeomorfismos.
\end{coro}
\begin{proof}
    Como $h$ es un homeomorfismo, existe $h^{-1}:Y\to X$ cumpliendo que $h^{-1}\circ h = Id_X$ y $h\circ h^{-1} = Id_Y$. Por tanto, se tiene que
    \begin{gather*}
        (h^{-1}_{h(x)})_* \circ (h_x)_* = (h^{-1}\circ h)_* = (Id_X)_* = Id_{\pi_1(X,x)}\\
        (h_x)_* \circ (h^{-1}_{h(x)})_* = (h\circ h^{-1})_* = (Id_Y)_* = Id_{\pi_1(Y,h(x))}
    \end{gather*}
    Por tanto, $(h_x)_*$ es un homomorfismo biyectivo, luego es un isomorfismo de grupos.
\end{proof}

Aunque ya será conocido por el lector, definimos la operación en el producto directo de grupos.
\begin{definicion}
    Si $(G_1,\ast_1)$ y $(G_2, \ast_2)$ son dos grupos algebraicos, consideramos sobre $G_1\times G_2$ el producto $\ast$ dado por 
    \begin{gather*}
        (g_1,g_2)\ast(g_1',g_2') := (g_1\ast_1 g_1', g_2 \ast_2 g_2')
    \end{gather*}
    para $g_1,g_1'\in G_1$, $g_2,g_2'\in G_2$
\end{definicion}
...
\begin{teo}
    ...
    \end{gather*}
    \begin{proof}
        Veamos que la siguiente aplicación
        ...
        \end{gather*}
        Como $p_1,p_2$ son continuas, por el lema del homomorfismo inducido $(p_1)_*,(p_2)_*$ están bien definidas, luego $\phi$ está bien definida. Asimismo, por ser $(p_1)_*,(p_2)_*$ homomorfismos, $\phi$ también lo es. Consideramos ahora la siguiente aplicación:
        \Func{\psi}{\pi_1(X,x)\times \pi_1(Y,y)}{\pi_1(X\times Y, (x,y))}{([\beta], [\gamma])}{[(\beta, \gamma)]}

        Veamos ahora que está bien definida. Para ello, tomamos $[\beta_1] = [\beta_2]$ y $[\gamma_1] = [\gamma_2]$. Entonces existen homotopías por arcos $H_1$ y $H_2$ entre $\beta_1,\beta_2$ y $\gamma_1,\gamma_2$ respectivamente. Consideramos entonces la aplicación $H=(H_1,H_2)$. Esta es continua por ser producto de continuas y verifica:
        \begin{gather*}
            H(s,0) = (H_1(s,0), H_2(s,0)) = (\beta_1(s), \gamma_1(s))\qquad \forall s\in [0,1]\\
            H(s,1) = (H_1(s,1), H_2(s,1)) = (\beta_2(s), \gamma_2(s))\qquad \forall s\in [0,1]\\
            H(0,t) = (H_1(0,t), H_2(0,t)) = (x,y) = H(1,t)\qquad \forall t\in [0,1]
        \end{gather*}
        Por tanto, $[(\beta_1,\gamma_1)] = [(\beta_2,\gamma_2)]$, luego $\psi$ está bien definida. Veamos ahora que son inversas:
        \begin{align*}
            (\psi \circ \phi)([\alpha]) &= \psi([p_1\circ \alpha], [p_2\circ \alpha]) = [((p_1\circ \alpha), (p_2\circ \alpha))] = [\alpha] = Id_{\pi_1(X\times Y, (x,y))}([\alpha])
            \\
            (\phi \circ \psi)([\beta], [\gamma]) &= \phi([(\beta, \gamma)]) = ([p_1\circ (\beta, \gamma)], [p_2\circ (\beta, \gamma)]) = ([\beta], [\gamma]) = Id_{\pi_1(X,x) \times \pi_1(Y,y)}([\beta], [\gamma])
        \end{align*}
        Por tanto, $\phi$ y $\psi$ son inversas. Como $\phi$ es un homomorfismo biyectivo, es un isomorfismo de grupos, luego $\pi_1(X\times Y, (x,y)) \cong \pi_1(X,x) \times \pi_1(Y,y)$.
    \end{proof}
\end{teo}
...
\begin{lema}
    ...
    \end{gather*}
    donde $\varphi([\gamma]) = \left[\tilde{\alpha}\right] \ast [\gamma] \ast [\alpha]$. En particular, $f_*$ es inyectiva (respectivamente sobreyectiva) si y solo si $g_*$ lo es.
    ...
    \begin{proof}
        ...
        Pero tenemos además
        \begin{gather*}
            (\varphi \circ f_*) ([\beta]) = \left[\tilde{\alpha}\right] \ast [f\circ \beta] \ast [\alpha]
        \end{gather*}
