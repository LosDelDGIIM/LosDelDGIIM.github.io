...
\begin{ejercicio}
    ...
    \begin{itemize}
        \item Si $a=b$, entonces $\gamma:[0,1]\to X$ dada por:
            ...
            \begin{equation*}
                \gamma^{-1}(B) = \gamma^{-1}(\left]a,b\right[\times \{0,1\}) = \gamma^{-1}(\left]a,b\right[\times \{a\}) \text{\ abierto de\ } [0,1]
            \end{equation*}
            Ya que el conjunto $\left]a,b\right[\times \{a\}$ es un abierto para la topología usual inducida en $X$y $\alpha$ es una aplicación continua para la topología usual.
...
\begin{ejercicio} % // TODO: HACER
    ...
    Por un razonamiento análogo al anterior, tenemos que $X_1$ y $X_2$ son conjuntos arcoconexos. Asimismo, el conjunto unitario $\{(1,0)\}$ es arcoconexo. Por lo que tenemos tres conjuntos arcoconexos que forman una partición de $X$: $X_1$, $X_2$ y $\{(1,0)\}$. Por un razonamiento análogo al visto en el ejemplo de la página~\ref{ej:conexo_no_arcoconexo}, tenemos que $X_1$, $X_2$ y $\{(1,0)\}$ son las componentes arcoconexas de $X$, ya que si algún arco uniera un punto de dos de ellos, este no sería continuo, por lo que no existiría tal arco. Por tanto, el número de componentes arcoconexas de $X$ es 3.
