...
\begin{prop}
    ...
    \begin{enumerate}
        \item $X$ es conexo.
        \item Los únicos subconjuntos de $X$ que son abiertos y cerrados a la vez son el vacío y el total. 
        \item Los únicos subconjuntos de $X$ con frontera vacía son el vacío y el total. 
    \end{enumerate}
\end{prop}
...
\begin{ejemplo}\
    \begin{enumerate}
        ...
        \begin{proof}
            ...
            \begin{description}
                \item[Opción 1] Sea $\alpha:[0,1]\to X$ un arco, y consideremos $x_0 = \alpha(0)\in X$. Como $X$ tiene la topología discreta, $\{x_0\}$ es abierto y cerrado en $X$. Por ser $\alpha$ continua, $\alpha^{-1}(\{x_0\})$ es abierto y cerrado en $[0,1]$. Considerando que $[0,1]$ es conexo, se tiene que $\alpha^{-1}(\{x_0\}) \in \{\emptyset, [0,1]\}$. Como $0\in \alpha^{-1}(\{x_0\})$, se tiene que $\alpha^{-1}(\{x_0\}) = [0,1]$ y por tanto:
                \begin{gather*}
                    \alpha(t) = x_0 \ \ \forall t\in [0,1]
                \end{gather*}
                es decir, $\alpha$ es un arco constante.

                \item[Opción 2] Sea $\alpha:[0,1]\to X$ un arco. Puesto que $[0,1]$ es compacto y $\alpha$ es continua, $\alpha([0,1])$ es compacto en $X$. Como $X$ tiene la topología discreta, $\alpha([0,1])$ ha de ser un conjunto unitario, es decir, $\alpha([0,1]) = \{x_0\}$ para algún $x_0\in X$. Por tanto, $\alpha$ es un arco constante.
            \end{description}
        \end{proof}
        ...
        \item \label{ej:arcos_s2}
        Consideramos $\bb{S}^n\subset \bb{R}^{n+1}$, y tomamos $x,y\in \bb{S}^n$ cualesquiera. Buscamos un arco en $\bb{S}^n$ que una $x$ con $y$, como podemos ver en la figura \ref{fig:arcos_s2}.
        \begin{figure}
            \centering
            \begin{tikzpicture}
...
\begin{ejemplo}\label{ej:arcos_s2_2}
    Veamos ahora otra forma de probar que $\bb{S}^n = \{x\in \bb{R}^{n+1} : |x| = 1\}$ es arcoconexo, distinta de la que se da en el ejemplo de la Página~\pageref{ej:arcos_s2}. Puesto que $\bb{S}^n\setminus \{N\}$, con $N=(0,\dots,0,1)$, es homeomorfo a $\bb{R}^n$ que es arcoconexo, tenemos que $\bb{S}^n \setminus\{N\}$ es arcoconexo. Análogamente, $\bb{S}^n \setminus\{S\}$ con $S=(0,\dots,0,-1)$ es arcoconexo y podemos escribir
    \begin{gather*}
