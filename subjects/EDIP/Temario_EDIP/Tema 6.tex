...
\begin{coro}
    Sea $X$ una variable aleatoria discreta con valores en $Re_X=\{x_1, x_2, \ldots\} \subset \R$ y $h:\R \rightarrow \R$ medible tal que $Y=h(x)$, entonces $Re_Y=h(Re_X)$ y la función masa de probabilidad de $Y$ será:
    \begin{equation*}
        P[Y=y] = \sum_{x\in h^{-1}(y)\cap Re_X} P[X=x] \qquad \forall y \in Re_Y
    \end{equation*}
\end{coro}
\begin{proof}
    Como $h$ es medible y $Re_X$ es numerable, entonces $h(Re_X)=Re_Y$ es numerable, por lo que $Y$ es discreto. Por el Teorema de Cambio de Variable, tenemos que:
    \begin{equation*}
...
\begin{prop}
    ...
    $X(\omega)=c\quad \forall \omega \in \mathcal{A}$, entonces:
    \begin{equation*}
        E[X]=c
    \end{equation*}
\end{prop}
