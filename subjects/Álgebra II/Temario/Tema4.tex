...
Este Capítulo trata sobre los grupos resolubles, propiedad interesante de un grupo que tendrá numerosas aplicaciones, como por ejemplo en la solución de ecuaciones por radicales. Sin embargo, la definición de grupo resoluble ha de esperar, pues primero tenemos que hacer un estudio de las ``series de un grupo''.
...
\begin{ejemplo}
    ...
    \begin{equation*}
        A_4 > V > \{1\}
    \end{equation*}
    Un refinamiento propios de la misma es:
    \begin{equation*}
...
\begin{prop}
    Si un grupo $G$ tiene una serie de composición, entonces sus factores de composición son grupos simples.
    \begin{proof}
        Sea una serie de composición de longitud $r$:
        \begin{equation}\label{eq:prop_1}
            G=G_0 \rhd G_1 \rhd \ldots \rhd G_r = \{1\}
        \end{equation}
        Supongamos que existe un índice $i \in \{1,\ldots,r\}$ de forma que $G_{i-1}/G_i$ no sea un grupo simple. En cuyo caso, existirá un subgrupo propio normal suyo no trivial $\{1\}\neq H \lhd G_{i-1}/G_i$. Si usamos ahora el Tercer Teorema de Isomorfía considerando la proyección $p_i:G_{i-1}\to G_{i-1}/G_i$, tenemos que:
        \begin{equation*}
            p_i^\ast(H) \lhd G_{i-1}
        \end{equation*}
        Además, como $H$ es un subgrupo del cociente, tenemos que $\{G_i\} \subseteq H$, luego:
        \begin{equation*}
            G_i = \ker(p_i) = p_i^\ast(G_i) \subseteq p_i^\ast(H) \lhd G_{i-1}
        \end{equation*}
        Y por ser $G_i\lhd G_{i-1}$, deducimos que también $G_i\lhd p_i^\ast(H)$. Hemos encontrado un subgrupo normal de $G$ que estaría entre $G_i$ y $G_{i-1}$:
        \begin{equation}\label{eq:prop_2}
            G=G_0 \rhd G_1 \rhd \ldots \rhd G_{i-1} \rhd p_i^\ast(H) \rhd G_i \rhd \ldots \rhd G_r = \{1\}
        \end{equation}
        Por lo que~(\ref{eq:prop_2}) es un refinamiento de~(\ref{eq:prop_1}), que era una serie de composición, \underline{contradicción}, que venía de suponer que un factor de la serie de composición no era simple.
    \end{proof}
\end{prop}
...
\begin{prop}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item Si $G$ es el grupo trivial, es fácil hayar la serie de composición.
            \item Si $G$ es simple, entonces no tiene subgrupos normales propios, por lo que tiene una única serie de composición:
                \begin{equation*}
                    G \rhd \{1\}
                \end{equation*}
            ...
            \item Si $|G|$ no es primo y $G$ no es simple, por inducción sobre $n = |G|$, suponemos que es cierto para todo grupo $H$ con $|H|<|G|$ (observemos que el punto anterior nos sirve como caso base).
                ...
                Como $G$ es finito, tiene un número finito de subgrupos, entre los que podemos encontrar (por ser $G$ no simple) $G_1$, un subgrupo normal propio maximal\footnote{Es decir, que no existe $K\lhd G$ con $G_1 \lhd  K$.} de $G$. Como $|G_1| < |G|$ ($G_1$ es subgrupo propio), por hipótesis de inducción tenemos una serie de composición para $G_1$:
                \begin{equation*}
...
\begin{teo}[de Refinamiento de Schreier]
    ...
    \begin{proof}
        ...
        \begin{equation*}
            H_{ij} = H_j(G_i\cap H_j-1) \lhd H_j(G_{i-1}\cap H_{j-1}) = H_{i-1j} \qquad \forall i \in \{1, \ldots, r\}
        \end{equation*}
...
\noindent
Podemos pensar en calcular series de composición de un grupo conocida una serie de composición en un grupo isomorfo, resultado que podemos esperar que sea cierto (y que de hecho vamos a probar a continuación); sin embargo, el recíproco no es cierto en general: si tenemos dos series de composición, una de un grupo $G$ y otra de otro grupo $K$ que son isomorfas, en general $G$ y $K$ no van a ser isomorfos.
...
\begin{ejemplo}
    ...
    En $\mathbb{Z}_6$, que no es isomorfo a $S_3$ por ser abeliano, si observamos su retículo:
            ...
            \begin{equation*}
                \mathbb{Z}_6 \rhd \langle 2 \rangle  \rhd \{0\}
            \end{equation*}
            ...
            \end{equation*}
            Pero como esta no es isomorfa a la primera y sabemos que todas las series de composición de un mismo grupo son isomorfas, sabemos que esta segunda no es de composición. Veamos que las series:
            \begin{gather*}
                S_3 \stackrel{2}{\rhd} A_3 \stackrel{3}{\rhd} \{1\} \\
            ...
            \begin{gather*}
                ...
                A_3/\{0\} \cong A_3 \cong \mathbb{Z}_3 \cong \langle 2 \rangle  \cong \langle 2 \rangle /\{0\}
            \end{gather*}
\end{ejemplo}
...
\begin{prop}
    ...
    %     Como todas las series de composición de $G$ son isomorfas entre sí y todas las series de composición de $K$ también (por el Teorema de Jordan-Holder), basta ver que hay una serie de composición de $G$ que es isomorfa a una serie de composición de $K$. Para ello, sean:
    %     \begin{align*}
    %         G &= G_0 \rhd G_1 \rhd \ldots \rhd G_r = \{1\} \\
    %         H &= H_0 \rhd H_1 \rhd \ldots \rhd H_s = \{1\}
    %     \end{align*}
    %     dos series de composición, como $G$ y $K$ son grupos isomorfos, ha de existir $f:G\to K$ un isomorfismo de grupos.
    % \end{proof}
\end{prop}~\\
...
\begin{teo}[de Abel]
    ...
    \begin{proof} % // TODO: Revisarla
        ...
        \begin{enumerate}
            ...
            \item Si $\sigma$ tiene un ciclo de longitud mayor o igual que 3 en el que mueve a $x_1,x_2$ y $x_3$:
                \begin{align*}
                    \tau &= (x_3\ x_4\ x_5) \\
...
Antes de pasar con la definición de grupos resolubles, hemos de respasar ciertos conceptos relacionados con la operación de conmutador que ya definimos sobre los elementos de $G$, recordamos que era la aplicación $[\cdot ,\cdot ]:G\times G\to G$ dada por:
\begin{equation*}
...
\begin{prop}
    ...
    \begin{proof}
        Llamando $\Lambda$ a dicho conjunto, para ver que es un subgrupo, si $a,b\in \Lambda$, entonces $\exists n,m\in \mathbb{Z}$ de forma que:
        \begin{equation*}
            a = {([x,y])}^{n} \qquad b = {([x,y])}^{m}
        \end{equation*}
        En dicho caso:
        \begin{equation*}
            ab^{-1} = {([x,y])}^{n} {([x,y])}^{-m} = {([x,y])}^{n-m}
        \end{equation*}
        Con $n-m\in \mathbb{Z}$, por lo que $ab^{-1}\in \Lambda$, de donde $\Lambda < G$.\\

        \noindent
        Ahora, para ver que $\Lambda \lhd G$, sea $a\in \Lambda$, existirán $n\in \mathbb{Z}$, $x,y\in G$ de forma que $a = {([x,y])}^{n}$. Sea $z\in G$:
        \begin{itemize}
            \item Si $n = 1$:
                \begin{equation*}
                    z[x,y]z^{-1} = [zxz^{-1}, zyz^{-1}] \in \Lambda
                \end{equation*}
            \item Si $n = -1$:
                \begin{equation*}
                    z{([x,y])}^{-1}z^{-1} = z[y,x]z^{-1} = [zyz^{-1}, zxz^{-1}] \in \Lambda
                \end{equation*}
            \item Supuesto para $n-1$, para $n$:
                \begin{equation*}
                    z{([x,y])}^{n}z^{-1} = z[x,y]z^{-1}z{([x,y])}^{n-1}z^{-1} = (z[x,y]z^{-1})(z{([x,y])}^{n-1}z^{-1}) 
                \end{equation*}
                Con $z[x,y]z^{-1}\in \Lambda$ por ser $[x,y]\in \Lambda$ y $z{([x,y])}^{n-1}z^{-1} \in \Lambda$ por hipótesis de inducción. Como $\Lambda < G$, concluimos que $z{([x,y])}^{n}z^{-1}\in \Lambda$. Escribiendo $m = -n$:
                \begin{equation*}
                    {\left(z{([x,y])}^{m}z^{-1}\right)}^{-1} = z^{-1}{([x,y])}^{-m}z \in \Lambda
                \end{equation*}
                Por ser $\Lambda < G$, concluimos que $z{([x,y])}^{m}z^{-1}\in \Lambda$.
        \end{itemize}
        En definitiva, $\Lambda\lhd G$.
    \end{proof}
\end{prop}
...
\begin{prop}\label{prop:abelianizado}
    ...
    \begin{proof}
        ...
        \begin{description}
            \item [$\Longrightarrow)$] Si consideramos la proyección al cociente $p:G\to G/N$, sea $a\in [G,G]$, entonces existirán $n\in \mathbb{Z}$ y $x,y\in G$ de forma que:
                \begin{equation*}
                    a = {([x,y])}^{n}
                \end{equation*}
                Si calculamos la imagen de $a$ por $p$ (que recordamos que es un homomorfismo):
                \begin{multline*}
                    p(a) = {(p([x,y]))}^{n} = {\left(p\left(xy{(yx)}^{-1}\right)\right)}^{-1} = {(p(xy))}^{n}{(p(yx))}^{-n} \\ \AstIg {(p(x))}^{n} {(p(y))}^{n} {(p(y))}^{-n} {(p(x))}^{-n} = 1
                \end{multline*}
                Donde en $(\ast)$ hemos usado que $G/N$ es abeliano. Tenemos que $a\in \ker(p) = N$.
            \item [$\Longleftarrow)$] Sean $x,y\in G$, entonces:
                ...
                Pero:
                \begin{equation*}
                    (xN)(yN) = xyN = yxN = (yN)(xN) \Longleftrightarrow xy{(yx)}^{-1}N = N 
                \end{equation*}
                Por lo que $(xN)(yN) = (yN)(xN)$, para todos $x,y\in G$.
        \end{description}
    \end{proof}
\end{prop}
...
\begin{ejemplo}
    ...
    \begin{itemize}
        ...
        \item $S_4$ es resoluble, ya que $S_4' = [S_4, S_4] = A_4$ y ya tenemos la serie de $A_4$:
            ...
            \end{align*}
            En general, si $G$ es un grupo cuyo grupo derivado es resoluble, entonces $G$ será resoluble.
...
\begin{teo}[Caracterización de grupos resolubles para grupos finitos]\ \\
    ...
    \begin{proof}
        ...
        \begin{description}
            ...
            \item [$iv) \Longrightarrow i)$] Consideramos una serie normal con factores cíclicos (luego abelianos): % // TODO: Comprobar esta última implicación
                \begin{equation*}
                    G = G_0 \rhd G_1 \rhd \ldots \rhd G_r = \{1\}
                \end{equation*}
                Veamos que $G^{(k)} < G_k$, para todo $k\in \{1, \ldots, r\}$: como $G_{k-1}/G_k$ es abeliano por ser un factor, entonces por la Proposición~\ref{prop:abelianizado}:
                \begin{equation*}
                    [G_{k-1},G_{k-1}] = G^{(k)} < G_k
                \end{equation*}
                En particular:
                \begin{equation*}
                    G^{(r)} < G_r = \{1\}
                \end{equation*}
                Y tenemos ya que la serie derivada alcanza el mínimo:
                \begin{equation*}
                    G = G^0 \rhd G' \rhd \ldots \rhd G^{(r)} = \{1\}
                \end{equation*}
