...
\begin{ejemplo}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[node distance=2cm]
            \node (Z12) {$C_{12}$};
            \node (2) [below left of=Z12] {$\langle x^2 \rangle$};
            \node (3) [below right of=Z12] {$\langle x^3 \rangle$};
            \node (4) [below of=2] {$\langle x^4 \rangle$};
            \node (6) [below of=3] {$\langle x^6 \rangle$};
            ...
            \draw (2) -- node[above right] {$3$} (6);
        \end{tikzpicture}
        \caption{Diagrama de Hasse para los subgrupos de $\mathbb{C}_{12}$.}
        \label{fig:hasse_c12}
    \end{figure}
\end{ejemplo}
...
\begin{prop}
    ...
    \begin{equation*}
        \langle a^k \rangle  = \langle a^d \rangle \Longleftrightarrow d = \mcd(n,k)
    \end{equation*}
    En cuyo caso, $O(a^k) = \dfrac{n}{d}$.
    \begin{proof}
        Por doble inclusión:
        \begin{description}
            \item [$\subseteq)$] Como $d\mid k$, tenemos que $k = dt$, luego $a^k = a^{dt}\in \langle a^d \rangle $.
            \item [$\supseteq)$] Como $d = \mcd(n,k)$, entonces (por Bezout) existen $u,v$ de forma que $un+vk = d$, luego:
                \begin{equation*}
                    a^d = a^{un}a^{vk} = {(a^{k})}^{v} \in \langle a^k \rangle 
                \end{equation*}
        \end{description}
        Para ver que $O(a^k) = \dfrac{n}{d}$:
        \begin{equation*}
            O(a^k) = O(a^d)
        \end{equation*}
        Y como:
...
\begin{ejemplo}
    En $\mathbb{Z}_{12}$, ¿por qué no aparecía la clase del 8? Porque coincidía con la de 4, ya que:
    \begin{equation*}
        \langle 4\cdot \overline{2} \rangle = \langle \overline{8} \rangle  = \langle \overline{4} \rangle = \langle 2\cdot \overline{2} \rangle 
    \end{equation*}
    Y tenemos que $O(\overline{2}) = 6$, con $\mcd(6,4)=2$.
\end{ejemplo}

\begin{coro}
    Sea $G$ un grupo y $a\in G$ con $O(a) = n$, entonces
    \begin{equation*}
        \langle a^i \rangle  = \langle a^j \rangle  \Longleftrightarrow \mcd(n,i) = \mcd(n,j)
    \end{equation*}
\end{coro}
...
\begin{coro}
    ...
    \end{equation*}
    Es decir, el número de generadores de $G$ es el $\varphi$ de Euler:
    \begin{equation*}
...
\begin{ejemplo}
    EN $\mathbb{Z}_{12}$:
    \begin{equation*}
        \langle \overline{1} \rangle  = \langle \overline{5} \rangle  = \langle \overline{7} \rangle  = \langle \overline{11} \rangle 
    \end{equation*}
\end{ejemplo}

% // TODO: Meter por ahi que si G = <a> con O(a) = n <=> |G| = n
