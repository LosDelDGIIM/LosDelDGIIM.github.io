...
%     \item Si $n_2 = 1$ o $n_3 = 1$, tendremos en cualquier caso la existencia de un $p-$subgrupo de Sylow ($p\in \{2,3\}$) $K\lhd G$. Si consideramos su complemento, $H<G$, tendremos que:
%         \begin{equation*}
...
%                 \end{equation*}
%             \item En el caso $C_3\rtimes (C_2\times C_2)$ solo hay un homomorfismo no trivial, aquel que lleva un generador en $\alpha$ y el otro en la identidad; es decir, si $C_2\times C_2 = \langle x,y \rangle $, entonces tenemos:
%                 \begin{equation*}
%                     \theta(x) = \alpha \qquad \theta(y) = 1
%                 \end{equation*}
%                 Y tendremos que:
%                 \begin{equation*}
%                     C_3\rtimes_\theta (C_2\times C_2) = \langle x,y,z \mid x^3=y^2 = z^2 = 1, yxy^{-1}=x^{-1},zxz^{-1}=x, yzy^{-1}=zy \rangle 
%                 \end{equation*}
...
%     \item Tras no abelianos, $D_6$, $Q_3$ y $A_4$.
% \end{itemize}
