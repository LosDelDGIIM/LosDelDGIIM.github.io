...
Recordemos que en $\mathbb{R}^n$ un conjunto era acotado si y solo si cada una de sus proyecciones es un conjunto acotado. Este Corolario hace ese papel de este Corolario.
% // TODO: Estudiar la relacion con dicho resultado, cuando G sea un Banach de dimensión finita.

Este Corolario y el siguiente se usan mucho.
...
\begin{ejercicio}
    Sean $X,Y$ dos espacios de Banach, $T\in L(X,Y)$, definimos para cada $n\in \mathbb{N}$, para cada $y \in Y$:
    \begin{equation*}
        \|y\|_n := \inf\{\|u\| + n\|v\| : u\in X, v\in Y, y = T(u)+v\}
    \end{equation*}
