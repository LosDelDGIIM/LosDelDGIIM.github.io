...
\begin{ejercicio}\label{ej:5_rel1}
...
\begin{ejercicio}\label{ej:7_rel1}
...
\begin{ejercicio}
    ...
    \begin{description}
        \item [$1\Longrightarrow 2)$] Dados $\beta_1, \ldots, \beta_n\in \mathbb{R}$, como $E^\ast$ es un espacio vectorial, podemos considerar la aplicación lineal y continua:
            \begin{equation*}
        ...
        \item [$2\Longrightarrow 1)$] Démonos cuenta de que si definimos $\varphi:E\to \mathbb{R}^n$ dada por:
            \begin{equation*}
                \varphi(x) = (f_1(x), \ldots, f_n(x)) \qquad \forall x\in E
            \end{equation*}
            Y escribiendo también $\alpha = (\alpha_1, \ldots, \alpha_n)\in \mathbb{R}^n$ podemos reformular el enunciado preguntándonos si para todo $\varepsilon>0$ se tiene que:
            \begin{equation*}
                \alpha \in  \varphi\left(\overline{B}(0,M+\varepsilon)\right)
            \end{equation*}
            Por reducción al absurdo, supongamos que no:
            \begin{equation*}
                \alpha \notin  \varphi\left(\overline{B}(0,M+\varepsilon)\right)
            \end{equation*}
            Con vistas a aplicar el Ejercicio~\ref{ej:9_rel1}:
            \begin{itemize}
                \item Veamos primero que $\varphi$ es lineal, pues si $x,y\in E$ y $\lm\in \mathbb{R}$ tenemos que:
                    \begin{align*}
                        \varphi(\lm x + y) &= (f_1(\lm x+y), \ldots, f_n(\lm x + y)) \\ 
                                           &= (\lm f_1(x) + f_1(y) , \ldots, \lm f_n(x) + f_n(y)) = \lm \varphi(x) + \varphi(y)
                    \end{align*}
                \item Ahora veamos que $\varphi\left(\overline{B}(0,M+\varepsilon)\right)$ es un conjunto convexo, pues si tomamos $y,y'\in \varphi\left(\overline{B}(0,M+\varepsilon)\right)$ y $t\in [0,1]$, tenemos entonces que existen $x,x'\in \overline{B}(0,M+\varepsilon)$ de forma que $\varphi(x) = y$, $\varphi(x') = y'$. Como $\overline{B}(0,M+\varepsilon)$ es convexo tendremos que $tx + (1-t)x'\in \overline{B}(0,M+\varepsilon)$ y usando ahora que $\varphi$ es lineal:
                    \begin{equation*}
                        t\varphi(x) + (1-t)\varphi(x') = \varphi(tx+(1-t)x') \in \varphi(\overline{B}(0,M+\varepsilon))
                    \end{equation*}
                    De hecho, toda imagen de un conjunto convexo por una función lineal es convexa, y la prueba es la misma que acabamos de hacer.
            \end{itemize}
            Tenemos entonces que $\{\alpha\}$ y $\varphi(\overline{B}(0,M+\varepsilon))$ son dos conjuntos disjuntos y convexos de $\mathbb{R}^n$, con lo que aplicando el Ejercicio~\ref{ej:9_rel1} tenemos que existe $f:\mathbb{R}^n\to \mathbb{R}$ lineal y continua de forma que:
            \begin{equation*}
                f(x) \leq f(\alpha) \qquad \forall x\in \varphi\left(\overline{B}(0,M+\varepsilon)\right)
            \end{equation*}
            O escrito de otra forma, que:
            \begin{equation}\label{eq:ejercicio_11}
                \sup_{x\in \varphi\left(\overline{B}(0,M+\varepsilon)\right)}f(x) \leq f(\alpha)
            \end{equation}
            Como $\mathbb{R}^n$ es un espacio de Hilbert podemos aplicar el Teorema de Riesz-Fréchet, obteniendo que existe $\beta\in \mathbb{R}^n$ de forma que:
            \begin{equation*}
                f(x) = \langle x,\beta \rangle  \qquad \forall x\in \mathbb{R}^n
            \end{equation*}
            Si consideramos las componentes de $\beta$ en $\mathbb{R}^n$:
            \begin{equation*}
                \beta = (\beta_1, \ldots, \beta_n)
            \end{equation*}
            Para terminar la demostración veamos finalmente que:
            \begin{equation*}
                f\circ \varphi = \sum_{i=1}^{n}\beta_if_i
            \end{equation*}

            pues:
            \begin{equation*}
                (f\circ \varphi)(x) = \langle \varphi(x),\beta \rangle  = \sum_{i=1}^{n} f_i(x)\beta_i = \left(\sum_{i=1}^{n}f_i\beta_i\right)(x) \qquad \forall x\in E
            \end{equation*}
            \begin{description}
                \item [Si suponemos que $f_1, \ldots f_n$ son linealmente independientes.]~\\
                Tenemos entonces que:
                \begin{equation*}
                    \sum_{k=1}^{n}\beta_if_i \neq 0
                \end{equation*}
                Por lo que:
                \begin{align*}
                    M\left\|\sum_{i=1}^{n}\beta_if_i\right\| &< (M+\varepsilon)\left\|\sum_{i=1}^{n}\beta_if_i\right\| = (M+\varepsilon)\|f\circ \varphi\| \\ 
                                                             &\leq (M+\varepsilon)\sup_{x\in \overline{B}(0,1)} |(f\circ \varphi)(x)| \AstIg \sup_{x\in \overline{B}(0,1)} |(f\circ \varphi)((M+\varepsilon)x)| \\
                                                             &= \sup_{x\in \overline{B}(0,M+\varepsilon)}|(f\circ \varphi)(x)| \stackrel{(\ast\ast)}{\leq} f(\alpha) = \langle \alpha,\beta \rangle  = \sum_{i=1}^{n}\alpha_i \beta_i \leq \left|\sum_{i=1}^{n}\alpha_i \beta_i\right|
                \end{align*}
                Donde:
                \begin{itemize}
                    \item En $(\ast)$ usamos que $M+\varepsilon\geq 0$ y que tanto $f$ como $\varphi$ son lineales.
                    \item En $(\ast\ast)$ usamos la desigualdad~(\ref{eq:ejercicio_11}).
                \end{itemize}
                Es decir, hemos encontrado $\beta_1, \ldots, \beta_n\in \mathbb{R}$ de forma que:
                \begin{equation*}
                    M\left\|\sum_{i=1}^{n}\beta_if_i\right\| < \left|\sum_{i=1}^{n}\alpha_i \beta_i\right|
                \end{equation*}
                Lo que contradice 2, por lo que:
                \begin{equation*}
                    \alpha \in \varphi(\overline{B}(0,M+\varepsilon)) \qquad \forall \varepsilon>0
                \end{equation*}
                Que era lo que queríamos probar.
                \item [Si no son linealmente independientes.]~\\
                    Tomamos de $\{f_1, \ldots, f_n\}$ un conjunto maximal (en cuanto a número de elementos) que sí sea linealmente independiente, supondremos sin pérdida de generalidad (salvo enumeración de los $f_i$) que $\{f_1, \ldots, f_m\}$ para $m\geq 1$ es un conjunto maximal linealmente independiente, por lo que $f_{m+1},\ldots, f_n$ pueden obtenerse como combinación lineal de elementos de $\{f_1, \ldots, f_m\}$, por lo que:
                    \begin{equation*}
                        f_j = \sum_{i=1}^{m} \lm_{j,i} \cdot f_i\qquad \lm_{j,1},\ldots,\lm_{j,n}\in \mathbb{R}, \quad \forall  j \in \{m+1,\ldots,n\}
                    \end{equation*}
                    Y realizando un razonamiento análogo al caso anterior tomando:
                    \begin{equation*}
                        \alpha_j = \sum_{i=1}^{m} \lm_{j,i}\cdot \alpha_i, \qquad \beta_j = \sum_{i=1}^{m} \lm_{j,i}\cdot \beta_i \qquad \forall j \in \{m+1,\ldots,n\}
                    \end{equation*}
                    Podemos llegar a probar la implicación que queremos.
            \end{description}
    \end{description}
\end{ejercicio}
...
\begin{ejercicio}
    ...
    \noindent
    \begin{description}
        \item [Probamos que $H^{\perp}\cap \Int P \neq \emptyset $.]~\\
            Para ello, por reducción al absurdo, supongamos que $M^\perp\cap \Int P=\emptyset $. En estas condiciones, vemos que:
            \begin{itemize}
                \item $\Int P$ es un conjunto convexo por ser $P$ un conjunto convexo y aplicar el Ejercicio~\ref{ej:7_rel1}. Además es claramente un conjunto abierto.
                \item $M^\perp$ es un conjunto convexo ya que, de hecho, es un espacio vectorial\footnote{Lo sabemos de Geometría II.}.
            \end{itemize}
            Podemos aplicar\footnote{De hecho bastaría aplicar el Ejercicio~\ref{ej:9_rel1}, ya que estamos trabajando en dimensión finita.} el Teorema de Hahn-Banach (la primera versión geométrica), obteniendo una aplicación $f:\mathbb{R}^n\to \mathbb{R}$ lineal y continua que no se anula en todo $\mathbb{R}^n$ de forma que:
            \begin{equation*}
                f(x) \leq f(y) \qquad \forall x\in M^\perp, \quad \forall y\in \Int P
            \end{equation*}
            Observamos ahora que:
            \begin{itemize}
                \item $0\in M^\perp$, por lo que:
                    \begin{equation*}
                        0 = f(0) \leq f(y) \qquad \forall y\in \Int P
                    \end{equation*}
                \item Podemos tomar una sucesión $\{y_n\}$ de puntos de $\Int P$ convergente a $0$ (por ejemplo, podemos tomar $\{(\nicefrac{1}{n}, \ldots, \nicefrac{1}{n})\}$) y como:
                    \begin{equation*}
                        f(x) \leq f(y_n) \qquad \forall x\in M^\perp \quad \forall n\in \mathbb{N}
                    \end{equation*}
                    tendremos entonces que $f(x) \leq 0$ para todo $x\in M^\perp$.
            \end{itemize}
            Es decir:
            \begin{equation*}
                f(x) \leq 0 \leq f(y) \qquad \forall x\in M^\perp, \quad \forall y\in \Int P
            \end{equation*}
            En este punto, aplicamos que:
            \begin{center}
                Si $f:V\to W$ es una aplicación lineal entre espacios vectoriales (con $W$ espacio normado) y $f(V)$ está acotado, entonces $f(V) = \{0\}$.
            \end{center}
            \begin{proof}
                Si existiese $u\in V$ de forma que $v = f(u)\neq 0$, entonces tendríamos que el conjunto:
                \begin{equation*}
                    \{f(\lm u) : \lm \in \mathbb{R}\} = \{\lm f(u) : \lm \in \mathbb{R}\} = \{\lm v : \lm \in \mathbb{R}\}
                \end{equation*}
                estaría acotado ($f(V)$ está acotado), pero esto solo puede pasar si $v = 0$.
            \end{proof}
            Por lo que obtenemos que de hecho $f(x) = 0 \quad \forall x\in M^\perp$. Aplicando el Teorema de Riesz-Fréchet obtenemos $\beta\in \mathbb{R}^n$ de forma que:
            \begin{equation*}
                f(x) = \langle x,\beta \rangle  \qquad \forall x\in \mathbb{R}^n
            \end{equation*}
            Y observamos ahora que:
            \begin{itemize}
                \item $\beta\in M$, puesto que:
                    \begin{equation*}
                        0 = f(x) = \langle x,\beta \rangle  \qquad \forall x\in M^\perp
                    \end{equation*}
                    Por lo que $\beta\in {(M^\perp)}^{\perp} = M$.
                \item $\beta\in P$, puesto que si consideramos la base usual de $\mathbb{R}^n$:
                    \begin{equation*}
                        \{e_1, \ldots, e_n\} \subset \Int P = \{x\in \mathbb{R}^n : x_i > 0 \quad \forall i \in \{1,\ldots,n\}\}
                    \end{equation*}
                    tenemos entonces que:
                    \begin{equation*}
                        \beta_i = \langle \beta,e_i \rangle  = f(e_i) \geq 0 \qquad \forall i \in \{1,\ldots,n\}
                    \end{equation*}
                    De donde $\beta\in P$.
            \end{itemize}
            Tenemos por tanto que $\beta\in M\cap P = \{0\}$, de donde ha de ser $f=0$, pero $f$ no se podía anular en todo $\mathbb{R}^n$, luego hemos llegado a una \underline{contradicción}.
        \item [Terminamos la demostración]~\\
            Comenzando ahora la demostración con la hipótesis $M^\perp \cap \Int P \neq \emptyset $, podemos tomar $z\in M^\perp\cap \Int P$, por lo que:
            \begin{itemize}
                \item Tendremos que $\langle x,z \rangle  = 0 \quad \forall x\in M$.
                \item Tendremos que $z_i > 0 \quad \forall  i \in \{1,\ldots,n\}$.
            \end{itemize}
            Si definimos la aplicación lineal y continua $f:\mathbb{R}^n \to \mathbb{R}$ dada por:
            \begin{equation*}
                f(x) = \langle x,z \rangle  \qquad \forall x\in \mathbb{R}^n
            \end{equation*}
            Y consideramos el hiperplano $H = [f=0] = \ker f$, tenemos:
            \begin{itemize}
                \item Por una lado:
                    \begin{equation*}
                        H = \ker f = \{x\in \mathbb{R}^n : f(x) = 0\} = \{x\in \mathbb{R}^n : \langle x,z \rangle = 0 \} \supset M
                    \end{equation*}
                \item Por otro, si $y\in H\cap P$ tenemos entonces que $y_1,\ldots,y_n\geq 0$ por ser $y \in P$ y por ser $y\in M$ tenemos que:
                    \begin{equation*}
                         0 = f(y) = \langle y,z \rangle  = \sum_{i=1}^{n}y_iz_i
                    \end{equation*}
                    Pero como $z_i>0 \quad \forall i \in \{1,\ldots,n\}$ y las coordenadas de $y$ no son negativas, esto solo puede suceder si $y_i = 0 \quad \forall  i \in \{1,\ldots,n\}$ es decir, si $y = 0$.
            \end{itemize}
    \end{description}
\end{ejercicio}
        ...
        \item Prueba que $X$ e $Y$ son subespacios cerrados de $E$ y que $\overline{X+Y} = E$.
            ...
            \begin{itemize}
                ...
                \item Es claro que $Y$ es un subespacio vectorial de $E$. Para ver que es cerrado, consideramos la aplicación $G:E\to E$ dada por:
                    ...
                    \end{equation*}
                    Tenemos que $G$ está bien definida ($G(y) \in E$ para cada $y\in E$), puesto que si $y \in E$ y $z = G(y)$, entonces:
                    \begin{align*}
