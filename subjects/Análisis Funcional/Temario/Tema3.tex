...
\begin{prop}\label{prop:cierre_esfera}
    ...
    \end{equation*}
    \begin{proof}
        Lo más difícil es probar que:
        \begin{equation*}
            B \subset \overline{S}^{\sigma(E,E^\ast)}
        \end{equation*}
        Una vez probado, usando que $S\subset \overline{S}^{\sigma(E,E^\ast)}$ tenemos que:
        \begin{equation*}
            \overline{B}\subset \overline{S}^{\sigma(E,E^\ast)}
        \end{equation*}
        Por otra parte, veamos que $\overline{S}^{\sigma(E,E^\ast)} \subset \overline{B}$.\\ % // TODO: Ejercicio, tenemos que probar que \ol{B} es sigma(E,E^\ast)-cerrado (débilmente cerrado). Es consecuencia de Hahn-Banach
        ...
        \begin{equation*}
            B \subset \overline{S}^{\sigma(E,E^\ast)}
        \end{equation*}
            Sea $x_0\in B$, si consideramos $V$ un entorno de $x_0$ en la topología débil, tenemos que existen $f_1,\ldots,f_k\in E^\ast$ y $\varepsilon>0$ de forma que:
            \begin{equation*}
            ...
            \end{equation*}
            que es una función continua, con $g(0) = \|x_0\| < 1$, así como que $\lim\limits_{n\to\infty}g(t) = \infty$. % // TODO: Ejercicio, probar 
            ...
            \end{equation*}
            donde en $(\ast)$ usamos que $y_0\in \ker\psi$. En definitiva, tenemos que $x_0+t_0y_0 \in S\cap V$, donde $V$ era un entorno arbitrario de $x_0$ para la topología débil. Hemos probado que $x_0\in \overline{S}^{\sigma(E,E^\ast)}$ para cada $x_0\in B$, por lo que tenemos la inclusión.
    \end{proof}
\end{prop}
    ...
    \begin{proof}
        ...
        \begin{description}
            \item [$\Longrightarrow )$] La hemos discutido anteriormente, pues se tiene que:
                ...
                \end{equation*}
            \item [$\Longleftarrow )$]  Si $A$ es un subconjunto de $E$ que es cerrado y convexo, queremos ver que es $\sigma(E,E^\ast)-$cerrado. Para ello, veamos que $E\setminus A$ es débilmente abieto. Para esto último, si tomamos $x_0\in E\setminus A$ por la segunda versión geométrica del Teorema de Hahn-Banach tenemos que existen $f\in E^\ast$  y $\alpha\in \mathbb{R}$ tales que:
                \begin{equation*}
                ...
                \end{equation*}
                con $f^{-1}(\alpha)$ un conjunto $\sigma(E,E^\ast)-$abierto, por la definición de $\sigma(E,E^\ast)$. Como $f^{-1}(\alpha)\cap A = \emptyset $, tenemos que:
                \begin{equation*}
...
\begin{coro}[Teorema de Mazur]
    ...
    \begin{enumerate}
        ...
        \item $\{y_n\}\to x$.
    \end{enumerate}
    % \begin{proof} % // TODO: EJERCICIO
    % {x_k} tiene a x como punto de acumulacion en la top debil, por lo que x estará en el cierre debil de Conv(x_k : k ii N), por lo que el cierre de Conv(x_k : k ii N) es debilmente cerrado. Demostrar que la envolvente convexa se obtiene haciendo conbinaciones convexas finitas de {x_k : k ii N}.
    % \end{proof}
\end{coro}
