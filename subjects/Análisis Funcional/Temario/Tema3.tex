...
\begin{ejercicio}
    ...
    \begin{proof}
        ...
        \begin{description}
            ...
            \item [$3\Longrightarrow 1)$] Si tomamos $\{x_n\}\to x$ con $\{x_n\}$ una sucesión de puntos de $E$, tendremos entonces que $\{x_n\}\rightharpoonup x$, y usando ahora la hipótesis de que la aplicación\newline $T:(E,\sigma(E,E^\ast))\to (F,\sigma(F,F^\ast))$ es continua, tendremos entonces que\newline $\{T(x_n)\}\rightharpoonup T(x)$. En esta situación, para toda $f\in F^\ast$ tenemos que\newline $\{f(T(x_n))\}\to f(T(x))$, por lo que por ser $\sigma(F,F^\ast)$ la topología inicial asociada a la familia $F^\ast$ tenemos que $\{T(x_n)\}\to T(x)$, lo que prubea que $T:(E,\tau_{\|\cdot \|_E})\to (F,\tau_{\|\cdot \|_F})$ es continua.
        \end{description}
    \end{proof}
\end{ejercicio}
