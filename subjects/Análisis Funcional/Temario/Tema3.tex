...
\begin{ejercicio}
    ...
    \begin{enumerate}
        \item $T:(E,\tau_{\|\cdot \|})\to (F,\tau_{\|\cdot \|})$ es continua.
        \item $T:(E,\tau_{\|\cdot \|})\to (F,\sigma(F,F^\ast))$ es continua.
        \item $T:(E,\sigma(E,E^\ast)) \to (F,\sigma(F,F^\ast))$ es continua.
    \end{enumerate}
\end{ejercicio}

\begin{ejercicio}
    Sea $E$ un espacio normado, tenemos que $E^\ast$ es de Banach, con lo que podemos considerar su topología débil $\sigma(E^\ast,E^{\ast\ast})$. Como siempre teníamos una inyección canónica
    \Func{J}{E}{E^{\ast\ast}}{x}{J(x)}

    donde teníamos:
    \Func{J(x)}{E^\ast}{\bb{R}}{f}{\langle f,x\rangle}
    La topología débil $\ast$ es $\sigma(E^\ast,J(E))$, que tiene una relación con $\sigma(E^\ast,E^{\ast\ast})$
    Probar:
    \begin{enumerate}
        \item Es Hausdorff.
        \item Cuales son los entornos básicos.
        \item Cómo es la convergencia.
    \end{enumerate}
\end{ejercicio}
