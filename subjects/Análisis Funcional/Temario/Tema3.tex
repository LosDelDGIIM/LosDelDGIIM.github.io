    ...
    \begin{proof}
        ...
        \begin{description}
            \item [$\subseteq )$] Para esta inclusión, como tenemos que $S\subset \overline{B}$, basta probar que $\overline{B}$ es débilmente cerrado, con lo que:
                ...
                \end{equation*}
                \begin{description}
                    \item [Opción 1.] Si tenemos una sucesión de puntos de $\overline{B}$ $\{x_n\}\rightharpoonup x$ con $x\in E$, vimos entonces que $\|x\|\leq \liminf \|x_n\|$, pero como $\|x_n\|\leq 1 \quad \forall n\in \mathbb{N}$ tendremos entonces que:
                        \begin{equation*}
                            \|x\| \leq \liminf \|x_n\| \leq 1
                        \end{equation*}
                        Por lo que $x\in \overline{B}$, de donde $\overline{B}$ es $\sigma(E,E^\ast)-$cerrado.
                    \item [Opción 2.] Si tenemos $\{x_n\}\rightharpoonup x$ con $x_n\in \overline{B}\quad \forall n\in \mathbb{N}$ y $x\in E$, el Corolario~\ref{coro:calcular_norma_x} del Teorema de Hahn-Banach nos dice que:
                        \begin{equation*}
                            \|x\| = \sup_{\|f\|\leq 1}|f(x)|
                        \end{equation*}
                        Si tomamos $f\in E^\ast$ con $\|f\| \leq 1$ tenemos por la convergencia débil que $\{f(x_n)\}\to f(x)$, y tenemos:
                        \begin{equation*}
                            |f(x_n)| \leq \|f\|\|x_n\| \leq 1 \qquad \forall n\in \mathbb{N}
                        \end{equation*}
                        Por lo que también tenemos que $|f(x)|\leq 1$, para toda $f\in E^\ast$ que verifique $\|f\|\leq 1$, luego $\|x\|\leq 1$, de donde $x\in \overline{B}$, por lo que $\overline{B}$ es $\sigma(E,E^\ast)-$cerrado.
                \end{description}
    ...
    \begin{proof}
        ...
        \begin{description}
            \item [$\Longrightarrow )$] La hemos discutido anteriormente, pues se tiene que:
                ...
                \end{equation*}
            \item [$\Longleftarrow )$]  Si $A$ es un subconjunto de $E$ que es cerrado y convexo, queremos ver que es $\sigma(E,E^\ast)-$cerrado. Para ello, veamos que $E\setminus A$ es débilmente abieto. Para esto último, si tomamos $x_0\in E\setminus A$ tenemos por la segunda versión geométrica del Teorema de Hahn-Banach ($\{x_0\}$ es compacto y $A$ cerrado con $x_0\notin A$) tenemos que existen $f\in E^\ast$  y $\alpha\in \mathbb{R}$ tales que:
                \begin{equation*}
                    \langle f,x_0 \rangle < \alpha < \langle f,x \rangle  \qquad \forall x\in A
                \end{equation*}
                Tenemos por tanto que:
                \begin{equation*}
                    x_0 \in \{y\in E : \langle f,y \rangle <\alpha\} = f^{-1}(\alpha)
                \end{equation*}
                con $f^{-1}(\alpha)$ un conjunto $\sigma(E,E^\ast)-$abierto\footnote{También podríamos haber dicho que $f^{-1}(\alpha) = V(f;\alpha)$.}, por la definición de $\sigma(E,E^\ast)$. Como $f^{-1}(\alpha)\cap A = \emptyset $, tenemos que:
                \begin{equation*}
                    x_0 \in f^{-1}(\alpha) \subset E\setminus A
                \end{equation*}
...
\begin{teo}
    ...
    \end{enumerate}
    \begin{proof}
        Demostremos todas las implicaciones: % // TODO: METER OPCIONES
        \begin{description}
            \item [$1\Longrightarrow 2)$] Si tomamos $U\in \sigma(F,F^\ast)\subset \tau_{\|\cdot \|_F}$ tendremos entonces que $T^{-1}(U)\in \tau_{\|\cdot \|_E}$.
            \item [$1\Longrightarrow 2)$] Si $\{x_n\}\to x$, entonces $\{f(x_n)\}\to f(x)$, de donde $\{f(x_n)\}\rightharpoonup f(x)$.
            \item [$1\Longrightarrow 2)$] Si tomo $f\in F^\ast$, tenemos $f:(F,\tau_{\|\cdot \|_F})\to \mathbb{R}$ continua, por lo que $f\circ T:(E,\tau_{\|\cdot \|_E})\to \mathbb{R}$ es continua.
            \item [$2\Longrightarrow 3)$] Sea $f\in F^\ast$, tenemos que:
                \begin{equation*}
            ...
            \item [$2\Longrightarrow 3)$] Si $U\in \sigma(F,F^\ast)$, tenemos entonces que:
                \begin{equation*}
                ...
                \end{equation*}
            \item [$2\Longrightarrow 3)$] $T:(E,\tau_{\|\cdot \|_E})\to (F,\sigma(F,F^\ast))$ es continua si y solo si $f\circ T:(E,\tau_{\|\cdot \|_E})\to \mathbb{R}$ es contiua para toda $f\in F^\ast$, si y solo is $f\circ T\in E^\ast$ para toda $f\in F^\ast$, si y solo sí (por la definición de topología débil) $f\circ T:(E,\sigma(E,E^\ast))\to\mathbb{R}$ es continua para toda $f\in F^\ast$, que ocurre si y solo si $T:(E,\sigma(E,E^\ast))\to(F,\sigma(F,F^\ast))$ es continua.
            \item [$3\Longrightarrow 1)$] Si tomamos una sucesión de la gráfica convergente, $\{(x_n,T(x_n))\}\to (x,y)$, tenemos:
                \begin{equation*}
            ...
            \item [$3\Longrightarrow 1)$] Si consideramos:
                \begin{equation*}
                    Gr(T) = \{(x,Tx) : x\in E\}\subset E\times F
                \end{equation*}
                Si $T$ es continua, entonces $Gr(T)$ es cerrado en $(E\times F, \sigma(E,E^\ast)\times \sigma(F,F^\ast))$. Puede probarse que: % // TODO: EJERCICIO
                \begin{equation*}
...
\begin{coro}
    Si $H$ es un hiperplano en $E^\ast$ que es $\sigma(E^\ast,E)-$cerrado, entonces:
    \begin{equation*}
...
\begin{teo}
    Sea $E$ un espacio de Banach:
    \begin{enumerate}
        \item[$i)$] Si $E$ es separable, entonces $\overline{B}_{E^\ast}$ es metrizable en la topología débil$-\ast$.
        \item[$ii)$] Si $\overline{B}_{E}$ es metrizable en la topología débil, entonces $E$ es separable.
    \end{enumerate}
    \begin{proof}
        Demostramos cada apartado:
        \begin{enumerate}
            \item[$i)$] Como $E$ es separable, será también $\overline{B}_{E}$ un conjunto separable, luego existe $\{x_n:n\in \mathbb{N}\}\subset \overline{B}_E$ denso. Definimos ahora para cada $f\in E^\ast$ con $\|f\| \leq 1$:
                \begin{equation*}
                    [f] = \sum_{n=1}^{\infty} \frac{1}{2^n} |\langle f,x_n \rangle |
                \end{equation*}
                La aplicación está bien definida, puesto que:
                \begin{equation*}
                    \frac{1}{2^n}|\langle f,x_n \rangle| \leq \frac{1}{2^n}\|f\| \|x_n\| \leq \frac{1}{2^n} \qquad \forall n\in \mathbb{N}
                \end{equation*}
                Por lo que la serie $\sum\limits_{n=1}^{\infty}\frac{1}{2^n}|\langle f,x_n \rangle| $ convergente en $\mathbb{R}$. Vemos ahora que:
                \begin{itemize}
                    \item $[f]$ es una norma en $\{f\in E^\ast : \|f\|\leq 1\}$:
                        \begin{itemize}
                            \item Si tenemos $f\in E^\ast$ con $\|f\|\leq 1$ de forma que:
                                \begin{equation*}
                            ...
                            \item Si $f,g\in E^\ast$ con $\|f\|,\|g\|\leq 1$ tenemos entonces que:
                                \begin{align*}
                                ...
                                \end{align*}
                            \item Si $f\in E^\ast$ con $\|f\|\leq 1$ y considero $\lm\in \mathbb{R}$, tenemos que:
                                \begin{equation*}
                ...
                \begin{itemize} % // TODO: HACER:
                    \item Probar que $[f]\leq \|f\|$.
                    \item Probar que $(\overline{B}_{E^\ast}, \tau_{[\cdot ]}) = \left(\overline{B}_{E^\ast}, \sigma(E^\ast,E)\big|_{\overline{B}_{E^\ast}}\right)$.
                \end{itemize} % Para supset, coger un abierto, meter un entorno básico, que será abierto de la izuiqerda, basta hacerlo para los entornos básicos, que tienen que ver con fijar un nº de funcionales y \ldots, que se controla con [f]
            \item[$ii)$] % // TODO: HACER
        \end{enumerate}
    \end{proof}
\end{teo}
