...
\begin{ejercicio}\label{ej:lineal_debil*}
...
% // TODO: Podemos preparar teo de kakutani
% // TODO: Esto está antes del Teo de Kakutani, es para el ej 3.24, que es a partir del Lema de Goldstine

\begin{lema}[de Goldstine]\label{lema:Goldstine}
    Sea $E$ un espacio de Banach. Entonces $J(\overline{B}_E)$ es denso en $\overline{B}_{E^{\ast\ast}}$ para $\sigma(E^{\ast\ast},E^\ast)$.
    \begin{proof}
        Dado $\xi \in \overline{B}_{E^{\ast\ast}}$, tomamos un entorno de la base de entornos cualquiera:
        \begin{equation*}
            V = \{\eta \in E^{\ast\ast} : |\langle \eta-\xi, f_i \rangle |<\varepsilon, \quad i \in \{1,\ldots,k\}\} \qquad f_1,\ldots,f_k\in E^\ast, \quad \varepsilon>0
        \end{equation*}
        Y queremos probar que $V\cap J(\overline{B}_E)\neq \emptyset $, es decir, que existe $x\in \overline{B}_E$ de forma que $Jx\in V$ con 
        \begin{equation*}
            |\langle Jx-\xi,f_i \rangle |<\varepsilon \qquad \forall i \in \{1,\ldots,k\}
        \end{equation*}
        si definimos:
        \begin{equation*}
            \gamma_i = \langle \xi, f_i \rangle  \qquad \forall i \in \{1,\ldots,k\}
        \end{equation*}
        Queremos ver entonces que:
        
        Por reduccion al absurdo, supongamos que $\nexists x\in \overline{B}_E$ de forma que:
        \begin{equation*}
            |\langle Jx-\xi,f_i \rangle |<\varepsilon\qquad \forall i \in \{1,\ldots,k\}
        \end{equation*}
        de donde para todo $x\in \overline{B}_E$ tendremos que:
        \begin{equation*}
            |\langle Jx-\xi,f_i \rangle | \geq \varepsilon>0 \qquad \forall i \in \{1,\ldots,k\}
        \end{equation*}

        De donde $\gamma = (\gamma_1, \ldots, \gamma_k)\notin \phi(\overline{B}_E)$ y tenemos:
        \begin{equation*}
            \phi(x) = (\langle f_1,x \rangle , \ldots, \langle f_k,x \rangle )
        \end{equation*}
        Tenemos:
        \begin{itemize}
            \item $\overline{\phi(\overline{B}_E)}$ es cerrado y convexo.
            \item $\{\xi\}$ es compacto y cerrado.
            \item $\{\xi\}\cap \overline{\phi(\overline{B}_E)} = \emptyset $
        \end{itemize}
        Por la segunda veresión geométrica de Hahn-Banach tenemos que existe $\beta\in \mathbb{R}^k$ y $\alpha\in \mathbb{R}$ de forma que:
        \begin{equation*}
            \beta\phi(x) < \alpha < \beta \gamma \qquad \forall x\in \overline{B}_E
        \end{equation*}
        Y tenemos:
        \begin{gather*}
            \beta\phi(x) = \sum_{i=1}^{k} \beta_i \langle f_i,x \rangle   = \left\langle \sum_{i=1}^{k}\beta_if_i,x \right\rangle  = \langle g,x \rangle < \alpha \\
            \beta\gamma = \langle \xi, \sum_{i=1}^{k}\beta_if_i \rangle  = \langle \xi,g \rangle
        \end{gather*}
        donde tomamos $g = \sum\limits_{i=1}^{k}\beta_if_i$. De la primera desigualdad tenemos que $\|g\|<\alpha$ y así:
        \begin{equation*}
            \|g\| \leq \alpha < \langle \xi,g \rangle \leq \|\xi\|\|g\| \leq \|g\|
        \end{equation*}
        y tenemos la \underline{contradicción}.
    \end{proof}
\end{lema}
...
\begin{prop}
    ...
    \begin{proof}
        ...
        es sobreyectiva. Dada $\xi \in {(l^p)}^{\ast\ast}$, como tenemos que ${(l^p)}^{\ast}\cong l^q$ para:
        \begin{equation*}
            \frac{1}{p} + \frac{1}{q} = 1
        \end{equation*}
...
\section{Espacios uniformemente convexos} % // TODO: Completar esta sección
\begin{definicion}
    Un espacio de Banach $E$ se dice que es uniformemente convexo si $\forall \varepsilon>0~\exists \delta>0$  de forma que:
    \begin{equation*}
        x,y\in E \text{\ con\ } \|x\|,\|y\| \leq 1 \text{\ y\ } \|x-y\|>\varepsilon \quad\Longrightarrow\quad \left\|\frac{x+y}{2}\right\| < 1-\delta
    \end{equation*}
\end{definicion}

% // TODO: QUizas meter intuicion

\begin{teo}[de Milman-Pettis]
    Todo espacio de Banach uniformemente convexo es reflexivo.
    \begin{proof} 
        Sea $E$ un espacio de Banach uniformemente conexo, queremos ver que la aplicación
        \Func{J}{E}{E^{\ast\ast}}{x}{Jx}
        es sobreyectiva. Para ello, basta ver que si $\xi \in E^{\ast\ast}$ con $\|\xi\| = 1$ entonces podemos encontrar $x\in E$ con $J(x) = \xi$, puesto que si $\varphi \in E^{\ast\ast}$ con $\|\varphi\|\neq 0$ (si $\|\varphi\| = 0 \Longrightarrow \varphi = 0 = J(0)$) tomando $\xi = \frac{\varphi}{\|\varphi\|}$ tenemos que $\xi \in E^{\ast\ast}$ con $\|\xi\|=1$, pudiendo encontrar $x\in E$ con:
        \begin{equation*}
            J(x) = \frac{\varphi}{\|\varphi\|} \quad\Longrightarrow\quad J(\|\varphi\| x) = \varphi
        \end{equation*}
        Sea pues $\xi \in E^{\ast\ast}$ con $\|\xi\| = 1$, buscamos comprobar que\footnote{Notemos que como $J$ es una isometría el elemento $x\in E$ buscado tiene que tener módulo 1.} $\xi \in J\left(\overline{B}_E\right)$. Para ello, observemos que $J\left(\overline{B}_E\right)$ es un conjunto cerrado, pues $\overline{B}_E\subset E$ es un conjunto cerrado, luego será un espacio completo (por ser subespacio cerrado de un Banach) y por tanto será también $J\left(\overline{B}_E\right)$ un espacio completo, por ser $J$ una isometría, de donde $J\left(\overline{B}_E\right)$ es cerrado, ya que todos los subespacios completos de un espacio completo son cerrados. De esta forma, para ver que $\xi \in J\left(\overline{B}_E\right)$ es suficiente ver que:
        \begin{equation}\label{eq:milman-pettis}
            \forall \varepsilon>0~\exists x\in \overline{B}_E \quad \text{con}\quad  \|\xi-J(x)\| \leq \varepsilon
        \end{equation}
        Para ello, fijado $\varepsilon>0$, tomamos $\delta>0$ dado por ser $E$ uniformemente conexo, como tenemos que:
        \begin{equation*}
            1 = \|\xi\| = \sup_{\|f\|\leq 1}|\langle \xi,f \rangle | = \sup_{\|f\| =1}|\langle \xi,f \rangle |
        \end{equation*}
        para $1-\nicefrac{\delta}{2}<1$ tenemos por la caracterización de supremo que existe $f\in E^\ast$ con $\|f\| = 1$ y:
        \begin{equation*}
            \langle \xi,f \rangle > 1-\frac{\delta}{2}
        \end{equation*}
        Si tomamos:
        \begin{equation*}
            V = V\left(f;\frac{\delta}{2}\right) = \left\{\eta \in E^{\ast\ast} : |\langle \eta-\xi,f \rangle |<\frac{\delta}{2}\right\}
        \end{equation*}
        tenemos que $V$ es un entorno de $\xi$ en la topología $\sigma(E^{\ast\ast},E^\ast)$. El Lema de Goldstine (Lema~\label{lema:Goldstine}) nos dice que $J\left(\overline{B}_E\right)$ es un conjunto denso en $\overline{B}_{E^{\ast\ast}}$ para $\sigma(E^{\ast\ast},E^\ast)$, por lo que:
        \begin{equation*}
            V\cap J\left(\overline{B}_E\right) \neq \emptyset 
        \end{equation*}
        de donde podemos tomar $x\in \overline{B}_E$ con $J(x) \in V$. Veamos que este elemento $x$ es el que nos da la condición~\eqref{eq:milman-pettis}. Por reducción al absurdo, supongamos que:
        \begin{equation*}
            \|\xi - J(x)\| > \varepsilon
        \end{equation*}
        O, equivalentemente, que:
        \begin{equation*}
            \xi \in {(\overline{B}(J(x),\varepsilon))}^{c}
        \end{equation*}
        Si tomamos $W = {(\overline{B}(J(x),\varepsilon))}^{c}$, tenemos que $W$ es $\sigma(E^{\ast\ast},E^\ast)-$abierto, pues:
        \begin{center}
            $W$ es $\sigma(E^{\ast\ast},E^\ast)-$abierto $\Longleftrightarrow \overline{B}(J(x),\varepsilon)$ es $\sigma(E^{\ast\ast},E^\ast)-$cerrado $\Longleftrightarrow \overline{B}_{E^{\ast\ast}}$ es $\sigma(E^{\ast\ast},E^\ast)-$cerrado
        \end{center}
        Y el Teorema de Banach-Alaoglu-Bourbaki nos dice que $\overline{B}_{E^{\ast\ast}}$ es $\sigma(E^{\ast\ast},E^\ast)-$compacto, luego $\sigma(E^{\ast\ast},E^\ast)-$cerrado, puesto que $\sigma(E^{\ast\ast},E^\ast)$ es Hausdorff. Así, tenemos que $W$ es también un entorno de $\xi$ en $\sigma(E^{\ast\ast},E^\ast)$, por lo que $V\cap W$ es un entorno de $\xi$ para $\sigma(E^{\ast\ast},E^\ast)$, como intersección de entornos. Aplicando de nuevo el Lema de Goldstine tenemos que:
        \begin{equation*}
            V\cap W \cap J\left(\overline{B}_E\right) \neq \emptyset 
        \end{equation*}
        Por lo que existe $y\in \overline{B}_E$ con $J(y) \in V\cap W$. Como tenemos que $J(x),J(y)\in W$ deducimos que:
        \begin{align*}
            |\langle f,x \rangle -\langle \xi,f \rangle | &= |\langle Jx,f \rangle -\langle \xi,f \rangle | = |\langle Jx-\xi,f \rangle | < \frac{\delta}{2} \\
            |\langle f,y \rangle -\langle \xi,f \rangle | &= |\langle Jx,f \rangle -\langle \xi,f \rangle | = |\langle Jy-\xi,f \rangle | < \frac{\delta}{2} 
        \end{align*}
        de donde:
        \begin{equation*}
            2\langle \xi,f \rangle  = \langle \xi,f \rangle + \langle \xi,f \rangle < \langle f,x \rangle  + \frac{\delta}{2} + \langle f,y \rangle  + \frac{\delta}{2} = \langle f,x+y \rangle  + \delta \leq \cancelto{1}{\|f\|}\|x+y\| + \delta 
        \end{equation*}
        y si usamos ahora que $\langle \xi,f \rangle > 1-\frac{\delta}{2}$ tenemos que:
        \begin{equation*}
            2-\delta < 2\langle \xi,f \rangle  < \|x+y\| + \delta \quad\Longrightarrow\quad 1-\frac{\delta}{2} < \left\|\frac{x+y}{2}\right\| + \frac{\delta}{2}
        \end{equation*}
        Por lo que:
        \begin{equation*}
            \left\|\frac{x+y}{2}\right\| \geq 1-\delta
        \end{equation*}
        y como $E$ es uniformemente conexo podemos usar el contrarrecíproco de la condición de ser uniformemente conexo, y como $\|x\|,\|y\| \leq 1$ tenemos que tiene que ser:
        \begin{equation*}
            \|x-y\| \leq \varepsilon
        \end{equation*}
        Pero teníamos que $J(y) \in W = {(\overline{B}(J(x),\varepsilon))}^{c}$, por lo que:
        \begin{equation*}
            \varepsilon < \|J(y)-J(x)\| = \|J(y-x)\| \AstIg \|y-x\|
        \end{equation*}
        donde en $(\ast)$ usamos que $J$ es una isometría, hemos llegado a una contradicción, que viene de suponer que el elemento $x$ que habíamos obtenido no cumplía la condición~\eqref{eq:milman-pettis}, por lo que tenemos el Teorema demostrado.
    \end{proof}
\end{teo}

% // TODO: Todo espacio de hilbert es uniformemente convexo, usando la identidad del paralelogramo
% // TODO: Se deja como ejercicio
% Luego hilbert => reflexivo
