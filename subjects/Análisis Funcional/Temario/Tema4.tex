...
\begin{itemize}
    ...
    \item Además, como en un espacio completo tenemos que un subespacio es completo si y solo si es cerrado, deducimos que $T(L^{p'}(\Omega))$ es un subespacio cerrado de $L^p(\Omega)^\ast$.
...
\begin{ejercicio} % // TODO: HACER
    ...
    \begin{enumerate}
        \item $T$ es compacto si y solo si para toda sucesión acotada de puntos de $E$ existe una parcial suya $\{x_{\sigma(n)}\}$ tal que $\{T(x_{\sigma(n)})\}$ es convergente en $F$.
...
\begin{prop}
    ...
    \begin{center}
        $T$ es compacto si y solo si $\overline{T(\overline{B}_E)}$ es compacto en $F$.
    \end{center}
    En cuyo caso, tendremos entonces que toda sucesión débilmente convergente a un punto de $E$ $\{x_n\}\rightharpoonup x\in E$ verifica que $\{T(x_n)\}\to Tx$.
\end{prop}

\begin{observacion}
    Vemos por la propiedad 3 de la Proposición anterior que si $T$ es lineal y compacta tenemos que $T$ es continua.
\end{observacion}
        ...
        \begin{enumerate}
            \item Si $\lm$ es un valor propio de $T$, existe $u\in H\setminus\{0\}$ de forma que $T(u) = \lm u$, y podemos suponer sin pérdida de generalidad que $\|u\| = 1$, por lo que:
                ...
                \begin{equation*}
                    |\lm| \leq \|Tu\| \leq \|T\|\|u\| = \| T\|
                \end{equation*}
            \item Si tenemos que $Tu = \lm u$ y que $Tv = \beta v$ con $\lm\neq \beta$ y $u,v\neq 0$. Tenemos que:
                \begin{equation*}
            ...
            \item Por reducción al absurdo, si tomamos $V=\ker (T-\lm I)$ con $\lm\neq 0$, supondremos que $\dim V = \infty$. Es claro que $V$ es un subespacio de $H$ que además es cerrado ($V = {(T-\lm I)}^{-1}(\{0\})$), por lo que $V$ también es un espacio de Hilbert, y todos ellos son reflexivos. 
                ...
                Somos capaces de encontrar (pensar en la bola unidad, que no es compacta\footnote{O algo de eso dijo.}) $\{u_k\}$ en $V$ con $\|u_k\| = 1$, $\{u_k\}\rightharpoonup u$ y $\{u_k\}\not\to u$. Como $T$ es compacta, tenemos por la Proposición anterior que $\{T(u_k)\}\to Tu$, pero por otra parte tenemos que $T(u_k) = \lm u_k$, por ser $\{u_k\}\subset V$. En dicho caso:
                \begin{equation*}
                    \{\lm u_k\} = \{T(u_k)\} \to Tu
                \end{equation*}
...
\begin{lema}
    ...
    \begin{proof}
        ...
        \end{center}
        Lo que haremos será buscar $u_1\in H\setminus\{0\}$ con $\|u_1\| = 1$ de forma que $T(u_1) = \lm_1 u_1$, por lo que $\lm_1 = \langle T(u_1),u_1 \rangle $. Es decir, deberemos probar además que $\lm_1$ en realidad es un máximo.\\
...
\begin{lema}
    ...
    \begin{equation*}
        \lm_{-1} = \inf\{\langle Tu,u \rangle : \|u\| = 1\}
    \end{equation*}
    Observemos que $\lm_{-1}$ es valor propio si existe $u_{-1}$ de módulo 1 con $T(u_{-1}) = \lm_{-1}u_{-1}$.
    \begin{enumerate}
        \item Si $dim H = \infty\Longrightarrow \lm_{-1}\leq 0$.
...
\begin{ejercicio} % // TODO: HACER
    ...
    \begin{equation*}
        \|T\| = \sup\{\|Tx\| : \|x\| = 1\} = \sup_{\|u\| =1} |\langle Tu,u \rangle|
    \end{equation*}
\end{ejercicio}
