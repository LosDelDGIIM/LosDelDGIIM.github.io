...
\begin{teo}[Hahn-Banach, versión analítica]
...
% // TODO: Terminar de poner bien el problema inicial

\subsection{Versión geométrica del Teorema}
\noindent
La siguiente veresión del Teorema de Hahn Banach es equivalente a la ya vista. Para ello, será necesraio repasar ciertos conceptos:
...
\subsection{Funcional de Minkowski de un conjunto}
\noindent
Sea $E$ un espacio normado y $C\subset E$ convexo, abierto y que contiene al origen, definimos $p:E\to \mathbb{R}$ por:
\begin{equation*}
    p(x) = \inf\left\{\alpha\in \mathbb{R}^+ : \frac{x}{\alpha}\in C\right\} \qquad \forall x\in E
\end{equation*} % // TODO: Hacer dibujo
observamos que $p(0) = 0$. Veamos que tiene las siguiente propiedades:
\begin{enumerate}
    \item $p(\lm x) = \lm p(x)\qquad \forall x\in E$, $\forall \lm > 0$
        \begin{equation*}
            p(\lm x) = \inf\left\{\alpha\in \mathbb{R}^+ : \frac{x}{\nicefrac{\alpha}{\lm}}  =\frac{\lm x}{\alpha}\in C\right\} \stackrel{\text{$\lm > 0$}}{=} \lm \inf \left\{\alpha\in \mathbb{R}^+ : \frac{x}{\alpha} \in C\right\} = \lm p(x)
        \end{equation*}
    \item $\exists M>0 : 0\leq p(x)\leq M\|x\| \qquad \forall x\in E$

        Como $C$ es abierto y $0\in C$, $\exists r>0$ de forma que $B(0,r)\subset C$. Si tomamos:
        \begin{equation*}
            \alpha> \dfrac{\|x\|}{r} \Longrightarrow \left\|\dfrac{x}{\alpha} \right\|< r \Longrightarrow \dfrac{x}{\alpha} \in B(0,r)\subset C
        \end{equation*}
        Por tanto, $\left]\frac{\|x\|}{r},+\infty\right[\subset \left\{\alpha>0 : \frac{x}{\alpha}\in C\right\}$. De donde el ínfimo de la izaquierda será menor o igual que el ínfimo de la derecha:
        \begin{equation*}
            p(x) \leq \dfrac{\|x\|}{r}
        \end{equation*}
    \item $C= \{x\in E : p(x) < 1\}$
