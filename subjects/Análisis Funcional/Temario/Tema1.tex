...
\begin{coro}
    Sea $E$ un espcio vectorial, $F\subset E$ un subespacio vectorial de forma que $\overline{F}\neq E$, entonces existe $f\in E^\ast$, $f\neq 0$ de forma que:
    \begin{equation*}
...
\noindent   % // TODO: Poner esto bien
Notemos que enunciar el teorema así es equivalente para hacerlo con $G\subset E$ un subespacio cerrado. Nos interesa más esta forma de ver el teorema por su contrarrecíproco:
Si $F\subset E$ subespacio vectorial, si para toda aplicación del dual menos el cero existe $x\in F$ de forma que $f(x) \neq 0$, entonces $F$ es denso en $E$.

Equivalentemente, si $f\in E^\ast\setminus\{0\}$ y $f\big|_F = 0 \Longrightarrow f=0$, entonces $F$ es denso en $E$.
