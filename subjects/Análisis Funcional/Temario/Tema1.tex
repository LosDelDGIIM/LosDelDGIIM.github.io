...
\begin{prop}
    El espacio $l^p$ para $1\leq p \leq \infty$ es de Banach.
    \begin{proof}
        \begin{description}
            \item [Para $p=\infty$.] Recordamos que trabajamos en el espacio:
                ...
                \begin{equation*}
                    |x_p(n) - x_q(n)| \leq \sup_{k \in \mathbb{N}}|x_p(k)-x_q(k)| = \|x_p-x_q\| < \varepsilon \qquad \forall p,q\geq m_0
                \end{equation*}
...
\noindent
Sea $E$ un espacio de banach y $(E^\ast,\|\cdot \|_{E^\ast})$ su espacio dual con:
\begin{equation*}
    \|f\|_{E^\ast} := \sup_{\|x\|\leq 1}|\langle f,x \rangle |
\end{equation*}
tenemos que $(E^\ast, \|\cdot \|_{E^\ast})$ es completo (aunque $E$) no lo fuera. Podemos entonces  definir:
\begin{equation*}
    E^{\ast\ast} = {(E^\ast)}^{\ast}
\end{equation*}
A cuyos elementos solemos denotar por legras griegas, si $\chi:E^\ast \to \mathbb{R}$:
\Func{\chi}{E^\ast}{\bb{R}}{f}{\langle \chi, f\rangle}
y en dicho espacio consideramos:
\begin{equation*}
    \|\chi\| = \sup_{\|f\|\leq 1}|\langle \chi,f \rangle |
\end{equation*}

\begin{ejemplo}
    Sea $x\in E$, le hacemos corresponder $\chi_x$, con:
    \Func{\chi}{E^\ast}{\bb{R}}{f}{\langle f,x\rangle}
    Que es lineal:
    \begin{equation*}
        \chi_x(\lm f+g) = \langle \lm f+g,x \rangle  = \lm \langle f,x \rangle  + \langle g,x \rangle = \chi_x(\lm f)+\chi_x(g)
    \end{equation*}
    Y continua:
    \begin{equation*}
        \|\chi_x\| = \sup_{\|f\|\leq 1}|\chi_x(f)| = \sup_{\|f\|\leq 1}|\langle f,x \rangle | \AstIg \|x\|
    \end{equation*}
    donde en $(\ast)$ usamos un Corolario de Hahn-Banach. En definitiva, tenemos una aplicación:
    \Func{J}{E}{E^{\ast\ast}}{x}{J(x)}
    % // TODO: DEMOSTRAR
    Lineal, inyectiva y que conserva la norma, por lo que si restringimos $J$ en codominio a $J(E)$, tenemos entonces que $E$ y $J(E)$ son isométicos, por lo que podemos ver que $E$ como subespacio vectorial de $E^{\ast\ast}$. 

    Decimos que un espacio $E$ es reflexivo si $J$ es sobreyectiva.
\end{ejemplo}
