...
\begin{ejercicio}
    ...
    \begin{enumerate}
        ...
        \item Prueba que $\forall r<R$, $\exists L\geq 0$ de forma que
            ...
            tomando normas:
            \begin{align*}
                \|x_1 - x_2\| &= (1-t)\|u-x_1\| = (1-t)\|u-x_0+x_0-x_1\| \\ &\geq (1-t)\left|\|u-x_0\| - \|x_0-x_1\|\right| \geq (1-t)(R-r)
            \end{align*}
            por lo que:
            \begin{equation*}
...
\begin{ejercicio}
    ...
    En otras palabras:
    \begin{equation*}
        (|p((\lm_0+t)x_{\sigma(k)})| \leq \varepsilon\quad \forall k\geq \overline{n} ) \quad \forall t\in \left]-\delta,\delta\right[
    \end{equation*}
    Ahora:
    \begin{equation*}
    ...
    de donde:
    \begin{equation*}
        p(\alpha_{\sigma(k)}x_{\sigma(k)}) \geq p((\lm_0 + \alpha_{\sigma(k)}-\alpha)x_{\sigma(k)}) - p((\lm_0-\alpha)x_{\sigma(k)})
    \end{equation*}
...
\begin{ejercicio}\label{ej:5_rel2}
    ...
    \begin{itemize}
        \item Si $x\in B(0,r)$, tenemos para cada $n\in \mathbb{N}$ que:
            ...
            \begin{equation*}
                -g_n(x) = g_n(-x) \leq C(-x) \qquad \forall x\in B(0,r)
            \end{equation*}
            Por lo que tenemos:
            \begin{equation*}
                |g_n(x)| \leq \tilde{C}(x) := \max\{C(x), C(-x)\} \qquad \forall x\in B(0,r)
            \end{equation*}
        \item Si $x\in E$ con $\|x\|\geq r$, si consideramos $y=\frac{r}{2}\frac{x}{\|x\|}$ tenemos que $y\in B(0,r)$, por lo que para cada $n\in \mathbb{N}$ podemos aplicar la cota anteriormente conseguida a $y$, y usando además que $g_n$ es lineal obtenemos:
            \begin{equation*}
                \tilde{C}(y) \geq |g_n(y)| = \frac{r}{2\|x\|}|g_n(x)| \quad \Longrightarrow \quad |g_n(x)| \leq \frac{2\|x\|}{r}\tilde{C}(y)
            \end{equation*}
    \end{itemize}
    ...
    Por lo que:
    \begin{equation*}
        C\geq \|g_n\| = \|f_n\|(1-\varepsilon_n\|g_n\|) \geq \|f_n\|(1-\varepsilon_n M)
    \end{equation*}
    Y como $\{1-\varepsilon_n M\}\to 1$, $\exists m\in \mathbb{N}$ de forma que
    \begin{equation*}
        1-\varepsilon_n M > \frac{1}{2} \qquad \forall n\geq m
    \end{equation*}
    Por lo que:
    \begin{equation*}
        C\geq \|f_n\|(1-\varepsilon_n M) > \|f_n\|\cdot \frac{1}{2} \Longrightarrow \|f_n\| \leq 2M \qquad \forall n\geq m
    \end{equation*}
    De donde tenemos que $\{f_n\}$ está acotada.
\end{ejercicio}
...
\begin{ejercicio}
    Sea $\alpha = \{\alpha_n\}$ una sucesión de números reales y sea $1\leq p \leq \infty$. Supongamos que $\sum\limits_{n=1}^{\infty} |\alpha_n||x_n| < \infty$ para cada elemento $x=\{x_n\}$ de $l_p$. Prueba que $\alpha\in l_{p'}$.\\

    \noindent
    Recordamos que para $1\leq p < \infty$ teníamos:
    \begin{equation*}
        l_p = \left\{x:\mathbb{N}\to\mathbb{R} \text{\ tales que\ } \sum_{n=1}^{\infty}{|x_n|}^{p}<\infty\right\}, \qquad 
    ...
    \noindent
    Para cada $n\in \mathbb{N}$ definimos $T_n:l^p\to \mathbb{R}$ dada por:
    \begin{equation*}
        T_n(x) = \sum_{k=1}^{n}\alpha(k)x(k)
    \end{equation*}
    Fijado $n\in \mathbb{N}$, es claro que $T_n$ es lineal, y vemos que es continua, puesto que:
    \begin{equation*}
        |T_n(x)| = \left|\sum_{k=1}^{n}\alpha(k)x(k)\right| \leq \sum_{k=1}^{n}|\alpha(k)||x(k)| \leq \sum_{k=1}^{n}(|\alpha(k)|\|x\|_p) = \sum_{k=1}^{n}|\alpha(k)|\cdot \|x\|_p
    \end{equation*}
    Si definimos $T:l^p\to \mathbb{R}$ dada por:
    \begin{equation*}
        T(x) = \sum_{n=1}^{\infty}\alpha(k)x(k)
    \end{equation*}
    Tenemos por la hipótesis del ejercicio que $T$ está bien definida, así como que $\{T_nx\}\to Tx$. De esta forma, podemos aplicar el Teorema de Banach-Steinhaus, obteniendo que $\sup\limits_{n\in \mathbb{N}}\|T_n\|<\infty$. Tomando $C\in \mathbb{R}$ con $C\geq \sup\limits_{n\in \mathbb{N}}\|T_n\|$ vemos que:
    \begin{equation*}
        |T_n(x)| \leq \|T_n\|\|x\|_p \leq C\|x\|_p \qquad \forall x\in l^p, \quad \forall n\in \mathbb{N}
    \end{equation*}
    Por lo que:
    \begin{equation*}
        |T(x)| \leq C\|x\|_p \qquad \forall x\in l^p
    \end{equation*}
    Distinguimos casos:
    \begin{description}
        \item [Si $p=\infty$.] Consideramos como $x$ la sucesión:
            \begin{equation*}
                x = \{sgn(\alpha(n))\}_{n\in \mathbb{N}} \in l^\infty
            \end{equation*}
            Y tenemos que:
            \begin{equation*}
                \sum_{n=1}^{\infty}|\alpha(n)| = \left|\sum_{n=1}^{\infty}|\alpha(n)|\right| = \left|\sum_{n=1}^{\infty}T(x)\right| \leq C\|x\|_\infty = C
            \end{equation*}
            Por lo que $\|\alpha\|_1 < \infty$, de donde $\alpha\in l^1$.
        \item [Si $p=1$.] Para cada $n\in \mathbb{N}$ consideramos:
            \begin{equation*}
                x = e_n 
            \end{equation*}
            Y tenemos que:
            \begin{equation*}
                |\alpha(n)| = \left|\sum_{k=1}^{n} \alpha(k)e_n\right| = |T_n(x)| \leq C\|x\|_1 = C
            \end{equation*}
            Por lo que $\alpha\in l^\infty$.
        \item [Si $1<p<\infty$.] Dado $N\in \mathbb{N}$, tenemos que:
            \begin{align*}
                |T_Nx| &= \left|\sum_{k=1}^{N}\alpha(k)x(k)\right| \leq \sum_{k=1}^{N}|\alpha(k)||x(k)| \leq {\left(\sum_{k=1}^{N}{|\alpha(k)|}^{p'}\right)}^{\frac{1}{p'}}{\left(\sum_{k=1}^{N}{|x(k)|}^{p}\right)}^{\frac{1}{p}} \\ 
                       &\leq {\left(\sum_{k=1}^{N}{|\alpha(k)|}^{p'}\right)}^{\frac{1}{p'}}\|x\|_p \qquad \forall x\in l^p
            \end{align*}
            Por lo que:
            \begin{equation*}
                \|T_nx\| \leq {\left(\sum_{k=1}^{N}{|\alpha(k)|}^{p'}\right)}^{\frac{1}{p'}}
            \end{equation*}
            Si consideramos ahora la sucesión $x$ dada por:
            \begin{equation*}
                x_n = \left\{\begin{array}{ll}
                    sgn(\alpha_n)|\alpha_n|^{p'-1} & \text{si\ } 1\leq n \leq N \\
                     0& \text{si\ } n> N
                \end{array}\right.  \in l^p
            \end{equation*}
            Tenemos entonces que:
            \begin{equation*}
                |T_Nx| = {\left(\sum_{k=1}^{N}{|\alpha(k)|}^{p'}\right)}^{\frac{1}{p'}}\|x\|_p
            \end{equation*}
            Por lo que:
            \begin{equation*}
                \|T_Nx|| = {\left(\sum_{k=1}^{N}{|\alpha(k)|}^{p'}\right)}^{\frac{1}{p'}} \qquad \forall N\in \mathbb{N}
            \end{equation*}
            De donde tenemos que:
            \begin{equation*}
                {\left(\sum_{k=1}^{N}{|\alpha(k)|}^{p'}\right)}^{\frac{1}{p'}} \leq C\qquad \forall N\in \mathbb{N} \qquad \Longrightarrow\qquad  \alpha\in l^{p'}
            \end{equation*}
    \end{description}
\end{ejercicio}
...
\begin{ejercicio}\label{ej:10_rel2}
...
\begin{ejercicio}\label{ej:15_rel2}
    ...
    \noindent
    Definimos $T:E_1\times E_2\to F$ dada por:
    \begin{equation*}
        T(x,y) = T_1(x) + T_2(y) \qquad \forall (x,y)\in E_1\times E_2
    \end{equation*}
    Y tenemos que $T$ es lineal, puesto que si $(x,y),(u,v)\in E_1\times E_2$ y $t\in \mathbb{R}$ entonces:
    \begin{align*}
        T(t(x,y) + (u,v)) &= T(tx+u, ty+v) = T_1(tx+u) + T_2(ty+v) \\
                          &= t(T_1(x) + T_2(y)) + T_1(u) + T_2(v) = tT(x,y) + T(u,v)
    \end{align*}
    Además, si consideramos en $E_1\times E_2$ la norma de la suma, tenemos que $T$ es continua, puesto que:
    \begin{align*}
        \|T(x,y)\| &= \|T_1(x) + T_2(y)\| \leq \|T_1(x)\| + \|T_2(y)\| \leq \|T_1\|\|x\|+ \|T_2\|\|y\| \\ &\leq \max\{\|T_1\|,\|T_2\|\}(\|x\|+\|y\|)
    \end{align*}
    Observamos ahora que:
    \begin{equation*}
        Im T = \{T(x,y) = T_1(x) + T_2(y) : (x,y)\in E_1\times E_2\} = Im T_1 + Im T_2 = F
    \end{equation*}
    Por lo que $T$ es sobreyectiva. Como $E_1$ y $E_2$ son de Banach, tendremos que $E_1\times E_2$ es de Banach (con la norma de la suma), por lo que podemos aplicar el Teorema de la aplicación abierta, obteniendo que $T$ es una aplicación abierta. Buscamos probar que $Im T_1$ es cerrado. Para ello, escribimos:
    \begin{equation*}
        Im T_1 = \{T_1(x) : x\in E_1\} = \{T(x,y) : x\in E_1, y\in \ker T_2\} = T(E_1\times \ker T_2)
    \end{equation*}
    Estamos en las condiciones del Ejercicio~\ref{ej:10_rel2}, por lo que tenemos que el conjunto $Im T_1 = T(E_1 \times \ker T_2)$ es cerrado en $F$ si y solo si $(E_1\times \ker T_2)+\ker T$  es cerrado en $E_1\times E_2$. Observemos ahora que:
    \begin{equation*}
        \ker T = \ker T_1 \times \ker T_2
    \end{equation*}
    \begin{description}
        \item [$\supseteq)$] Es clara.
        \item [$\subseteq)$] Sea $(u,v)\in \ker T$, tenemos entonces que:
            \begin{equation*}
                0 = T(u,v) = T_1(u) + T_2(v) \Longrightarrow T_1(u) = -T_2(v)
            \end{equation*}
            \begin{itemize}
                \item Si $T_1(u) = 0$ entonces $T_2(v) = 0$ y tenemos que $(u,v)\in \ker T_1\times \ker T_2$.
                \item Supuesto que $T_1(u)\neq 0$, tenemos entonces que $T_2(-v) = T_1(u)$, por lo que $T_1(u) \in Im T_1 \cap Im T_2 = \{0\}$, luego este caso es imposible.
            \end{itemize}
    \end{description}
    Por lo que tenemos que:
    \begin{equation*}
        (E_1\times \ker T_2) + \ker T = (E_1\times \ker T_2) + (\ker T_1 \times \ker T_2) = E_1\times \ker T_2
    \end{equation*}
    de donde $Im T_1$ es cerrado si y solo si $E_1\times \ker T_2$ es un conjunto cerrado, y este último conjunto sí que es cerrado, como producto cerrados ($\ker T_2$ es cerrado porque si $\{y_n\}\to y$ es una sucesión de $\ker T_2$ entonces $\{T_2(y_n)\} = \{0\} \to T_2(y)$, luego ha de ser $y \in \ker T_2$). Análogamente se comprueba que $Im T_2$ es cerrado, ya que por el Ejercicio~\ref{ej:10_rel2} tenemos que este conjunto es cerrado si y solo si el conjunto:
    \begin{equation*}
        (\ker T_1 \times E_2) + \ker T = \ker T_1 \times E_2
    \end{equation*}
    es cerrado.
\end{ejercicio}
...
\begin{ejercicio}
    Sean $E$ y $F$ dos espacios de Banach y sea $T\in L(E,F)$. Si $Im T$ tiene codimensión finita, es decir, existe un subespacio vectorial $X$ de $F$ de dimensión finita de forma que $X+Im T = F$ y $X\cap Im T = \{0\}$; prueba que $Im T$ es cerrado.\\

    \noindent
    Como $dim X < \infty$ tenemos que $X$ es un subespacio vectorial cerrado de $F$, y como $F$ es de Banach tendremos que $X$ también será de Banach. Si consideramos:
    \Func{T_2}{X}{X}{x}{x}
    Tenemos que $T_2\in L(X,X)$, y estamos en las hipótesis del Ejercicio~\ref{ej:10_rel2} con $T_1 = T$. Deducimos que $Im T$ es cerrado.
\end{ejercicio}
