...
El modelo de programación de memoria compartida ofrece varias ventajas al programador respecto al modelo basado en paso de mensajes: evita (o al menos relaja) la necesidad de distribuir la carga de trabajo (código y datos). Esta es la principal razón por la que los multiprocesadores han tenido una amplia aceptación tanto en el mundo comercial como en computación científica.\\

Sin embargo, para programar de forma correcta un multiprocesador, los programadores deben conocer la semántica del sistema de memoria. Si programan en bajo nivel con ensamblador o desarrollando herramientas de programación, sistemas operativos, controladores, \ldots deben conocer la semántica proporcionada por el multiprocesador, también llamado \emph{modelo de consistencia de memoria del hardware} (la arquitectura abstracta del sistema de memoria). Por otra parte, si programan en alto nivel haciendo uso de herramientas de programación, deben tener en cuenta la semántica del sistema de memoria proporcionada por dichas herramientas software, también llamado \emph{modelo de consistencia de memoria del software}.
...
Un \textbf{modelo de consistencia de memoria} es un modelo abstracto del sistema de memoria que nos ayuda a especificar el orden en el cual las operaciones de acceso a memoria deben \emph{parecer} haberse realizado (esto es, las operaciones de lectura y escritura). Es de vital importancia conocer en qué orden se escribe o se lee de memoria, para poder desarrollar programas sin toparnos con errores indeseados.\\

En sistemas \underline{uniprocesadores}, el orden en el que deben parecer haberse ejecutado los accesos a memoria es el orden secuencial especificado por el programador en el programa desarrollado, denominado \textbf{orden del programa}. Sin embargo, esto no significa que el hardware o que las herramientas de programación que usamos (como copiladores con opciones de optimización) puedan alterar el orden de las instrucciones para mejorar las prestaciones. De hecho lo hacen bastante a menudo, pero debe parecer que no se ha alterado, para garantizarle al programador el \underline{modelo de consistencia}. En concreto, debe garantizarse que:
\begin{itemize}
...
\begin{itemize}
    ...
    \item Globalmente, se debe mantener un orden secuencial entre toas las operaciones (atomicidad de las operaciones).
\end{itemize}
...
Los modlos de consistencia de memoria relajados difieren en cuanto a los requisitos para ganrantizar la consistencia secuencial que relajan:
\begin{description}
    \item [Orden del programa.] Los modelos pueden permitir que en el código ejecutado en un procesador se relaje el orden entre dos accesos a distintas direcciones. Nos encontramos distintos tipos de modelos, que podemos clasificar en relación a las órdenes que nos permiten relajar. Podemos encontrar alteraciones entre:
        \begin{itemize}
            \item escrituras ($W\rightarrow W$).
            \item lecturas ($R\rightarrow R$).
            \item una lectura y una escritura posterior ($W\rightarrow R$).
            \item una escritura y una lectura posterior ($R\rightarrow W$).
        \end{itemize}
    \item [Atomicidad.] Hay modelos que permiten que un procesador pueda leer el valor escrito por otro procesador en uan posición de memoria antes de que dicha escritura se haga visible al resto de procesadores.
\end{description}
Una relajación habitual es que un procesador pueda leer con antelación una escritura propia. Dicha relajación está relacionada tanto con el orden del programa como con la atomicidad de la operación.
...
Con esta relajación, la mayor parte de los códigos de para consistencia secuencial funcionan correctamente. Cuando sea necesario, se pueden utilizar para garantizar un resultado correcto instrucciones especiales de ordenación que garanticen el orden en el programa. 
...
\subsubsection{Cualquier reordenación}
...
% // TODO: Seguir por libro tocho, comienzo página 565
ESTOS APUNTES ESTÁN AÚN EN DESAROLLO.

% --------------------------------------------------
% // TODO: Apuntes de clase

Si no permitimos que las escrituras sean adelantadas, el tiempo de ejecución del código se vería muy perjudicado.
Para ejecutar operaciones es necesario leer datos de memoria; cuando antes consiga leer, antes se realizarán las operaciones en la CPU.
Esto se puede hacer siempre y cuando las escrituras y lecturas sean en distintas posiciones de memoria (si no, se produciría un RAW).
En la actualidad, todos los procesadores permiten que una lectura adelante a una escritura anterior en distintas posiciones de memoria.
También podemos hacer que lecturas adelanten a lecturas (por ejemplo, a una que haya dado fallo en caché, que puede suponer 2 o 4 paquetes, dependiendo del protocolo de coherencia). Nos permite también reducir tiempo de ejecución.

Todos los modelos de consistencia relajan los accesos a memoria.
El propio sistema de memoria también permite reordenar los accesos a memoria, de forma que se realicen en orden distinto al que se han emitido.
El compilador también reordena operaciones independientes y accesos a memoria independientes.

La consistencia de memoria está relacionado con el orden de los accesos a memoria.

El modelo de consistencia de memoria hardware lo que hace es visualizar al resto de procesadores un acceso a memoria. Especifica el orden en el que los accesos a memoria PARECEN haberse realizado. En realidad, los accesos al sistema de memoria se hacen en paralelo, pero a efectos de los que vemos en el programa, parece que se han realizado de forma secuencial.
Este debe tenerse en cuenta sobretodo cuando va a implementar herramientas de sincronización. Las funciones del sistema operativo deben conocer bien el modelo de consistencia de memoria.
También las herramientas de programación paralela ofrecen un modelo a nivel software que no tiene por qué coincidir con el del hardware. 

Notemos que la coherencia sólo se refiere a accesos a memoria a una misma dirección (todos los núcleos tienen que ver el mismo orden en los accesos a memoria a una misma dirección). En consistencia, habalmos del orden de los accesos pero a todas las posiciones de memoria.

Con OpenMP tenemos un modelo de consistencia secuencial, el que el programador desearía que tuviese. Lo que se obtendría al ejecutar un código paralelo es lo que por lógica uno espera que se obtenga.
Ver diapositiva 85. en un x86 sí pero en un ARM no por su modelo de consistencia (puede pasar que la escritura en k se realice antes (la adelante) que en A, haciendo una ejecución del programa que no esperamos). Además, un compilador podría reordenar las instrucciones o eliminar código muerto. Para ello, tenemos que declarar la variable como `volatile`.

Para garantizar consistencia secuencial, se debe cumplir:
- Todos los procesadores deben ver los accesos a memoria de todos ellos en el mismo orden en el que esos accesos están en el código que ejecutan. Orden del programa.
- Todos los procesadores tienen que ver el mismo orden en el acceso a memoria, aunque ambos vean el orden del programa (ver diap. 87 en ejemplo 3, vemos siempre 1 antes de 2 y a antes de b). Orden/Serialización global.
Diapositiva 86: En el 2o ej vemos que no se cumple la 2a. Puede suceder que a P3 llegue la actualización B=1 pero que A=0, aunque en P2 haya A=1. La lectura de P3 es adelantada al resto, con un protocolo de coherencia de actualización. En el nuestro de invalidación esto no puede suceder.

A->B significa que A está primero y luego B y que dicho orden no se asegura (se ejecuta B antes que A).

Modelos relajados:
- orden del programa: permiten que instrucciones adelanten a otras.
- atomicidad: Permiten que un procesador lea antes que otro.
