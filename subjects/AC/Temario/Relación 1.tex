...
\begin{ejercicio}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            P_{\max} = \frac{S_{\max}}{N_{\text{etapas}}} = \frac{3}{4}
        \end{equation*}
        % // TODO: Qué es la productividad de un cauce?
...
\begin{ejercicio} \label{ej:1.3}
    ...
    \begin{align*}
        %T_{CPU}^1 &= NI^1 \cdot (\underbrace{0.2 \cdot 4}_{\texttt|BR|} + \underbrace{0.8 \cdot 3}_{\verb|CMP|+\text{Resto}}) \cdot T_c^1 = NI^1 \cdot 3.2 \cdot 0.75 \cdot T_c^2 = NI^1 \cdot 2.4 \cdot T_c^2\\
        %T_{CPU}^2 &= NI^1 \cdot (\underbrace{0.2 \cdot 4}_{\verb|CMPBR|} + \underbrace{0.6\dot 3}_{\text{Resto}}) \cdot T_c^2 = NI^1 \cdot 2.6 \cdot T_c^2
    \end{align*}
