...
\begin{proof}
    ...
    \begin{equation*}
        f(g(p)) = f(o+k_2\vec{op}) = o+k_1\vec{o(g(p))} = o + k_1(g(p)-o) = o + k_1(\cancel{o}+k_2\vec{op}-\cancel{o}) = o + k_1k_2\vec{op} = H_{o,k_1k_2}(p)
    \end{equation*}
\end{proof}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Representación gráfica de la proyección afín, donde se ve que el vector $\vec{p_0~\pi_{S,T}(q)}=\pi_{\vec{S}, \vec{T}}(\vec{p_0q})$ no contiene la componente en $\vec{T}$, por ser la proyección en $\vec{S}$.}
\end{figure}
...
\begin{definicion}[Simetría afín]
    ...
    \Func{\sigma_{S,T}}{\cc{A}}{\cc{A}}{q}{p_0+\sigma_{\vec{S}, \vec{T}}\left(\vec{p_0q}\right) = p_0+\pi_{\vec{S}, \vec{T}}\left(\vec{p_0q}\right)-\pi_{\vec{T}}\left(\vec{p_0q}\right)}
\end{definicion}
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
...
\section{Teorema de Pappus y Desargues}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            ...
            \end{split} 
        \end{equation*}
        Por tanto, tenemos que $\vec{R_{a_1c_2}} = \cc{L}\{\vec{a_1c_2}\} = \cc{L}\{\vec{c_1a_2}\} = \vec{R_{c_1a_2}}$. Por tanto, $R_{a_1c_2}\| R_{c_1a_2}$.
...
\begin{teo}[de Desargues]
    Sea $\cc{A}$ un espacio afín,
    %// TODO: Teorema de Desargues    
\end{teo}
