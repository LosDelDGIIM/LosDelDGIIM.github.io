...
\begin{equation*}\begin{split}
    ...
    = O' + \vec{O'O} + \frac{1}{k}\sum_{i=1}^k \vec{OP_i}\\
...
\Func{f_{\cc{R}}}{\cc{A}}{\bb{R}^n}{q}{q_{\cc{R}}=(x_1,\dots,x_n)}
...
Entonces, tenemos el siguiente corolario:
...
\begin{teo}
    ...
    \begin{equation*}
        \exists v_0\in \cc{A'}\mid f=t_{v_0}\circ g \Longleftrightarrow \vec{f}=\vec{g}
    \end{equation*}
...
\begin{proof}\
    \begin{description}
        ...
        Veamos ahora que $f$ es una homotecia:
        \begin{equation*}
    ...
    \begin{equation*}
        f(p)=p \Longleftrightarrow o+k\vec{op} = p \Longleftrightarrow o+kp-ko = p \Longleftrightarrow (1-k)o = (1-k)p \Longleftarrow p=o
    \end{equation*}
\end{proof}
...
\begin{definicion}[Proyección]
    ...
    \Func{\pi_S}{\cc{A}}{S}{q}{p_0+\pi_{\vec{S}}\left(\vec{p_0q}\right)}
\end{definicion}
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
    \caption{Representación gráfica de la proyección afín, donde se ve que el vector $\vec{p_0~\pi_S(q)}=\pi_{\vec{S}}(\vec{p_0q})$ no contiene la componente en $\vec{T}$, por ser la proyección en $\vec{S}$.}
\end{figure}
...
\begin{prop}
    ...
    \begin{enumerate}
        \item $\pi_S:\cc{A}\to S$ es una aplicación afín, con $\vec{\pi_S}=\pi_{\vec{S}}$.
        \item $\cc{P}_{\pi_S}=Im(\pi_{S})=S$.
        \item $\pi_{S}\circ \pi_S =\pi_S$ (idempotencia).
    \end{enumerate}
\end{prop}
...
\begin{enumerate}
    ...
    \begin{equation*}
        \vec{\pi_S}(\vec{pq}) = \vec{\pi_S(p)\pi_S(q)} = \vec{(p_0+\pi_{\vec{S}}(\vec{p_0p}))(p_0+\pi_{\vec{S}}(\vec{p_0q}))} = \pi_{\vec{S}}(\vec{p_0q})-\pi_{\vec{S}}(\vec{p_0p}) = \pi_{\vec{S}}(\vec{pq})
    \end{equation*}
    ...
    \begin{equation*}
        \pi_S(q)=q \Longleftrightarrow p_0 + \pi_{\vec{S}}(\vec{p_0q}) = q
        \Longleftrightarrow \pi_{\vec{S}}(\vec{p_0q}) = \vec{p_0q}
        \Longleftrightarrow \vec{p_0q}\in \vec{S}
        \Longleftrightarrow q\in S
    \end{equation*}
    donde he aplicado que los vectores propios de $\pi_{\vec{S}}$ con valor propio $1$ son los vectores de $\vec{S}$.

    \item Tenemos que $\pi_S(q)\in S$ para todo $q\in \cc{A}$. Por tanto, es un punto fijo, por lo que:
    \begin{equation*}
        (\pi_S\circ \pi_S)(q) = \pi_S(\pi_S(q)) = \pi_S(q) \hspace{1cm} \forall q\in \cc{A}
    \end{equation*}
\end{enumerate}
\end{proof}

\begin{definicion}[Simetría afín]
    Sean $S,T$ subespacios afines complementarios de $\cc{A}$. Entonces, por la Proposición \ref{prop:ComplementariosSumaIntersec}, tenemos que $S\cap T=\{p_0\}$. Entonces, definimos la simetría (o reflexión) afín en $S$ paralela a $T$ como:
    \Func{\sigma_S}{\cc{A}}{\cc{A}}{q}{p_0+\sigma_{\vec{S}}\left(\vec{p_0q}\right) = p_0+\pi_{\vec{S}}\left(\vec{p_0q}\right)-\pi_{\vec{T}}\left(\vec{p_0q}\right)}
\end{definicion}
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
...
Respecto a las simetrías, tenemos los siguientes resultados:
\begin{prop}
    Sea $\cc{A}$ un espacio afín, y sean $S,T$ subespacios afines complementarios de $\cc{A}$. Entonces:
    \begin{enumerate}
        \item $\sigma_S:\cc{A}\to \cc{A}$ es una aplicación afín, con $\vec{\sigma_S}=\sigma_{\vec{S}}$.
        \item $\cc{P}_{\pi_S}=S$.
        \item $\sigma_S\circ \sigma_S =Id_{\cc{A}}$ (involución).
    \end{enumerate}
\end{prop}
\begin{proof} Demostramos cada resultado por separado:
\begin{enumerate}
    \item Veamos que la dada es la aplicación lineal asociada:
    \begin{equation*}
        \vec{\sigma_S}(\vec{pq}) = \vec{\sigma_S(p)\sigma_S(q)} = \vec{(p_0+\sigma_{\vec{S}}(\vec{p_0p}))(p_0+\sigma_{\vec{S}}(\vec{p_0q}))} = \sigma_{\vec{S}}(\vec{p_0q})-\sigma_{\vec{S}}(\vec{p_0p}) = \sigma_{\vec{S}}(\vec{pq})
    \end{equation*}
    donde he empleado que las simetrías vectoriales son aplicaciones lineales.

    \item Veamos los puntos fijos de las simetrías:
    \begin{equation*}
        \sigma_S(q)=q \Longleftrightarrow p_0 + \sigma_{\vec{S}}(\vec{p_0q}) = q
        \Longleftrightarrow \sigma_{\vec{S}}(\vec{p_0q}) = \vec{p_0q}
        \Longleftrightarrow \vec{p_0q}\in \vec{S}
        \Longleftrightarrow q\in S
    \end{equation*}
    donde he aplicado que los vectores propios de $\sigma_{\vec{S}}$ con valor propio $1$ son los vectores de $\vec{S}$.

    \item En primer lugar, tenemos que $\vec{\sigma_S\circ \sigma_S} = \sigma_{\vec{S}}\circ \sigma_{\vec{S}} = Id_{\vec{A}}$.

    Además, $\exists a\in \cc{A}$ tal que $\sigma_S(a)=Id_{\cc{A}}(a)=a$, ya que $\sigma_S$ tiene puntos fijos.
