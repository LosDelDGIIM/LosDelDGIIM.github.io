...
% Parámetro 5: Expresión de la función
    ...
    \end{cases}
        ...
        \item Uso del comando personalizado \verb|\AstIg|.
        
        Para hacer referencia a igualdades de las que se ha de destacar algún aspecto, se puede usar este comando, obteniendo $\AstIg$.
    \end{itemize}

    \section{Sobre el compilado de figuras o fotos}
    Es habitual querer incluir en nuestro documento fotos o figuras complejas. La inclusión de estos elementos puede llegar a ralentizar el proceso de compilado del documento que genera \LaTeX, y con ello ralentizar el proceso de elaboración de documentos en \LaTeX. Para evitarlo, es una buena praxis disponer de un comando que impida el compilado de las porciones de código \LaTeX más costosas de nuestro documento, para así tener un comando que nos permita habilitar o deshabilitar su compilado. Querremos deshabilitarlo cuando estemos haciendo cambios en el documento que no tengan nada que ver con las figuras mencionadas, para acelerar el compilado; y por otra parte queremos habilitarlas para generar el documento final o bien para visualizar las figuras.

    Para conseguir este comportamiento, especificaremos dentro de nuestro documento \texttt{.tex} principal los comandos:
    \begin{listing}[H]
        \begin{minted}{latex}
            \newif\ifshowfigures
            \showfiguresfalse  % <---- Por defecto NO se muestran
        \end{minted}
    \end{listing}
    Y rodearemos cada figura que no queremos que se muestre para acelerar la compilación con los comandos:
    \begin{listing}[H]
        \begin{minted}{latex}
            \ifshowfigures
            \begin{figure}
               % Contenido de figura que no quiero que se muestre en cada compilado
            \end{figure}
            \fi
        \end{minted}
    \end{listing}
    Así, cuando queramos volver a que se vean todas las figuras de nuestro documento bastará cambiar el comando \texttt{showfiguresfalse} por el comando \texttt{showfigurestrue}.
...
\input{../../_assets/preambulo.tex} % Ruta correspondiente, puede elegirse otra
