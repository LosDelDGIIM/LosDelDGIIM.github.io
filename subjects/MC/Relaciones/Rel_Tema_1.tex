...
\begin{enumerate}
    ...
    \item Si $L$ es un lenguaje finito, entonces su cabecera ($CAB(L)$) también será finita.\\
        ...
        Verdadero, supongamos que tenemos un lenguaje finito $L$ formado por $n\in \mathbb{N}$ palabras. Dada una palabra $u\in L$ de longitud $m\in \mathbb{N}$, dicha palabra tendrá un total de $m$ prefijos distintos de $\veps$, todos ellos contenidos en $CAB(L)$. Definimos
        \Func{f}{L}{\mathbb{N}}{u}{|u|}
        la función que a cada palabra le asigna su longitud. Como $CAB(L)$ está formada por todos los prefijos de palabras de $L$, tendrá al menos $m$ palabras por cada palabra de $L$ de longitud $m$ más la palabra vacía. De esta forma:
        \begin{equation*}
            \# CAB(L) \leq \sum_{u\in L} f(u) + 1 < \infty
        \end{equation*}
        Donde no se da necesariamente la igualdad ya que dos palabras de $L$ pueden tener uno o más prefijos en común.
    ...
    \item Si $A$ es un alfabeto, entonces $A^+$ no incluye nunca la palabra vacı́a.
