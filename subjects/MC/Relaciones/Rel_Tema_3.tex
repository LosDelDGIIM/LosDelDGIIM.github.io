...
\begin{ejercicio}\label{ej:1.3.22}
    ...
    \begin{enumerate}
        ...
        %\begin{equation*}
        %    AB,\qquad \text{donde} A=S\quad \text{o} \quad B=q_3,q_5
        %\end{equation*}
    \end{enumerate}
\end{ejercicio}
...
\begin{ejercicio}\label{ej:1.3.24}
    ...
    \begin{figure}
        ...
        \resizebox{1.1\textwidth}{!}{
        \begin{tikzpicture}
            \node[state, initial] (p0) {$q_0p_0, q_1p_0$};
            \node[state, right of=p0] (p1) {$q_1p_1$};
            \node[state, right of=p1] (p2) {$q_2p_2$};
            \node[state, right of=p2, accepting, xshift=1em] (p3) {$q_0p_3, q_1p_3, q_2p_3$};
            \node[state, right of=p3, xshift=3em] (p4) {$q_1p_4, q_3p_4$};
            \node[state, right of=p4] (p5) {$q_1p_5$};
            \node[state, error, below of=p2] (E) {$E$};
    ...
    \end{figure}
    Como podemos ver en el AFD minimal de la Figura~\ref{fig:1.3.24-L1-L2-minimal}, este es isomorfo al de la Figura~\ref{fig:1.3.24-L2}, por lo que $L_1\cap L_2 = L_2$. Esto es algo que podríamos haber deducido al principio, ya que si $L_2\subset L_1$.
\end{ejercicio}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw   (q0) edge[above] node{0} (q1)
                    ...
                    (q4) edge[above] node{1} (q5)
                    (q5) edge[above, bend right] node{0} (q0);
        \end{tikzpicture}
        \caption{AFND asociado a la expresión regular del Ejercicio~\ref{ej:1.3.30}.}
        \label{fig:1.3.30-ND}
    \end{figure}

    El AFD asociado a la expresión regular es el de la Figura~\ref{fig:1.3.30}.
    \begin{figure}
        \centering
        \begin{tikzpicture}
            \node[state, initial, accepting] (q0) {$q_0$};
            ...
            \node[state, right of=q2q5, accepting] (q0q3) {$q_0q_3$};
            \node[state, right of=q0q3, accepting] (q2) {$q_2$};
            \node[state, right of=q2] (q3) {$q_3$};
            ...
            \draw   (q0) edge[above] node{0} (q1q4)
                    ...
                    (q2q5) edge[above] node{0} (q0q3)
                    (q2q5) edge[right] node{1} (E)
                    (q0q3) edge[above] node{1} (q2)
                    (q0q3) edge[above, bend right] node{0} (q1q4)
                    (q2) edge[above] node{0} (q3)
                    (q2) edge[above] node{1} (E)
        ...
        \begin{figure}
            ...
            \begin{tikzpicture}
                ...
                \draw   (q0) edge[above] node{0} (q1)
                        ...
                        (q2) edge[below, bend left] node{2} (q0)
...
\begin{enumerate}
    ...
    \item El conjunto de los números en binario que son múltiplos de 7 es regular.\\
        ...
        Verdadero, ya que podemos constuir un autómata finito determinista que reconozca dicho lenguaje, como vemos en la Figura~\ref{fig:tipo_test22}, donde hemos hecho uso de la tabla de estados~\ref{tab:estados}:
        \begin{table}[H]
        \centering
