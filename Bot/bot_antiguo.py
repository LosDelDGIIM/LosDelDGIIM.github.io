...
DOWNLOAD_BASEDIR = '/home/arturo/Bot'
...
def send_email(asunto, remitente, mensaje, adjuntos=[]) -> None:
    ...
    smtp_port = 587
    smtp_user = 'enviar_correos@yahoo.com'
    smtp_password = 'send_emails_PASS#'

    # Configuración del correo
    sender_email = smtp_user
    receiver_email = 'losdeldgiim@gmail.com'

    # Crear el mensaje MIME con el nombre del remitente
    msg = MIMEMultipart()
    ...
    # Añadir los archivos y capturas al correo
    ...
    # Enviar el correo y devolver si se ha enviado correctamente
    try:
        server = smtplib.SMTP(smtp_server, smtp_port)
        server.starttls()  # Iniciar TLS para asegurar la conexión
...
def gestionar_datos(datos: dict) -> bool:
    ...
    asunto = datos.get(OPTION_KEY)
    remitente = get_userIdentifier(datos)
    
    message_text = "<html><body>\n"
    message_text +="<h1>Colaboración: " + datos.pop(OPTION_KEY) + "</h1>\n"
    message_text += "<ul>\n"
